...
class AnalyzeWearableDataInput(BaseModel):
    ...
    metrics_of_interest: Optional[List[str]] = Field(
        None, description="Métricas específicas de interés (HRV, sueño, recuperación, etc.)"
    )
...
class WearableMetric(BaseModel):
    ...
    optimal_range: Optional[str] = Field(None, description="Rango óptimo para esta métrica")
...
class AnalyzeWearableDataOutput(BaseModel):
    ...
    metrics: List[WearableMetric] = Field(..., description="Métricas extraídas e interpretadas")
    ...
    recommendations: List[Dict[str, Any]] = Field(..., description="Recomendaciones personalizadas")
    biohacking_opportunities: List[Dict[str, Any]] = Field(..., description="Oportunidades de biohacking basadas en los datos")
...
class AnalyzeBiomarkerResultsInput(BaseModel):
    ...
    test_type: Optional[str] = Field(
        None, description="Tipo de prueba (análisis de sangre, genética, microbioma, etc.)"
    )
...
class BiomarkerResult(BaseModel):
    ...
    significance: str = Field(..., description="Significado e importancia del biomarcador")
    confidence: float = Field(..., description="Confianza en la extracción (0.0-1.0)")
...
class AnalyzeBiomarkerResultsOutput(BaseModel):
    ...
    biomarkers: List[BiomarkerResult] = Field(..., description="Biomarcadores extraídos y analizados")
    analysis_summary: str = Field(..., description="Resumen del análisis de los resultados")
    health_insights: List[str] = Field(..., description="Insights sobre la salud basados en los resultados")
    optimization_strategies: List[Dict[str, Any]] = Field(..., description="Estrategias de optimización recomendadas")
    supplement_recommendations: Optional[List[Dict[str, Any]]] = Field(None, description="Suplementos recomendados basados en los resultados")
    lifestyle_recommendations: List[str] = Field(..., description="Recomendaciones de estilo de vida")
...
class AnalyzeWearableDataSkill(GoogleADKSkill):
    ...
    async def handler(self, input_data: AnalyzeWearableDataInput) -> AnalyzeWearableDataOutput:
        ...
        try:
            ...
            metrics_of_interest = input_data.metrics_of_interest or ["HRV", "sueño", "recuperación", "actividad"]
            ...
            with self.agent.tracer.start_as_current_span("wearable_data_analysis"):
                ...
                multimodal_result = await self.agent.multimodal_adapter.process_multimodal(
                    prompt=prompt,
                ...
                metrics_response = await gemini_client.generate_structured_output(metrics_prompt)
                ...
                if isinstance(metrics_response, list):
                    for metric in metrics_response:
                        if isinstance(metric, dict) and "name" in metric:
                            metrics.append(WearableMetric(
                                name=metric.get("name", "No especificado"),
                                value=metric.get("value", "No disponible"),
                                unit=metric.get("unit"),
                                interpretation=metric.get("interpretation", "No disponible"),
                ...
                insights_response = await gemini_client.generate_structured_output(insights_prompt)
                
                # Procesar insights
                insights = []
                if isinstance(insights_response, list):
                    insights = [insight for insight in insights_response if isinstance(insight, str)]
                elif isinstance(insights_response, dict) and "insights" in insights_response:
                    insights = [insight for insight in insights_response["insights"] if isinstance(insight, str)]
                ...
                recommendations_response = await gemini_client.generate_structured_output(recommendations_prompt)
                ...
                biohacking_response = await gemini_client.generate_structured_output(biohacking_prompt)
                ...
                elif isinstance(biohacking_response, dict) and "opportunities" in biohacking_response:
                ...
                if device_type == "No especificado":
                    ...
                    elif "apple watch" in analysis_text or "apple health" in analysis_text:
                ...
                analysis_summary = multimodal_result.get("text", "").split("\n\n")[0] if multimodal_result.get("text") else "No se pudo generar un resumen del análisis."
            ...
            return AnalyzeWearableDataOutput(
                ...
                insights=["Se recomienda intentar nuevamente con una imagen de mejor calidad."],
                recommendations=[
                    {
...
class AnalyzeBiomarkerResultsSkill(GoogleADKSkill):
    ...
    async def handler(self, input_data: AnalyzeBiomarkerResultsInput) -> AnalyzeBiomarkerResultsOutput:
        ...
        try:
            ...
            with self.agent.tracer.start_as_current_span("biomarker_results_analysis"):
                ...
                biomarkers_response = await gemini_client.generate_structured_output(biomarkers_prompt)
                    ...
                    for marker in biomarkers_response:
                        if isinstance(marker, dict) and "name" in marker:
                            biomarkers.append(BiomarkerResult(
                                ...
                                significance=marker.get("significance", "No disponible"),
                                confidence=marker.get("confidence", 0.7)
                            ))
                elif isinstance(biomarkers_response, dict) and "biomarkers" in biomarkers_response:
                ...
                health_insights_response = await gemini_client.generate_structured_output(health_insights_prompt)
                
                # Procesar insights de salud
                health_insights = []
                if isinstance(health_insights_response, list):
                    health_insights = [insight for insight in health_insights_response if isinstance(insight, str)]
                elif isinstance(health_insights_response, dict) and "insights" in health_insights_response:
                    health_insights = [insight for insight in health_insights_response["insights"] if isinstance(insight, str)]
                ...
                optimization_strategies_response = await gemini_client.generate_structured_output(optimization_strategies_prompt)
                ...
                elif isinstance(optimization_strategies_response, dict) and "strategies" in optimization_strategies_response:
                    optimization_strategies = optimization_strategies_response["strategies"]
                ...
                supplement_recommendations_response = await gemini_client.generate_structured_output(supplement_recommendations_prompt)
                ...
                elif isinstance(supplement_recommendations_response, dict) and "supplements" in supplement_recommendations_response:
                    supplement_recommendations = supplement_recommendations_response["supplements"]
                ...
                lifestyle_recommendations_response = await gemini_client.generate_structured_output(lifestyle_recommendations_prompt)
                
                # Procesar recomendaciones de estilo de vida
                lifestyle_recommendations = []
                if isinstance(lifestyle_recommendations_response, list):
                    lifestyle_recommendations = [rec for rec in lifestyle_recommendations_response if isinstance(rec, str)]
                elif isinstance(lifestyle_recommendations_response, dict) and "recommendations" in lifestyle_recommendations_response:
                    lifestyle_recommendations = [rec for rec in lifestyle_recommendations_response["recommendations"] if isinstance(rec, str)]
                ...
                if test_type == "No especificado":
                    ...
                    analysis_text = multimodal_result.get("text", "").lower()
                    if "sangre" in analysis_text or "hemograma" in analysis_text or "bioquímica" in analysis_text:
                        test_type_detected = "Análisis de sangre"
                    elif "genética" in analysis_text or "adn" in analysis_text or "genoma" in analysis_text:
                        test_type_detected = "Prueba genética"
                    elif "microbioma" in analysis_text or "intestinal" in analysis_text or "bacterias" in analysis_text:
            ...
            logger.error(f"Error al analizar resultados de pruebas biológicas: {e}", exc_info=True)
            ...
            return AnalyzeBiomarkerResultsOutput(
                ...
                analysis_summary="No se pudo realizar el análisis debido a un error en el procesamiento.",
                health_insights=["Se recomienda consultar con un profesional de la salud para una interpretación adecuada de los resultados."],
                optimization_strategies=[
                    {
                ...
                supplement_recommendations=[],
                lifestyle_recommendations=["Mantener hábitos saludables generales mientras se obtiene una interpretación profesional de los resultados."]
            )
...
class BiohackingInnovator(ADKAgent):
        ...
        **kwargs
        ...
        try:
            ...
            self.vision_processor = type('DummyVisionProcessor', (), {
                'analyze_image': lambda self, image_data: asyncio.Future().set_result({"text": "Análisis de imagen simulado"}),
                'extract_text': lambda self, image_data: asyncio.Future().set_result({"text": "Texto extraído simulado"})
            })()
            self.multimodal_adapter = type('DummyMultimodalAdapter', (), {
                'process_multimodal': lambda self, prompt, image_data, temperature=0.2, max_output_tokens=1024:
                    asyncio.Future().set_result({"text": "Análisis multimodal simulado"})
            })()
    ...
    def _classify_query_by_keywords(self, query: str) -> str:
        ...
        biohacking_keywords = [
            ...
            "monitoreo", "dispositivos", "autoexperimentación"
        ]
        ...
        wearable_keywords = [
            "wearable", "dispositivo", "anillo", "reloj", "oura", "whoop", "apple watch",
            "garmin", "fitbit", "hrv", "sueño", "recuperación", "actividad", "pasos",
            "frecuencia cardíaca", "monitoreo", "métricas", "datos"
        ]
        
        # Palabras clave para análisis de biomarcadores
        biomarker_keywords = [
            "biomarcador", "análisis", "sangre", "prueba", "laboratorio", "resultados",
            "genética", "microbioma", "hormonal", "inflamación", "metabolismo",
            "colesterol", "glucosa", "insulina", "vitamina", "mineral", "enzima"
        ]
    ...
    async def _run_async_impl(self, input_text: str, user_id: Optional[str] = None, **kwargs) -> Dict[str, Any]:
        ...
        if query_type == "biohacking":
            ...
            wearable_skill = next((skill for skill in self.skills if skill.name == "analyze_wearable_data"), None)
            if wearable_skill:
                # Extraer información relevante del perfil del usuario si está disponible
                ...
                for insight in result.insights[:3]:  # Limitar a 3 insights para mantener la respuesta concisa
                    response += f"- {insight}\n"
                
                # Añadir recomendaciones principales
                response += "\nRecomendaciones principales:\n"
                for rec in result.recommendations[:2]:  # Limitar a 2 recomendaciones
                    response += f"- {rec.get('title', '')}: {rec.get('description', '')}\n"
                ...
                context["protocols"].append({
                    ...
                    "metrics": [{"name": m.name, "value": m.value, "interpretation": m.interpretation} for m in result.metrics],
            ...
            biomarker_skill = next((skill for skill in self.skills if skill.name == "analyze_biomarker_results"), None)
            if biomarker_skill:
                # Extraer información relevante del perfil del usuario si está disponible
                # Nota: En un caso real, aquí se procesaría la imagen proporcionada por el usuario
                ...
                for insight in result.health_insights[:3]:  # Limitar a 3 insights para mantener la respuesta concisa
                    response += f"- {insight}\n"
                
                # Añadir estrategias de optimización principales
                response += "\nEstrategias de optimización recomendadas:\n"
                for strategy in result.optimization_strategies[:2]:  # Limitar a 2 estrategias
                    response += f"- {strategy.get('name', '')}: {strategy.get('description', '')}\n"
                
                # Añadir recomendaciones de estilo de vida
                response += "\nRecomendaciones de estilo de vida:\n"
                for rec in result.lifestyle_recommendations[:3]:  # Limitar a 3 recomendaciones
                    response += f"- {rec}\n"
                ...
                context["protocols"].append({
                    ...
                    "biomarkers": [{"name": b.name, "value": b.value, "status": b.status} for b in result.biomarkers],
