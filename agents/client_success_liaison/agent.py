...
from agents.client_success_liaison.schemas import (
    CommunityBuildingInput,
    CommunityBuildingOutput,
    UserExperienceInput,
    UserExperienceOutput,
    CustomerSupportInput,
    CustomerSupportOutput,
    RetentionStrategyInput,
    RetentionStrategyOutput,
    CommunicationManagementInput,
    CommunicationManagementOutput,
    WebSearchInput,
    WebSearchOutput,
    GeneralRequestInput,
    GeneralRequestOutput,
    # Nuevos esquemas para capacidades de visión
    FeedbackImageAnalysisInput,
    FeedbackImageAnalysisOutput,
    CommunityContentAnalysisInput,
    CommunityContentAnalysisOutput,
    UserJourneyVisualizationInput,
    UserJourneyVisualizationOutput,
)
...
class CommunityBuildingInput(BaseModel):
    query: str = Field(
        ..., description="Consulta del usuario sobre construcción de comunidad"
    )
    community_data: Optional[Dict[str, Any]] = Field(
        None, description="Datos de la comunidad actual"
    )


class CommunityBuildingOutput(BaseModel):
    response: str = Field(
        ..., description="Respuesta detallada sobre construcción de comunidad"
    )
    community_plan: Dict[str, Any] = Field(
        ..., description="Plan de comunidad estructurado"
    )


class UserExperienceInput(BaseModel):
    query: str = Field(
        ..., description="Consulta del usuario sobre experiencia de usuario"
    )
    experience_data: Optional[Dict[str, Any]] = Field(
        None, description="Datos de experiencia del usuario"
    )


class UserExperienceOutput(BaseModel):
    response: str = Field(
        ..., description="Respuesta detallada sobre experiencia de usuario"
    )
    journey_map: Optional[Dict[str, Any]] = Field(
        None, description="Mapa de customer journey"
    )


class CustomerSupportInput(BaseModel):
    query: str = Field(..., description="Consulta del usuario sobre soporte al cliente")
    problem_details: Optional[Dict[str, Any]] = Field(
        None, description="Detalles del problema reportado"
    )


class CustomerSupportOutput(BaseModel):
    response: str = Field(
        ..., description="Respuesta detallada sobre soporte al cliente"
    )
    ticket: Optional[Dict[str, Any]] = Field(
        None, description="Ticket de soporte generado"
    )


class RetentionStrategyInput(BaseModel):
    query: str = Field(
        ..., description="Consulta del usuario sobre estrategias de retención"
    )
    retention_data: Optional[Dict[str, Any]] = Field(
        None, description="Datos de retención del usuario"
    )


class RetentionStrategyOutput(BaseModel):
    response: str = Field(
        ..., description="Respuesta detallada sobre estrategias de retención"
    )
    retention_plan: Optional[Dict[str, Any]] = Field(
        None, description="Plan de retención estructurado"
    )


class CommunicationManagementInput(BaseModel):
    query: str = Field(
        ..., description="Consulta del usuario sobre gestión de comunicación"
    )
    communication_details: Optional[Dict[str, Any]] = Field(
        None, description="Detalles de comunicación"
    )


class CommunicationManagementOutput(BaseModel):
    response: str = Field(
        ..., description="Respuesta detallada sobre gestión de comunicación"
    )
    communication_plan: Optional[Dict[str, Any]] = Field(
        None, description="Plan de comunicación estructurado"
    )
...
class WebSearchOutput(BaseModel):
    ...
    search_results: Optional[List[Dict[str, Any]]] = Field(
        None, description="Resultados estructurados de la búsqueda"
    )
...
class CommunityBuildingSkill(GoogleADKSkill):
    ...
    async def handler(
        self, input_data: CommunityBuildingInput
    ) -> CommunityBuildingOutput:
        ...
        community_data_str = (
            f"Más detalles del contexto:\n{json.dumps(community_data, indent=2)}"
            if community_data
            else "No se proporcionaron detalles adicionales."
        )
        ...
        return CommunityBuildingOutput(response=response_text, community_plan=plan_json)


class UserExperienceSkill(GoogleADKSkill):
    name = "user_experience"
    description = (
        "Analiza y optimiza la experiencia del usuario en diferentes puntos de contacto"
    )
    ...
    async def handler(self, input_data: UserExperienceInput) -> UserExperienceOutput:
        ...
        # Construir el prompt para el modelo
        experience_data_str = (
            f"Detalles sobre la experiencia del usuario:\n{json.dumps(experience_data, indent=2)}"
            if experience_data
            else "No se proporcionaron datos específicos de la experiencia."
        )
            ...
            try:
                ...
                journey_json = {
                    ...
                    "pain_points": {
                        ...
                        "Onboarding": [
                            "Tutorial demasiado largo",
                            "Muchos pasos de configuración",
                        ],
                        "Uso": ["Interfaz no intuitiva", "Tiempos de carga lentos"],
                        "Retención": [
                            "Falta de incentivos para continuar",
                    ...
                    "opportunities": {
                        "Descubrimiento": [
                            "Mejorar SEO",
                            "Optimizar presencia en redes",
                        ],
                        "Consideración": [
                            "Simplificar comparativas",
                            "Añadir testimonios",
                        ],
        ...
        return UserExperienceOutput(response=response_text, journey_map=journey_json)
...
class CustomerSupportSkill(GoogleADKSkill):
    ...
    async def handler(self, input_data: CustomerSupportInput) -> CustomerSupportOutput:
        ...
        # Construir el prompt para el modelo
        problem_details_str = (
            "\n".join([f"- {k}: {v}" for k, v in problem_details.items()])
            if problem_details
            else "No se proporcionaron detalles adicionales del problema."
        )
        ...
        return CustomerSupportOutput(response=response_text, ticket=ticket)
...
class RetentionStrategySkill(GoogleADKSkill):
    ...
    async def handler(
        self, input_data: RetentionStrategyInput
    ) -> RetentionStrategyOutput:
        ...
        # Construir el prompt para el modelo
        retention_info_str = (
            "\n".join([f"- {k}: {v}" for k, v in retention_data.items()])
            if retention_data
            else "No se proporcionaron datos específicos para la retención."
        )
        ...
        return RetentionStrategyOutput(response=response_text, retention_plan=plan_json)


class CommunicationManagementSkill(GoogleADKSkill):
    name = "communication_management"
    description = (
        "Planifica y optimiza estrategias de comunicación con usuarios y clientes"
    )
    input_schema = CommunicationManagementInput
    output_schema = CommunicationManagementOutput

    async def handler(
        self, input_data: CommunicationManagementInput
    ) -> CommunicationManagementOutput:
        ...
        communication_details_str = (
            f"Detalles de la comunicación:\n{json.dumps(communication_details, indent=2)}"
            if communication_details
            else "No se proporcionaron detalles específicos de la comunicación."
        )
        ...
        return CommunicationManagementOutput(
            response=response_text, communication_plan=plan_json
        )
...
class WebSearchSkill(GoogleADKSkill):
    ...
    async def handler(self, input_data: WebSearchInput) -> WebSearchOutput:
        ...
        if not actual_query or actual_query == query:
            actual_query = query
            logger.warning(
                "No se pudo extraer la consulta de búsqueda específica, usando el texto completo."
            )

        try:
            logger.info(
                f"Invocando herramienta 'search_web' con query: '{actual_query}'"
            )
            ...
            if search_results and search_results.get("results"):
                ...
                logger.warning(
                    f"La herramienta 'search_web' no devolvió resultados para '{actual_query}'"
                )
                response_content = (
                    f"No pude encontrar resultados en la web para '{actual_query}'."
                )

        except Exception as e:
            logger.error(
                f"Error invocando la herramienta 'search_web': {e}", exc_info=True
            )
...
class FeedbackImageAnalysisSkill(GoogleADKSkill):
    ...
    async def handler(
        self, input_data: FeedbackImageAnalysisInput
    ) -> FeedbackImageAnalysisOutput:
        ...
        try:
            ...
            return FeedbackImageAnalysisOutput(
                ...
                analysis_summary=structured_data.get(
                    "analysis_summary", "Análisis de imagen de feedback"
                ),
                feedback_category=structured_data.get("feedback_category", "general"),
                content_type=structured_data.get("content_type", "imagen"),
                key_elements=structured_data.get(
                    "key_elements",
                    [{"element": "general", "description": "Contenido general"}],
                ),
                recommendations=structured_data.get(
                    "recommendations", ["Revisar el feedback en detalle"]
                ),
    ...
    def _generate_mock_feedback_analysis(
        self, input_data: FeedbackImageAnalysisInput
    ) -> FeedbackImageAnalysisOutput:
...
class CommunityContentAnalysisSkill(GoogleADKSkill):
    ...
    async def handler(
        self, input_data: CommunityContentAnalysisInput
    ) -> CommunityContentAnalysisOutput:
        ...
        try:
            ...
            return CommunityContentAnalysisOutput(
                analysis_id=analysis_id,
                content_summary=structured_data.get(
                    "content_summary", "Análisis de contenido de comunidad"
                ),
                engagement_potential=structured_data.get(
                    "engagement_potential", "medio"
                ),
                target_audience=structured_data.get(
                    "target_audience", ["Comunidad general"]
                ),
                content_recommendations=structured_data.get(
                    "content_recommendations",
                    [{"aspect": "general", "recommendation": "Revisar el contenido"}],
                ),
                distribution_channels=structured_data.get(
                    "distribution_channels", ["Redes sociales"]
                ),
                response=analysis_text,
            )
            ...
            logger.error(
                f"Error en análisis de contenido de comunidad: {e}", exc_info=True
            )
            return self._generate_mock_content_analysis(input_data)

    def _generate_mock_content_analysis(
        self, input_data: CommunityContentAnalysisInput
    ) -> CommunityContentAnalysisOutput:
        """Genera un análisis simulado cuando las capacidades de visión no están disponibles"""
        analysis_id = str(uuid.uuid4())
        ...
        return CommunityContentAnalysisOutput(
            ...
            content_recommendations=[
                {
                    "aspect": "descripción",
                    "recommendation": "Proporcionar una descripción textual del contenido",
                }
            ],
...
class UserJourneyVisualizationSkill(GoogleADKSkill):
    ...
    description = (
        "Genera visualizaciones de journey de usuario para mejorar la experiencia"
    )
    input_schema = UserJourneyVisualizationInput
    output_schema = UserJourneyVisualizationOutput

    async def handler(
        self, input_data: UserJourneyVisualizationInput
    ) -> UserJourneyVisualizationOutput:
        ...
        try:
            ...
            visualization_description = await gemini_client.generate_response(
                prompt, temperature=0.4
            )
            ...
            return UserJourneyVisualizationOutput(
                ...
                journey_summary=structured_data.get(
                    "journey_summary", "Visualización de journey de usuario"
                ),
                key_touchpoints=structured_data.get(
                    "key_touchpoints",
                    [
                        {
                            "touchpoint": "general",
                            "description": "Punto de contacto general",
                        }
                    ],
                ),
                improvement_areas=structured_data.get(
                    "improvement_areas",
                    [{"area": "general", "issue": "Área de mejora general"}],
                ),
            ...
            logger.error(
                f"Error en visualización de journey de usuario: {e}", exc_info=True
            )
            ...
            return UserJourneyVisualizationOutput(
                ...
                key_touchpoints=[
                    {
                        "touchpoint": "error",
                        "description": "Error en la generación de la visualización",
                    }
                ],
                improvement_areas=[
                    {
                        "area": "proceso",
                        "issue": "Revisar los datos de entrada del journey",
                    }
                ],
...
class ClientSuccessLiaison(ADKAgent):
    ...
    def __init__(
        ...
        self.gemini_client = (
            gemini_client
            if gemini_client
            else GeminiClient(model_name="gemini-1.5-flash")
        )
        ...
        examples = [
            Example(
                input={
                    "message": "¿Cómo puedo construir una comunidad activa alrededor de mi app de fitness?"
                },
                output={
                    "response": "Para construir una comunidad activa alrededor de tu app de fitness, te recomiendo implementar estas estrategias..."
                },
            ),
            Example(
                input={
                    "message": "Estamos teniendo problemas con la retención de usuarios después del primer mes"
                },
                output={
                    "response": "Para mejorar la retención de usuarios después del primer mes, considera implementar estas estrategias..."
                },
            ),
            Example(
                input={
                    "message": "¿Qué métricas debo seguir para evaluar la salud de mi comunidad?"
                },
                output={
                    "response": "Las métricas clave para evaluar la salud de tu comunidad incluyen..."
                },
            ),
            Example(
                input={
                    "message": "¿Puedes analizar esta imagen de feedback que compartió un cliente?"
                },
                output={
                    "response": "Basado en el análisis de la imagen compartida por el cliente, puedo identificar..."
                },
            ),
        ]
        ...
        try:
            ...
            logger.info(
                "Capacidades de visión y multimodales inicializadas correctamente para Client Success Liaison"
            )
        except Exception as e:
            logger.warning(
            ...
            logger.warning(
                "Usando implementaciones simuladas para capacidades de visión y multimodales"
            )

        # Inicializar estado del agente
        self.update_state(
            "community_calendars", {}
        )  # Almacenar calendarios de comunidad generados
        self.update_state(
            "customer_journey_maps", {}
        )  # Almacenar mapas de customer journey
        self.update_state("support_requests", {})  # Almacenar solicitudes de soporte

        logger.info(
            f"ClientSuccessLiaison inicializado con {len(capabilities)} capacidades y {len(skills)} skills"
        )
    ...
    async def _get_context(
        ...
        try:
            ...
            if not context:
                logger.info(
                    f"No se encontró contexto en adaptador del StateManager para user_id={user_id}, session_id={session_id}. Creando nuevo contexto."
                )
    ...
    async def _run_async_impl(
        ...
        if user_id:
            ...
            if not user_profile:
                logger.info(
                    f"Perfil no encontrado en contexto para {user_id}. Consultando Supabase vía MCP."
                )
                try:
                    # Construir la consulta SQL
                    sql_query = f"SELECT * FROM user_profiles WHERE user_id = '{user_id}' LIMIT 1;"
                    # Invocar la herramienta MCP de Supabase
                    query_result = await self.mcp_toolkit.invoke(
                        "supabase/query", sql=sql_query
                    )
                    ...
                    if (
                        ...
                        logger.warning(
                            f"No se encontró perfil en Supabase para {user_id} o resultado inesperado: {query_result}"
                        )
                        user_profile = {}
                    ...
                    logger.error(
                        f"Error al consultar perfil de usuario {user_id} vía MCP: {e}",
                        exc_info=True,
                    )
                    user_profile = {}
                    context["user_profile"] = {}
        ...
        if query_type == "community_building":
            ...
            community_skill = next(
                (skill for skill in self.skills if skill.name == "community_building"),
                None,
            )
            if community_skill:
                input_data = CommunityBuildingInput(
                    query=input_text, community_data=context.get("community_data", {})
                )
            ...
            experience_skill = next(
                (skill for skill in self.skills if skill.name == "user_experience"),
                None,
            )
            if experience_skill:
                input_data = UserExperienceInput(
                    query=input_text, experience_data=context.get("experience_data", {})
                )
            ...
            support_skill = next(
                (skill for skill in self.skills if skill.name == "customer_support"),
                None,
            )
            if support_skill:
                input_data = CustomerSupportInput(
                    query=input_text, problem_details=context.get("problem_details", {})
                )
            ...
            retention_skill = next(
                (
                    skill
                    for skill in self.skills
                    if skill.name == "retention_strategies"
                ),
                None,
            )
            if retention_skill:
                input_data = RetentionStrategyInput(
                    query=input_text, retention_data=context.get("retention_data", {})
                )
            ...
            communication_skill = next(
                (
                    skill
                    for skill in self.skills
                    if skill.name == "communication_management"
                ),
                None,
            )
            ...
            web_search_skill = next(
                (skill for skill in self.skills if skill.name == "web_search"), None
            )
            if web_search_skill:
                input_data = WebSearchInput(query=input_text)
        ...
        logger.info(
            f"ClientSuccessLiaison completó la ejecución en {execution_time:.2f} segundos"
        )
