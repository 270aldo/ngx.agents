...
class FeedbackImageAnalysisInput(BaseModel):
    ...
    feedback_type: Optional[str] = Field(
        None, description="Tipo de feedback (sugerencia, problema, pregunta)"
    )
    user_profile: Optional[Dict[str, Any]] = Field(
        None, description="Perfil del usuario que envía el feedback"
    )
...
class FeedbackImageAnalysisOutput(BaseModel):
    ...
    feedback_category: str = Field(
        ..., description="Categoría del feedback identificada"
    )
    content_type: str = Field(
        ..., description="Tipo de contenido identificado en la imagen"
    )
    key_elements: List[Dict[str, Any]] = Field(
        ..., description="Elementos clave identificados en la imagen"
    )
...
class CommunityContentAnalysisInput(BaseModel):
    ...
    content_purpose: Optional[str] = Field(
        None, description="Propósito del contenido (educativo, motivacional, etc.)"
    )
...
class CommunityContentAnalysisOutput(BaseModel):
    ...
    engagement_potential: str = Field(
        ..., description="Potencial de engagement estimado (alto, medio, bajo)"
    )
    target_audience: List[str] = Field(
        ..., description="Audiencia objetivo identificada"
    )
    content_recommendations: List[Dict[str, Any]] = Field(
        ..., description="Recomendaciones para mejorar el contenido"
    )
    distribution_channels: List[str] = Field(
        ..., description="Canales de distribución recomendados"
    )
    response: str = Field(..., description="Respuesta detallada para el usuario")
...
class UserJourneyVisualizationInput(BaseModel):
    ...
    journey_data: Dict[str, Any] = Field(
        ..., description="Datos del journey del usuario"
    )
    visualization_type: str = Field(
        ..., description="Tipo de visualización (timeline, flowchart, etc.)"
    )
    focus_points: Optional[List[str]] = Field(
        None, description="Puntos de enfoque específicos"
    )
...
class UserJourneyVisualizationOutput(BaseModel):
    ...
    key_touchpoints: List[Dict[str, Any]] = Field(
        ..., description="Puntos de contacto clave identificados"
    )
