...
async def register_custom_recognition_domain(
    ...
    try:
        ...
        logger.error(f"Error en skill register_custom_recognition_domain: {e}", exc_info=True)
        return {
            "success": False,
            "error": str(e),
...
async def get_available_recognition_domains() -> Dict[str, Any]:
    ...
    try:
        ...
        logger.error(f"Error en skill get_available_recognition_domains: {e}", exc_info=True)
        return {
            "success": False,
            "error": str(e),
            "domains": [],
...
async def process_document_with_image(
    ...
    try:
        ...
        if extract_tables:
            tasks.append(extract_tables_from_document(image_data, language_code, agent_id))
        
        if extract_forms:
            tasks.append(extract_forms_from_document(image_data, language_code, agent_id))
        
        if recognize_objects:
            tasks.append(recognize_objects_in_image(image_data, domain, confidence_threshold, agent_id))
        ...
        for i, result in enumerate(results):
            ...
            if isinstance(result, Exception):
                analysis_type = ["tables", "forms", "objects"][i] if i < 3 else f"analysis_{i}"
        ...
        if "tables" in combined_result and combined_result["tables"].get("success", False):
            table_count = combined_result["tables"].get("count", 0)
            message_parts.append(f"{table_count} tablas")
        
        if "forms" in combined_result and combined_result["forms"].get("success", False):
            field_count = combined_result["forms"].get("count", 0)
            message_parts.append(f"{field_count} campos de formulario")
        
        if "objects" in combined_result and combined_result["objects"].get("success", False):
            object_count = combined_result["objects"].get("count", 0)
            message_parts.append(f"{object_count} objetos")
        
        if message_parts:
            combined_result["message"] = f"Análisis completado. Se encontraron: {', '.join(message_parts)}."
        else:
            combined_result["message"] = "Análisis completado, pero no se encontraron elementos."
        
        # Actualizar estado general
        combined_result["success"] = combined_result["analyses_failed"] < combined_result["analyses_requested"]
