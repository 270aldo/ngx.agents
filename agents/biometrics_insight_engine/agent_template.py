...
class RecoveryCorrective(ADKAgent):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            or "https://recovery-corrective-api.ngx-agents.com/a2a",
        ...
        self.update_state(
            "recovery_protocols", {}
        )  # Almacenar protocolos de recuperación generados
        self.update_state("pain_assessments", {})  # Almacenar evaluaciones de dolor

        logger.info(
            f"RecoveryCorrective inicializado con {len(capabilities)} capacidades y {len(skills)} skills"
        )
    ...
    async def _run_async_impl(
        ...
        **kwargs,
        ...
        if (
            hasattr(self, "has_telemetry")
            and self.has_telemetry
            and hasattr(self, "request_counter")
        ):
        ...
        if (
            hasattr(self, "has_telemetry")
            and self.has_telemetry
            and hasattr(self, "tracer")
        ):
            with self.tracer.start_as_current_span(
                "recovery_corrective_process_request"
            ) as span:
        ...
        **kwargs,
        ...
        try:
            ...
            if any(
                ...
                for keyword in ["prevenir", "prevención", "evitar lesiones", "proteger"]
            ):
                ...
                keyword in input_text.lower()
                for keyword in ["rehabilitación", "recuperación", "rehab", "recuperar"]
            ):
                ...
                keyword in input_text.lower()
                for keyword in ["dolor", "molestia", "lesión", "lastimado"]
            ):
                # Generar evaluación de dolor
                try:
                    pain_assessment = await self._generate_pain_assessment(
                    ...
                    # Generar respuesta
                    response_text = await self._summarize_pain_assessment(
                        pain_assessment
                    )
                ...
                try:
                    recovery_protocol = await self._generate_recovery_protocol(
                        input_text, user_profile
                    )
                    ...
                    # Generar respuesta
                    response_text = await self._summarize_recovery_protocol(
                        recovery_protocol
                    )
            ...
            response = {"text": response_text, "data": result}
    ...
    def _summarize_pain_assessment(self, pain_assessment: Dict[str, Any]) -> str:
        ...
        if "location" in pain_assessment:
            summary_parts.append(
                f"El dolor se localiza en {pain_assessment['location']}."
            )

        if "intensity" in pain_assessment:
            summary_parts.append(
                f"La intensidad es de {pain_assessment['intensity']} en una escala de 1-10."
            )
        ...
        if "seek_medical_attention" in pain_assessment:
            summary_parts.append(
                f"Nota importante: {pain_assessment['seek_medical_attention']}."
            )
    ...
    def _summarize_recovery_protocol(self, recovery_protocol: Dict[str, Any]) -> str:
        ...
        if "objective" in recovery_protocol:
            summary_parts.append(
                f"El objetivo principal es: {recovery_protocol['objective']}."
            )
        ...
        if "phases" in recovery_protocol and recovery_protocol["phases"]:
            ...
            if isinstance(phases, list) and len(phases) > 0:
                phase = phases[0]
                summary_parts.append(
                    f"Comenzaremos con la fase '{phase.get('name', 'inicial')}' que dura {phase.get('duration', 'un periodo')} y se enfoca en {phase.get('focus', 'recuperación')}."
                )
        ...
        try:
            ...
            if any(
                ...
                artifact = self.create_artifact(
                    ...
                    parts=[self.create_data_part(recovery_protocol)],
                )
            ...
            if any(
                keyword in user_input.lower()
                for keyword in ["dolor", "lesión", "molestia", "inflamación"]
            ):
                pain_assessment = await self._generate_pain_assessment(
                    user_input, user_profile
                )
                ...
                artifact = self.create_artifact(
                    ...
                    parts=[self.create_data_part(pain_assessment)],
                )
                artifacts.append(artifact)
    ...
    async def _generate_recovery_protocol(
    ...
    async def _generate_pain_assessment(
        self, user_input: str, user_profile: Optional[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
        ...
        if not isinstance(response, dict):
            try:
                ...
                response = {
                    ...
                    "aggravating_factors": [
                        "Factores que aumentan el dolor no especificados"
                    ],
                    "relieving_factors": [
                        "Factores que disminuyen el dolor no especificados"
                    ],
