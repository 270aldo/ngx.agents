...
class NGXNexusOrchestrator(ADKAgent):
    ...
    def __init__(
        ...
        instruction: Optional[
            str
        ] = "Eres el orquestador principal de NGX Nexus. Tu función es analizar la solicitud del usuario, enrutarla a agentes especializados y sintetizar sus respuestas.",
        ...
        **kwargs: Any,
        ...
        _a2a_server_url = (
            a2a_server_url or f"http://{settings.A2A_HOST}:{settings.A2A_PORT}"
        )
        ...
        try:
            ...
            logger.info(
                "AI Platform (Vertex AI SDK) inicializado correctamente para el Orquestador."
            )
        except ImportError:
            logger.warning(
                "Google Cloud AI Platform no está disponible. Algunas funcionalidades podrían estar limitadas."
            )
        except Exception as e:
            logger.error(
                f"Error al inicializar AI Platform para el Orquestador: {e}",
                exc_info=True,
            )
        ...
        try:
            ...
            logger.info(
                "Procesador de visión inicializado correctamente para el Orquestador"
            )

            # Inicializar adaptador multimodal
            self.multimodal_adapter = MultimodalAdapter()
            logger.info(
                "Adaptador multimodal inicializado correctamente para el Orquestador"
            )
            ...
            logger.warning(
                f"No se pudieron inicializar componentes para capacidades de visión: {e}"
            )
            self._vision_capabilities_available = False
            logger.warning("El Orquestador funcionará sin capacidades de visión")
        except Exception as e:
            logger.error(
                f"Error al inicializar capacidades de visión para el Orquestador: {e}",
                exc_info=True,
            )
            self._vision_capabilities_available = False
            logger.warning("El Orquestador funcionará sin capacidades de visión")
    ...
    async def _skill_analyze_visual_content(
        self, prompt: str, image_data: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if (
                ...
                logger.warning(
                    "Capacidades de visión no disponibles para el Orquestador"
                )
                return {
                    "status": "error",
            ...
            intent_analysis = await intent_analyzer_adapter.analyze_intent(
                visual_intent_prompt
            )
            ...
            secondary_intents = [
                intent.lower()
                for intent in intent_analysis.get("secondary_intents", [])
            ]
    ...
    async def _skill_process_multimodal_input(
        self, prompt: str, image_data: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if (
                ...
                logger.warning(
                    "Capacidades multimodales no disponibles para el Orquestador"
                )
                return {
                    "status": "error",
    ...
    async def _skill_synthesize_response(
        self, prompt: str, agent_responses: Optional[Dict[str, Dict[str, Any]]] = None
    ) -> str:
        """
        ...
        **kwargs,
        ...
        try:
            result = await self._process_request(
                input_text, user_id, session_id, start_time=start_time, **kwargs
            )
            if "metadata" not in result:
                result["metadata"] = {}
            result["metadata"]["processing_time"] = result["metadata"].get(
    ...
    async def _process_request(
        ...
        **kwargs,
        ...
        logger.info(
            f"Orquestador procesando entrada: '{input_text[:50]}...' (user_id={user_id}, session_id={session_id})"
        )
        ...
        try:
            ...
            intent_analysis_result = await self.adk_toolkit.execute_skill(
                "analyze_intent", prompt=input_text
            )
            ...
            try:
                ...
                logger.warning(
                    f"No se pudo decodificar JSON del análisis de intención: {intent_analysis_result}. Usando fallback."
                )
                intent_data = {"primary_intent": "general", "confidence": 0.5}

            primary_intent = intent_data.get("primary_intent", "general").lower()
            secondary_intents = [
                intent.lower() for intent in intent_data.get("secondary_intents", [])
            ]
            ...
            if not agent_ids_to_call:
                ...
                await self._update_context(
                    user_id, session_id, input_text, no_agent_response
                )
                return {
            ...
            logger.info(
                f"Intención: {primary_intent}, confianza: {confidence}, agentes: {agent_ids_to_call}"
            )
        except Exception as e:
        ...
        await self._update_context(
            user_id, session_id, input_text, synthesized_response
        )
        ...
        if user_id:
            try:
                ...
                logger.warning(
                    f"Error al actualizar estadísticas en el adaptador del StateManager: {e}",
                    exc_info=True,
                )
    ...
    async def _get_context(
        ...
        try:
            ...
            logger.error(
                f"Error al obtener contexto del adaptador del StateManager: {e}",
                exc_info=True,
            )
            return {"history": []}
    ...
    async def _update_context(
        ...
        try:
            ...
            logger.error(
                f"Error al actualizar contexto en el adaptador del StateManager: {e}",
                exc_info=True,
            )
    ...
    async def _get_agent_responses(
        ...
        try:
            ...
            responses = await a2a_adapter.call_multiple_agents(
                user_input=user_input, agent_ids=agent_ids, context=task_context_data
            )
