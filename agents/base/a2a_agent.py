...
from core.skill import Skill, skill_registry, SkillResult, SkillStatus
from core.agent_card import AgentCard, Example, InputSchema, OutputSchema
from agents.base.base_agent import BaseAgent

logger = logging.getLogger(__name__)

AGENT_PING_INTERVAL = 25  # Segundos, ligeramente menor que HEARTBEAT_INTERVAL del servidor
...
class A2AAgent(BaseAgent):
    ...
    def __init__(
        ...
        **kwargs  # Aceptar parámetros adicionales como toolkit, etc.
        ...
        self.pending_messages: List[Dict[str, Any]] = [] # Cola para mensajes si se envían antes de conectar
    ...
    def register_skill(self, skill: Skill) -> bool:
        ...
        try:
            ...
            if skill_name in self.registered_skills:
                logger.warning(f"Skill {skill_name} ya está registrada en el agente {self.agent_id}. Sobrescribiendo.")
            ...
            logger.error(f"Error al registrar skill {getattr(skill, 'name', 'desconocida')}: {str(e)}")
            return False
    ...
    def unregister_skill(self, skill_name: str) -> bool:
        ...
        if skill_name in self.registered_skills:
            ...
            logger.warning(f"Skill {skill_name} no está registrada en el agente {self.agent_id}")
            return False
    ...
    async def register(self, max_retries: int = 3, retry_delay: float = 2.0) -> bool:
        ...
        while retries <= max_retries:
            try:
                logger.info(f"Intento {retries+1}/{max_retries+1} de registro del agente {self.agent_id}")
                
                # Registrar el agente mediante HTTP
                async with httpx.AsyncClient(base_url=A2A_SERVER_URL, timeout=10.0) as client:
                ...
                if data.get("status") == "success":
                    ...
                    logger.warning(f"Fallo al registrar agente {self.agent_id}: {error_msg}")
                    
                    # Si el error es 409 (conflicto), el agente ya está registrado
                    if response.status_code == 409:
                        logger.info(f"El agente {self.agent_id} ya estaba registrado, intentando conectar...")
                        self.is_registered = True
                        await self.connect()
                        return True
            except httpx.HTTPStatusError as e:
                logger.error(f"Error HTTP al registrar el agente {self.agent_id}: {e.response.status_code} - {e.response.text}")
                # Si el error es 409 (conflicto), el agente ya está registrado
        ...
        logger.error(f"Agotados todos los intentos de registro para el agente {self.agent_id}")
        return False
    
    async def connect(self, max_retries: int = 5, retry_delay: float = 1.5):
        """
        ...
        if not self.is_registered:
            logger.warning(f"Agente {self.agent_id} intentando conectar sin estar registrado.")
        ...
        while retries <= max_retries:
            try:
                logger.info(f"Intento {retries+1}/{max_retries+1} de conexión del agente {self.agent_id} a {websocket_url}")
                self.websocket = await websockets.connect(
                    websocket_url,
                    ping_interval=AGENT_PING_INTERVAL,  # Configurar ping automático de websockets
                    ping_timeout=AGENT_PING_INTERVAL * 2,  # Timeout para considerar la conexión perdida
                    close_timeout=5.0  # Timeout para cerrar la conexión
                )
                self.is_connected = True
                logger.info(f"Agente {self.agent_id} conectado al servidor A2A via WebSocket")

                # Enviar mensajes pendientes si los hay
                if self.pending_messages:
                    logger.info(f"Enviando {len(self.pending_messages)} mensajes pendientes")
                ...
                self._send_pings_task = asyncio.create_task(self._send_pings())  # Tarea para enviar pings
                ...
                if "401" in str(e) or "403" in str(e):
                    logger.critical(f"Error de autenticación. Agente {self.agent_id} no autorizado.")
                ...
                logger.error(f"Error al conectar el agente {self.agent_id} a {websocket_url}: {str(e)}")
        ...
        logger.error(f"Agotados todos los intentos de conexión para el agente {self.agent_id}")
        return False
    ...
    async def disconnect(self):
        ...
        if not self.is_connected or not self.websocket:
            logger.debug(f"Agente {self.agent_id} ya está desconectado o sin websocket.")
            return
    ...
    async def _message_loop(self):
        ...
        """
        if not self.is_connected or not self.websocket:
            logger.error(f"Intento de iniciar _message_loop sin conexión WebSocket para {self.agent_id}")
            return
        ...
        reconnect_enabled = True  # Flag para habilitar/deshabilitar reconexión automática
        ...
        try:
            async for message in self.websocket:
                try:
                    ...
                    if isinstance(data, dict) and data.get("type") == "ping":
                        logger.debug(f"Ping recibido de servidor, enviando pong para {self.agent_id}")
                    ...
                    logger.error(f"Mensaje inválido (no JSON) recibido para {self.agent_id}: {message[:100]}")
                except Exception as e:
            ...
            logger.warning(f"Conexión WebSocket cerrada con error para {self.agent_id}: {e}")
            ...
            logger.info(f"Saliendo de _message_loop para {self.agent_id}. Limpiando conexión.")
            ...
            if reconnect_enabled and reconnect_attempts < max_reconnect_attempts:
                ...
                actual_delay = min(reconnect_delay * (1.5 ** (reconnect_attempts - 1)), max_reconnect_delay)
                ...
                logger.info(f"Intentando reconectar en {actual_delay:.2f}s (intento {reconnect_attempts}/{max_reconnect_attempts})")
                ...
                try:
                    ...
                    if self.is_connected:
                        logger.info(f"Reconexión exitosa después de {reconnect_attempts} intentos")
                        ...
                        if reconnect_attempts < max_reconnect_attempts:
                            asyncio.create_task(self._delayed_reconnect(reconnect_attempts, max_reconnect_attempts, reconnect_delay, max_reconnect_delay))
                except Exception as e:
                ...
                logger.error(f"Máximo número de reconexiones alcanzado ({max_reconnect_attempts}). Abandonando.")
                
    async def _delayed_reconnect(self, current_attempts, max_attempts, base_delay, max_delay):
        """
        Método auxiliar para intentar reconectar después de un tiempo de espera.
        """
        current_attempts += 1
        if current_attempts > max_attempts:
            logger.error(f"Máximo número de reconexiones alcanzado ({max_attempts}). Abandonando.")
            return
        ...
        logger.info(f"Reintentando reconexión en {actual_delay:.2f}s (intento {current_attempts}/{max_attempts})")
        await asyncio.sleep(actual_delay)
        ...
        try:
            ...
            if self.is_connected:
                logger.info(f"Reconexión exitosa después de {current_attempts} intentos")
            else:
                # Si falla, programar otro intento
                await self._delayed_reconnect(current_attempts, max_attempts, base_delay, max_delay)
        except Exception as e:
            logger.error(f"Error al intentar reconectar: {str(e)}")
    ...
    async def _send_pings(self):
        ...
        while self.is_connected and self.websocket:
            try:
                ...
                logger.warning(f"No se pudo enviar ping, conexión cerrada para {self.agent_id}")
    ...
    async def _process_message(self, data: Dict[str, Any]):
        ...
        if message_type == "task":
            ...
            try:
                ...
                logger.info(f"Tarea '{task_id}' procesada por {self.agent_id} con estado '{status}'")
            except Exception as e:
                logger.error(f"Error al manejar tarea '{task_id}' en agente {self.agent_id}: {e}")
                result = {"error": str(e)}
                status = "failed"
            ...
            logger.debug(f"Enviada actualización para tarea '{task_id}' desde {self.agent_id}")

        elif message_type == "message":
            message_id = data.get("id")
            from_agent = data.get("from")
            content = data.get("content", {})
            logger.info(f"Mensaje '{message_id}' de {from_agent} recibido por agente {self.agent_id}")
            ...
            if self.message_handler:
                 ...
                 logger.warning(f"Agente {self.agent_id} no tiene message_handler configurado.")

        elif message_type == "ping":
            # Ya manejado en _message_loop, pero podría añadirse lógica aquí si fuera necesario
            pass
        else:
            logger.warning(f"Tipo de mensaje desconocido '{message_type}' recibido por {self.agent_id}")
    ...
    async def request_task(self, agent_id: str, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            async with httpx.AsyncClient(base_url=A2A_SERVER_URL, timeout=30.0) as client:
        ...
        try:
            ...
            if skill_name and skill_name in self.registered_skills:
                ...
                try:
                    ...
                    self.skill_tasks[task_id]["execution_time"] = time.time() - task_start_time
                ...
                if isinstance(result, dict) and "task_id" in result and "status" in result:
            ...
            return {
                ...
                "error": str(e),
                "execution_time": time.time() - task_start_time if 'task_start_time' in locals() else 0.0,
                "confidence": 0.0,
                "agent_id": self.agent_id,
                "metadata": {}
            }
    ...
    def create_file_part(self, mime_type: str, data: Optional[str] = None, uri: Optional[str] = None) -> Dict[str, Any]:
        """
    ...
    def create_artifact(self, artifact_id: str, artifact_type: str, parts: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
    ...
    def create_task(self, task_id: str, messages: List[Dict[str, Any]], artifacts: Optional[List[Dict[str, Any]]] = None) -> Dict[str, Any]:
        """
    ...
    async def _run_async_impl(self, input_text: str, user_id: Optional[str] = None, 
        ...
        if isinstance(response, dict):
            ...
            if "metadata" not in response:
                response["metadata"] = {
                    "capabilities_used": self.capabilities[:1] if self.capabilities else [],
                    "user_id": user_id
                }
            return response
        else:
        ...
        examples = [
            {
                ...
                "output": {"response": "Hola, soy un agente especializado. ¿En qué puedo ayudarte?"}
            }
        ]
