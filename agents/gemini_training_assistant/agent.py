...
class GenerateTrainingPlanInput(BaseModel):
    query: str = Field(
        ..., description="Consulta del usuario sobre plan de entrenamiento"
    )
...
class GenerateTrainingPlanOutput(BaseModel):
    response: str = Field(
        ..., description="Respuesta detallada con plan de entrenamiento"
    )
    training_plan: Optional[Dict[str, Any]] = Field(
        None, description="Plan de entrenamiento estructurado"
    )
...
class RecommendNutritionOutput(BaseModel):
    response: str = Field(
        ..., description="Respuesta detallada con recomendaciones nutricionales"
    )
    nutrition_plan: Optional[Dict[str, Any]] = Field(
        None, description="Plan nutricional estructurado"
    )
...
class AnswerFitnessQuestionInput(BaseModel):
    ...
    session_id: Optional[str] = Field(
        None, description="ID de sesión para mantener contexto"
    )


class AnswerFitnessQuestionOutput(BaseModel):
    response: str = Field(
        ..., description="Respuesta detallada a la pregunta de fitness"
    )
...
class AnalyzeProgressInput(BaseModel):
    query: str = Field(
        ..., description="Consulta del usuario sobre análisis de progreso"
    )
    context: Optional[Dict[str, Any]] = Field(
        None, description="Contexto adicional para la consulta"
    )
...
class AnalyzeProgressOutput(BaseModel):
    response: str = Field(
        ..., description="Respuesta detallada con análisis de progreso"
    )
    progress_analysis: Optional[Dict[str, Any]] = Field(
        None, description="Análisis de progreso estructurado"
    )
...
"""
        ...
        return GenerateTrainingPlanOutput(
            response=response_text, training_plan=plan_json
        )
...
class RecommendNutritionSkill(GoogleADKSkill):
    ...
    async def handler(
        self, input_data: RecommendNutritionInput
    ) -> RecommendNutritionOutput:
...
"""
        ...
        return RecommendNutritionOutput(
            response=response_text, nutrition_plan=plan_json
        )
...
class AnswerFitnessQuestionSkill(GoogleADKSkill):
    ...
    async def handler(
        self, input_data: AnswerFitnessQuestionInput
    ) -> AnswerFitnessQuestionOutput:
...
"""
        ...
        return AnalyzeProgressOutput(
            response=response_text, progress_analysis=analysis_json
        )
...
class GeminiTrainingAssistant(ADKAgent):
    ...
    def __init__(
        ...
        examples = [
            Example(
                ...
                output={
                    "response": "Aquí tienes un plan de entrenamiento de 16 semanas para prepararte para un maratón..."
                },
            ),
            Example(
                input={"message": "¿Qué debo comer antes de entrenar?"},
                output={
                    "response": "Antes de entrenar, es recomendable consumir carbohidratos complejos y proteínas magras..."
                },
            ),
            Example(
                input={"message": "¿Cómo puedo superar mi meseta en press de banca?"},
                output={
                    "response": "Para superar una meseta en press de banca, puedes implementar estas estrategias..."
                },
            ),
        ]
        ...
        self.update_state(
            "training_plans", {}
        )  # Almacenar planes de entrenamiento generados
        self.update_state(
            "nutrition_recommendations", {}
        )  # Almacenar recomendaciones nutricionales
        self.update_state("progress_analyses", {})  # Almacenar análisis de progreso

        logger.info(
            f"GeminiTrainingAssistant inicializado con {len(capabilities)} capacidades y {len(skills)} skills"
        )
    ...
    async def _get_context(
        ...
        try:
            ...
            if not context or not context.get("state_data"):
                logger.info(
                    f"No se encontró contexto en StateManager para user_id={user_id}, session_id={session_id}. Creando nuevo contexto."
                )
                ...
                logger.info(
                    f"Contexto cargado desde StateManager para user_id={user_id}, session_id={session_id}"
                )
    ...
    async def _update_context(
        ...
        try:
            ...
            logger.info(
                f"Contexto actualizado en StateManager para user_id={user_id}, session_id={session_id}"
            )
        except Exception as e:
    ...
    def _classify_query(self, query: str) -> str:
        ...
        if any(
            word in query_lower
            for word in ["plan", "entrenamiento", "rutina", "ejercicios", "programa"]
        ):
            ...
            word in query_lower
            for word in ["progreso", "avance", "resultados", "meseta", "estancamiento"]
        ):
    ...
    async def _run_async_impl(
        ...
        logger.info(
            f"Ejecutando GeminiTrainingAssistant con input: {input_text[:50]}..."
        )
        ...
        if query_type == "generate_training_plan":
            ...
            training_skill = next(
                (
                    ...
                    if skill.name == "generate_training_plan"
                ),
                None,
            )
            if training_skill:
            ...
            nutrition_skill = next(
                (skill for skill in self.skills if skill.name == "recommend_nutrition"),
                None,
            )
            if nutrition_skill:
                input_data = RecommendNutritionInput(query=input_text, context=context)
            ...
            progress_skill = next(
                (skill for skill in self.skills if skill.name == "analyze_progress"),
                None,
            )
            if progress_skill:
                input_data = AnalyzeProgressInput(query=input_text, context=context)
            ...
            question_skill = next(
                (
                    skill
                    for skill in self.skills
                    if skill.name == "answer_fitness_question"
                ),
                None,
            )
            if question_skill:
                input_data = AnswerFitnessQuestionInput(
                    query=input_text, session_id=session_id
                )
        ...
        logger.info(
            f"GeminiTrainingAssistant completó la ejecución en {execution_time:.2f} segundos"
        )
