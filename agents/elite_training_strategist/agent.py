...
class EliteTrainingStrategist(ADKAgent):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Inicializar skills de audio
        self.workout_voice_guide_skill = WorkoutVoiceGuideSkill()
        self.voice_command_skill = VoiceCommandSkill()
        self.audio_feedback_skill = AudioFeedbackSkill()
        ...
        self.skills = [
            ...
            Skill(
                name="workout_voice_guide",
                description="Proporciona guía de voz durante los entrenamientos con conteo de repeticiones y recordatorios de forma.",
                handler=self._skill_workout_voice_guide,
            ),
            Skill(
                name="process_workout_command",
                description="Procesa comandos de voz durante entrenamientos para control manos libres.",
                handler=self._skill_process_workout_command,
            ),
            Skill(
                name="exercise_audio_feedback",
                description="Genera feedback de audio personalizado para corrección de forma y motivación durante ejercicios.",
                handler=self._skill_exercise_audio_feedback,
            ),
        ]
        ...
        try:
            ...
            logger.info(
                "AI Platform (Vertex AI SDK) inicializado correctamente para ETS."
            )
        except Exception as e:
            logger.error(
                f"Error al inicializar AI Platform para ETS: {e}", exc_info=True
            )
        ...
        try:
            ...
            self.vision_processor = type(
                ...
                (),
                {
                    "analyze_image": lambda self, image_data, prompt=None: {
                        "text": "Análisis de imagen simulado"
                    },
                    "describe_image": lambda self, image_data, detail_level=None, focus_aspect=None: {
                        "text": "Descripción de imagen simulada"
                    },
                },
            )()
            ...
            self.multimodal_adapter = type(
                ...
                (),
                {
                    ...
                    "compare_images": lambda self, image_data1, image_data2, comparison_prompt=None, temperature=0.2, max_output_tokens=1024: {
                        "text": "Comparación de imágenes simulada",
                        "comparison": "",
                        "similarities": [],
                        "differences": [],
                    },
    ...
    async def _skill_generate_training_plan(
        ...
        logger.info(
            f"Ejecutando habilidad: _skill_generate_training_plan con input: {input_data}"
        )
    ...
    async def _skill_adapt_training_program(
        ...
        logger.info(
            f"Ejecutando habilidad: _skill_adapt_training_program con input: {input_data}"
        )
    ...
    async def _skill_analyze_performance_data(
        ...
        logger.info(
            f"Ejecutando habilidad: _skill_analyze_performance_data con input: {input_data}"
        )
        # Lógica de análisis...
        return AnalyzePerformanceDataOutput(
            analysis_summary="Análisis de rendimiento simulado.",
            key_observations=[
                {"metric": "metric1", "value": 100, "observation": "Bueno"}
            ],
            recommendations=[
                {
                    "recommendation_type": "Entrenamiento",
                    "description": "Continuar con el buen trabajo",
                }
            ],
        )
    ...
    async def _skill_set_training_intensity_volume(
        ...
        logger.info(
            f"Ejecutando habilidad: _skill_set_training_intensity_volume con input: {input_data}"
        )
    ...
    async def _skill_prescribe_exercise_routines(
        ...
        logger.info(
            f"Ejecutando habilidad: _skill_prescribe_exercise_routines con input: {input_data}"
        )
    ...
    async def _consult_other_agent(
        ...
        if not a2a_adapter:
            logger.error(
                "Adaptador A2A no disponible. No se puede consultar a otros agentes."
            )
        ...
        try:
            ...
            if context and "program_type" in context:
                ...
                try:
                    ...
                    logger.warning(
                        f"No se pudo enriquecer la consulta con información del programa: {e}"
                    )
    ...
    def _prepare_context(
        ...
        if context:
            ...
            relevant_context_keys = [
                ...
                if k not in ["user_id", "session_id", "client_profile", "history"]
            ]
        ...
        logger.debug(
            f"Contexto preparado para EliteTrainingStrategist: {prompt_context}"
        )
    ...
    async def _generate_training_plan(
        ...
        if context.get("age"):
            prompt_parts.append(f"- Edad: {context.get('age')}")
        if context.get("gender"):
            prompt_parts.append(f"- Género: {context.get('gender')}")
        if context.get("weight_kg"):
            prompt_parts.append(f"- Peso: {context.get('weight_kg')} kg")
        if context.get("height_cm"):
            prompt_parts.append(f"- Altura: {context.get('height_cm')} cm")
        if context.get("activity_level"):
            prompt_parts.append(
                f"- Nivel de Actividad: {context.get('activity_level')}"
            )
        if context.get("experience_level"):
            prompt_parts.append(
                f"- Nivel de Experiencia: {context.get('experience_level')}"
            )
        ...
        prompt_parts.append(
            "Incluye fases, microciclos, mesociclos si aplica, ejercicios específicos, series, repeticiones, descansos, y notas sobre intensidad y volumen."
        )
        prompt_parts.append(
            "Formato de Salida Esperado: JSON con claves como 'plan_name', 'program_type', 'duration_weeks', 'phases' (lista), etc."
        )
        prompt_parts.append(
            "Cada fase debe tener 'phase_name', 'duration', 'description', y 'weekly_schedule'."
        )
        prompt_parts.append(
            "Cada 'weekly_schedule' debe ser una lista de días, cada día con 'day_name', 'focus', y 'workouts' (lista de workouts)."
        )
        prompt_parts.append(
            "Cada 'workout' debe tener 'exercise_name', 'sets', 'reps', 'rest_seconds', 'intensity_notes'."
        )
        ...
        # Llamada a Gemini
        if not self.gemini_client or not hasattr(
            self.gemini_client, "generate_structured_output"
        ):
            logger.error(
                "GeminiClient no está configurado correctamente. No se puede generar el plan de entrenamiento."
            )
            return {
                "error": "GeminiClient not configured",
                "message": "Unable to generate training plan.",
            }

        generated_plan = await self.gemini_client.generate_structured_output(
            final_prompt
        )

        # Validar la estructura del plan generado
        if not isinstance(generated_plan, dict) or not all(
            k in generated_plan
            for k in ["plan_name", "program_type", "duration_weeks", "phases"]
        ):
            logger.error(
                f"La salida de Gemini para el plan de entrenamiento no tiene la estructura esperada. Salida: {generated_plan}"
            )
            return {
                "error": "Invalid plan structure from LLM",
                "details": generated_plan,
            }

        # Crear un artefacto para el plan generado
        plan_text_content = generated_plan.get(
            "response",
            f"Plan de entrenamiento {generated_plan['program_type']} de {generated_plan['duration_weeks']} semanas",
        )
    ...
    async def _adapt_training_program(
        ...
        if feedback:
            prompt_parts.append(
                f"Feedback del Atleta sobre el Plan Actual: {json.dumps(feedback)}"
            )
        if new_goals:
            prompt_parts.append(
                f"Nuevos Objetivos o Ajustes a Objetivos: {', '.join(new_goals)}"
            )
        ...
        prompt_parts.append(
            "Modifica fases, ejercicios, volumen, intensidad según sea necesario."
        )
        prompt_parts.append(
            "Formato de Salida Esperado: JSON similar a GenerateTrainingPlanOutput, pero con una sección 'adaptation_summary'."
        )
        prompt_parts.append(
        ...
        if not self.gemini_client or not hasattr(
            ...
            logger.error(
                "GeminiClient no está configurado correctamente. No se puede adaptar el plan."
            )
            return {
                "error": "GeminiClient not configured",
                "message": "Unable to adapt training plan.",
            }
        ...
        if (
            not isinstance(adapted_plan, dict)
            or "adapted_plan_name" not in adapted_plan
        ):
            logger.error(
                f"La salida de Gemini para la adaptación del plan no tiene la estructura esperada. Salida: {adapted_plan}"
            )
            return {
                "error": "Invalid adapted plan structure from LLM",
                "details": adapted_plan,
            }
    ...
    async def _analyze_performance_data(
        ...
        if user_query:
            prompt_parts.append(
                f"Pregunta Específica del Usuario sobre los Datos: {user_query}"
            )
        if metrics_to_focus:
            prompt_parts.append(
                f"Métricas Clave para Enfocar el Análisis: {', '.join(metrics_to_focus)}"
            )
        if comparison_period:
            prompt_parts.append(
                f"Período de Comparación (si aplica): {json.dumps(comparison_period)}"
            )
        ...
        prompt_parts.append(
            "Formato de Salida Esperado: JSON con 'analysis_summary', 'key_observations' (lista), 'recommendations' (lista)."
        )
        prompt_parts.append(
        ...
        final_prompt = "\n".join(prompt_parts)
        logger.debug(
            f"Prompt para Gemini (analizar rendimiento): {final_prompt[:500]}..."
        )
        ...
        if not self.gemini_client or not hasattr(
            ...
            logger.error(
                "GeminiClient no está configurado correctamente. No se puede analizar el rendimiento."
            )
            return {
                "error": "GeminiClient not configured",
        ...
        if not isinstance(analysis, dict) or "analysis_summary" not in analysis:
            logger.error(
                f"La salida de Gemini para el análisis de rendimiento no tiene la estructura esperada. Salida: {analysis}"
            )
    ...
    async def _get_program_type_from_profile(self, context: Dict[str, Any]) -> str:
        ...
        """
        logger.debug(
            f"Determinando tipo de programa desde contexto usando servicio centralizado: {context}"
        )

        # Utilizar el servicio de clasificación de programas
        program_type = await self.program_classification_service.classify_program_type(
            context
        )
        logger.info(
            f"Tipo de programa determinado por el servicio de clasificación: {program_type}"
        )
    ...
    async def _set_training_intensity_volume(
        ...
        prompt_parts.append(
            "Formato de Salida Esperado: JSON con 'adjustment_summary', 'recommended_intensity' (dict), 'recommended_volume' (dict)."
        )
        prompt_parts.append(
        ...
        if not self.gemini_client or not hasattr(
            ...
            logger.error(
                "GeminiClient no está configurado correctamente. No se pueden establecer I/V."
            )
            return {
                "error": "GeminiClient not configured",
                "message": "Unable to set intensity/volume.",
            }
        ...
        if not isinstance(settings, dict) or "adjustment_summary" not in settings:
            logger.error(
                f"La salida de Gemini para el ajuste de I/V no tiene la estructura esperada. Salida: {settings}"
            )
            return {
                "error": "Invalid I/V settings structure from LLM",
                "details": settings,
            }
    ...
    async def _skill_analyze_exercise_form(
        self, input_data: AnalyzeExerciseFormInput
    ) -> AnalyzeExerciseFormOutput:
        """
        ...
        """
        logger.info(
            f"Ejecutando habilidad: _skill_analyze_exercise_form para ejercicio: {input_data.exercise_name}"
        )
        ...
        try:
            ...
            analysis_focus = input_data.analysis_focus or [
                "postura",
                "alineación",
                "rango de movimiento",
            ]
            ...
            with self.tracer.start_as_current_span("exercise_form_analysis"):
                ...
                if isinstance(correction_points_response, list):
                        ...
                        if isinstance(point, dict) and "body_part" in point:
                            correction_points.append(
                                FormCorrectionPoint(
                                    ...
                                    correction=point.get(
                                        "correction", "No especificado"
                                    ),
                    ...
                    isinstance(correction_points_response, dict)
                    and "correction_points" in correction_points_response
                ):
                ...
                strengths_recommendations_response = (
                    await self.gemini_client.generate_structured_output(
                        strengths_recommendations_prompt
                    )
                )
                ...
                if isinstance(strengths_recommendations_response, dict):
                    ...
                    recommendations = strengths_recommendations_response.get(
                        "recommendations", []
                    )
                ...
                if not strengths:
                    strengths = [
                        "Disposición para mejorar la técnica",
                        "Interés en el análisis de forma",
                    ]

                if not recommendations:
                    recommendations = [
                        "Consulta a un entrenador personal para una evaluación detallada",
                        "Practica frente a un espejo para mayor conciencia corporal",
                    ]
                ...
                try:
                    ...
                    score_response = await self.gemini_client.generate_text(
                        score_prompt
                    )
                    try:
    ...
    def _generate_mock_exercise_form_analysis(
        self, input_data: AnalyzeExerciseFormInput
    ) -> AnalyzeExerciseFormOutput:
        """
    ...
    async def _skill_compare_exercise_progress(
        self, input_data: CompareExerciseProgressInput
    ) -> CompareExerciseProgressOutput:
        """
        ...
        """
        logger.info(
            f"Ejecutando habilidad: _skill_compare_exercise_progress para ejercicio: {input_data.exercise_name}"
        )
        ...
        try:
            ...
            metrics_to_compare = input_data.metrics_to_compare or [
                "postura",
                "técnica",
                "rango de movimiento",
                "estabilidad",
            ...
            if (
                ...
                logger.warning(
                    "Capacidades de visión no disponibles. Usando comparación simulada."
                )
            ...
            with self.tracer.start_as_current_span("exercise_progress_comparison"):
                ...
                progress_analysis = await self.gemini_client.generate_structured_output(
                ...
                progress_summary = progress_analysis.get(
                    "progress_summary", "No disponible"
                )
                ...
                if not key_improvements:
                    key_improvements = [
                        {
                            "area": "General",
                            "improvement": "No se pudieron identificar mejoras específicas",
                        }
                    ]
                ...
                if not form_changes:
                    form_changes = [
                        {
                            "aspect": "General",
                            "change": "No se pudieron identificar cambios específicos en la forma",
                        }
                    ]

                if not recommendations:
                    recommendations = [
                        "Continúa con la práctica regular",
                        "Consulta a un entrenador personal para una evaluación detallada",
                    ]
    ...
    def _generate_mock_exercise_progress_comparison(
        self, input_data: CompareExerciseProgressInput
    ) -> CompareExerciseProgressOutput:
        """
        ...
        return CompareExerciseProgressOutput(
            ...
            form_changes=[
                {
                    "aspect": "General",
                    "change": "No se pudieron identificar cambios específicos",
                }
            ],
            recommendations=[
                "Consulta a un entrenador personal para una evaluación detallada"
    ...
    async def _skill_workout_voice_guide(self, **kwargs) -> Dict[str, Any]:
        """
        Proporciona guía de voz durante entrenamientos.

        Args:
            **kwargs: Parámetros que incluyen:
                - exercise: Información del ejercicio actual
                - phase: Fase del ejercicio (introduction, setup, execution, breathing, counting)
                - rep_count: Número de repetición actual
                - set_number: Número de serie actual
                - rest_seconds: Segundos de descanso entre series
                - voice_preferences: Preferencias de voz del usuario

        Returns:
            Dict con audio de guía y metadata
        """
        try:
            exercise = kwargs.get("exercise", {})
            phase = kwargs.get("phase", "introduction")
            rep_count = kwargs.get("rep_count", 0)
            set_number = kwargs.get("set_number", 1)
            rest_seconds = kwargs.get("rest_seconds", 60)
            voice_preferences = kwargs.get("voice_preferences", {})

            # Enriquecer información del ejercicio si es necesario
            if not exercise.get("name"):
                exercise["name"] = "el ejercicio"

            # Agregar información específica según la fase
            if phase == "introduction" and not exercise.get("sets"):
                exercise["sets"] = 3
                exercise["reps"] = 10

            # Agregar instrucciones específicas del ejercicio
            if phase == "setup" and not exercise.get("setup_instructions"):
                exercise["setup_instructions"] = self._get_exercise_setup_instructions(
                    exercise.get("name", "")
                )

            if phase == "execution" and not exercise.get("key_points"):
                exercise["key_points"] = self._get_exercise_key_points(
                    exercise.get("name", "")
                )

            if phase == "breathing" and not exercise.get("inhale_phase"):
                breathing = self._get_exercise_breathing_pattern(
                    exercise.get("name", "")
                )
                exercise["inhale_phase"] = breathing["inhale"]
                exercise["exhale_phase"] = breathing["exhale"]

            # Usar la skill de guía de voz
            guide_result = await self.workout_voice_guide_skill.execute(
                {
                    "exercise": exercise,
                    "phase": phase,
                    "rep_count": rep_count,
            ...
            if guide_result.get("status") != "success":
                return {
                    "status": "error",
                    "error": guide_result.get("error", "Error generando guía de voz"),
                }

            # Agregar metadata adicional para el entrenador
            return {
                "status": "success",
                "audio_base64": guide_result.get("audio_base64"),
                "text": guide_result.get("text"),
                "duration": guide_result.get("duration", 3.0),
                "phase": phase,
                "next_cue_timing": guide_result.get("next_cue_timing", 3.0),
                "exercise_info": {
            ...
            logger.error(f"Error en guía de voz para entrenamiento: {e}")
            return {"status": "error", "error": str(e)}

    async def _skill_process_workout_command(self, **kwargs) -> Dict[str, Any]:
        """
        Procesa comandos de voz durante entrenamientos.

        Args:
            **kwargs: Parámetros que incluyen:
                - audio_data: Datos del audio del comando
                - workout_state: Estado actual del entrenamiento
                - current_exercise: Ejercicio actual
                - session_data: Datos de la sesión de entrenamiento

        Returns:
            Dict con comando identificado y acción a ejecutar
        """
        try:
            ...
            workout_state = kwargs.get("workout_state", {})
            current_exercise = kwargs.get("current_exercise", {})
            session_data = kwargs.get("session_data", {})

            # Enriquecer el estado del entrenamiento con información del contexto
            enriched_state = {
                **workout_state,
                "current_exercise": current_exercise.get("name", ""),
                "current_set": workout_state.get("current_set", 1),
                "total_sets": current_exercise.get("sets", 3),
                "current_rep": workout_state.get("current_rep", 0),
                "total_reps": current_exercise.get("reps", 10),
                "rest_seconds": current_exercise.get("rest_seconds", 60),
                "exercise_index": workout_state.get("exercise_index", 0),
                "total_exercises": len(session_data.get("exercises", [])),
            }

            # Usar la skill de comandos de voz
            command_result = await self.voice_command_skill.execute(
                {
                    "audio_data": audio_data,
                    "language_code": "es-ES",
                    "workout_state": enriched_state,
                }
            )
            ...
            if command_result.get("status") != "success":
                return {
                    "status": "error",
                    "error": command_result.get("error", "Error procesando comando"),
                }

            # Procesar la acción según el comando identificado
            action = command_result.get("action")
            command = command_result.get("command")

            # Agregar lógica específica del entrenador para ciertas acciones
            enhanced_response = {
                **command_result,
                "trainer_guidance": self._get_trainer_guidance_for_action(
                    action, enriched_state
                ),
            }

            # Si el comando es para mostrar forma, agregar información técnica
            if command == "show_form":
                enhanced_response["form_details"] = (
                    self._get_detailed_form_instructions(
                        current_exercise.get("name", "")
                    )
                )

            # Si es un comando de progreso, agregar estadísticas
            if command == "workout_status":
                enhanced_response["workout_stats"] = self._calculate_workout_stats(
                    enriched_state, session_data
                )
    ...
    async def _skill_exercise_audio_feedback(self, **kwargs) -> Dict[str, Any]:
        """
        Genera feedback de audio para corrección de forma y motivación.

        Args:
            **kwargs: Parámetros que incluyen:
                - feedback_type: Tipo de feedback (form_correction, encouragement, etc.)
                - exercise_name: Nombre del ejercicio actual
                - form_issues: Problemas de forma detectados (opcional)
                - performance_data: Datos de rendimiento (opcional)
                - user_profile: Perfil del usuario
                - voice_preferences: Preferencias de voz

        Returns:
            Dict con audio del feedback y metadata
        """
        try:
            feedback_type = kwargs.get("feedback_type", "encouragement")
            exercise_name = kwargs.get("exercise_name", "el ejercicio")
            form_issues = kwargs.get("form_issues", [])
            performance_data = kwargs.get("performance_data", {})
            user_profile = kwargs.get("user_profile", {})
            voice_preferences = kwargs.get("voice_preferences", {})

            # Generar feedback específico según el tipo y contexto
            if feedback_type == "form_correction" and form_issues:
                # Priorizar el problema de forma más importante
                primary_issue = form_issues[0] if form_issues else None
                if primary_issue:
                    feedback_message = f"Atención a tu {primary_issue.get('area', 'postura')}. {primary_issue.get('correction', 'Mantén la forma correcta')}."
                else:
                    feedback_message = "Recuerda mantener la técnica correcta durante todo el movimiento."

            elif feedback_type == "set_completion":
                # Feedback personalizado basado en rendimiento
                reps_completed = performance_data.get("reps_completed", 0)
                target_reps = performance_data.get("target_reps", 10)

                if reps_completed >= target_reps:
                    feedback_message = f"¡Excelente! Completaste todas las {reps_completed} repeticiones con buena forma."
                elif reps_completed >= target_reps * 0.8:
                    feedback_message = f"¡Muy bien! {reps_completed} repeticiones es un gran esfuerzo. La próxima vez llegarás a {target_reps}."
                else:
                    feedback_message = f"Buen trabajo con {reps_completed} repeticiones. Escucha a tu cuerpo y progresa gradualmente."

            elif feedback_type == "rest_reminder":
                rest_seconds = performance_data.get("rest_seconds", 60)
                next_exercise = performance_data.get(
                    "next_exercise", "el siguiente ejercicio"
                )
                feedback_message = f"Toma {rest_seconds} segundos de descanso. Hidrátate y prepárate para {next_exercise}."

            else:
                # Mensaje de aliento general
                feedback_message = self._generate_contextual_encouragement(
                    exercise_name, performance_data, user_profile
                )

            # Usar la skill de feedback de audio
            feedback_result = await self.audio_feedback_skill.execute(
                {
                    "feedback_type": feedback_type,
                    "parameters": {
                        "custom_message": feedback_message,
                        "exercise_name": exercise_name,
                    },
                    "voice_settings": voice_preferences,
            ...
            if feedback_result.get("status") != "success":
                return {
                    "status": "error",
                    "error": feedback_result.get(
                        "error", "Error generando feedback de audio"
                    ),
                }
            ...
            return {
                ...
                "duration": feedback_result.get("duration_estimate", 3.0),
                "feedback_type": feedback_type,
                "voice_used": feedback_result.get("voice_used"),
                "context": {
                    "exercise": exercise_name,
                    "targeted": bool(form_issues or performance_data),
                },
            }
    ...
    def _get_exercise_setup_instructions(self, exercise_name: str) -> str:
        """Obtiene instrucciones de configuración para un ejercicio."""
        setup_instructions = {
            "sentadilla": "Pies al ancho de hombros, peso en los talones, pecho arriba",
            "press banca": "Acuéstate con los ojos bajo la barra, retrae los omóplatos, pies firmes en el suelo",
            "peso muerto": "Pies al ancho de caderas, barra sobre medio pie, espalda neutra",
            "dominadas": "Agarre al ancho de hombros, cuelga con brazos extendidos, activa los dorsales",
        }

        exercise_lower = exercise_name.lower()
        for key, instruction in setup_instructions.items():
            if key in exercise_lower:
                return instruction

        return "Adopta la posición inicial correcta para el ejercicio"

    def _get_exercise_key_points(self, exercise_name: str) -> str:
        """Obtiene puntos clave de ejecución para un ejercicio."""
        key_points = {
            "sentadilla": "Baja controlado hasta romper el paralelo, mantén las rodillas alineadas con los pies",
            "press banca": "Baja la barra al pecho con control, empuja explosivamente manteniendo la técnica",
            "peso muerto": "Mantén la barra cerca del cuerpo, extiende caderas y rodillas simultáneamente",
            "dominadas": "Sube hasta que la barbilla pase la barra, baja con control total",
        }
        ...
        return "Ejecuta el movimiento con control y rango completo"

    def _get_exercise_breathing_pattern(self, exercise_name: str) -> Dict[str, str]:
        """Obtiene el patrón de respiración para un ejercicio."""
        breathing_patterns = {
            "sentadilla": {"inhale": "al bajar", "exhale": "al subir"},
            "press banca": {"inhale": "al bajar la barra", "exhale": "al empujar"},
            "peso muerto": {
                "inhale": "antes de levantar",
                "exhale": "al completar el movimiento",
            },
            "dominadas": {"inhale": "al bajar", "exhale": "al subir"},
        }

        exercise_lower = exercise_name.lower()
        for key, pattern in breathing_patterns.items():
            if key in exercise_lower:
                return pattern

        return {"inhale": "en la fase excéntrica", "exhale": "en la fase concéntrica"}

    def _get_trainer_guidance_for_action(
        self, action: str, workout_state: Dict[str, Any]
    ) -> str:
        """Genera guía específica del entrenador para una acción."""
        guidance_map = {
            "pause": "Buen momento para evaluar tu forma y ajustar si es necesario.",
            "resume": "Retoma con la técnica correcta. La calidad sobre cantidad.",
            "next_exercise": "Prepárate mentalmente para el siguiente ejercicio. Visualiza la ejecución perfecta.",
            "mark_set_complete": "Evalúa cómo te sentiste. ¿Podrías hacer 1-2 repeticiones más con buena forma?",
            "show_form": "La técnica perfecta es tu prioridad número uno para prevenir lesiones y maximizar resultados.",
        }

        return guidance_map.get(
            action, "Mantén el enfoque en la calidad del movimiento."
        )

    def _get_detailed_form_instructions(
        self, exercise_name: str
    ) -> Dict[str, List[str]]:
        """Obtiene instrucciones detalladas de forma para un ejercicio."""
        form_details = {
            "sentadilla": {
                "postura": [
                    "Pies ligeramente más anchos que los hombros",
                    "Dedos apuntando ligeramente hacia afuera",
                ],
                "movimiento": [
        ...
        exercise_lower = exercise_name.lower()
        for key, details in form_details.items():
            if key in exercise_lower:
                return details

        return {
            "postura": ["Mantén una postura estable y controlada"],
            "movimiento": ["Ejecuta el movimiento con control total"],
            "errores_comunes": [
                "Movimiento demasiado rápido",
                "Rango de movimiento incompleto",
            ],
            "señales": ["Respira correctamente", "Mantén la concentración"],
        }

    def _calculate_workout_stats(
        self, workout_state: Dict[str, Any], session_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Calcula estadísticas del entrenamiento actual."""
        current_exercise_index = workout_state.get("exercise_index", 0)
        total_exercises = workout_state.get("total_exercises", 0)

        # Calcular progreso
        if total_exercises > 0:
            progress_percentage = (current_exercise_index / total_exercises) * 100
        else:
            progress_percentage = 0

        # Calcular volumen total
        completed_sets = workout_state.get("completed_sets", 0)
        completed_reps = workout_state.get("total_reps_completed", 0)

        # Estimar tiempo restante
        avg_set_duration = session_data.get(
            "avg_set_duration", 120
        )  # 2 minutos por serie promedio
        remaining_sets = workout_state.get("remaining_sets", 0)
        estimated_time_remaining = remaining_sets * avg_set_duration

        return {
            "progress_percentage": round(progress_percentage, 1),
            "exercises_completed": current_exercise_index,
            "exercises_remaining": total_exercises - current_exercise_index,
            "total_volume": {"sets": completed_sets, "reps": completed_reps},
            "estimated_time_remaining_seconds": estimated_time_remaining,
            "current_exercise_progress": {
                "set": workout_state.get("current_set", 1),
                "total_sets": workout_state.get("total_sets", 3),
                "reps": workout_state.get("current_rep", 0),
                "total_reps": workout_state.get("total_reps", 10),
            },
        }

    def _generate_contextual_encouragement(
        self,
        exercise_name: str,
        performance_data: Dict[str, Any],
        user_profile: Dict[str, Any],
    ) -> str:
        """Genera mensajes de aliento contextualizados."""

        # Mensajes base por tipo de ejercicio
        if "sentadilla" in exercise_name.lower() or "squat" in exercise_name.lower():
            base_messages = [
                "¡Excelente profundidad en la sentadilla! Sigue así.",
                "Tu técnica en sentadilla está mejorando notablemente.",
                "Mantén esa activación del core. Perfecto para la sentadilla.",
            ]
        elif "press" in exercise_name.lower() or "bench" in exercise_name.lower():
            base_messages = [
                "¡Buen control en el press! Mantén esa estabilidad.",
                "Excelente trayectoria de barra. Sigue con esa técnica.",
                "Tu press de banca muestra gran progreso.",
            ]
        else:
            base_messages = [
                f"¡Excelente ejecución en {exercise_name}!",
                f"Tu técnica en {exercise_name} es impecable.",
                f"Sigue así con {exercise_name}. Gran trabajo.",
            ]
        ...
        # Personalizar si hay nombre de usuario
        if user_profile.get("name"):
            message = f"{user_profile['name']}, {message.lower()}"
