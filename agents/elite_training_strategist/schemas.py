...
class AnalyzeExerciseFormInput(BaseModel):
    ...
    analysis_focus: Optional[List[str]] = Field(
        None, description="Aspectos específicos a analizar (postura, alineación, rango de movimiento, etc.)"
    )


class FormCorrectionPoint(BaseModel):
    """Modelo para un punto de corrección de forma en un ejercicio."""
    body_part: str = Field(..., description="Parte del cuerpo relacionada con la corrección")
    issue: str = Field(..., description="Problema identificado")
    correction: str = Field(..., description="Corrección recomendada")
    severity: str = Field(..., description="Severidad del problema (leve, moderada, grave)")
...
class AnalyzeExerciseFormOutput(BaseModel):
    ...
    exercise_name: str = Field(..., description="Nombre del ejercicio identificado o proporcionado")
    form_quality_score: float = Field(..., description="Puntuación general de calidad de forma (0-10)")
    form_analysis: str = Field(..., description="Análisis detallado de la forma del ejercicio")
    correction_points: List[FormCorrectionPoint] = Field(..., description="Puntos específicos de corrección")
...
class CompareExerciseProgressOutput(BaseModel):
    ...
    key_improvements: List[Dict[str, Any]] = Field(..., description="Mejoras clave identificadas")
    form_changes: List[Dict[str, Any]] = Field(..., description="Cambios en la forma del ejercicio")
