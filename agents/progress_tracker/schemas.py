...
class AnalyzeBodyProgressInput(BaseModel):
    ...
    user_id: Optional[str] = Field(
        None, description="ID del usuario"
    )
    time_between_images: Optional[str] = Field(
        None, description="Tiempo transcurrido entre las imágenes"
    )
    focus_areas: Optional[List[str]] = Field(
        None, description="Áreas específicas a analizar (composición corporal, postura, etc.)"
    )
...
class BodyAreaChange(BaseModel):
    ...
    improvement_level: str = Field(..., description="Nivel de mejora (ninguno, leve, moderado, significativo)")
...
class AnalyzeBodyProgressOutput(BaseModel):
    ...
    metrics: List[BodyChangeMetric] = Field(..., description="Métricas de cambio corporal")
    body_areas: List[BodyAreaChange] = Field(..., description="Cambios por áreas corporales")
    overall_progress_score: float = Field(..., description="Puntuación general de progreso (0-10)")
    recommendations: List[str] = Field(..., description="Recomendaciones basadas en el progreso")
    comparison_image_url: Optional[str] = Field(None, description="URL de la imagen comparativa generada")
...
class GenerateProgressVisualizationInput(BaseModel):
    ...
    visualization_type: str = Field(
        "side_by_side", description="Tipo de visualización (side_by_side, overlay, grid, timeline)"
    )
...
class GenerateProgressVisualizationOutput(BaseModel):
    ...
    notes: Optional[str] = Field(None, description="Notas adicionales sobre la visualización")
...
class BodyProgressAnalysisArtifact(BaseModel):
    ...
    comparison_image_url: Optional[str] = Field(None, description="URL de la imagen comparativa")
