...
from agents.skills.visualization_skills import (
...
class ProgressTracker(ADKAgent):
    ...
    AGENT_DESCRIPTION = (
        "Especialista en seguimiento, análisis y visualización de progreso"
    )
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Agregar skills de visualización avanzada
        ...
        _skills.extend(visualization_skills)
        ...
        try:
            ...
            class MockMultimodalAdapter:
                async def compare_images(
                    self, image_data1, image_data2, comparison_prompt, **kwargs
                ):
        ...
        try:
            ...
            logger.info(
                "AI Platform (Vertex AI SDK) inicializado correctamente para ProgressTracker."
            )
        except Exception as e:
            logger.error(
                f"Error al inicializar AI Platform para ProgressTracker: {e}",
                exc_info=True,
            )
    ...
    async def _skill_analyze_progress(
        self, input_data: AnalyzeProgressInput
    ) -> AnalyzeProgressOutput:
        """Skill para analizar el progreso del usuario."""
        logger.info(
            f"Ejecutando habilidad: _skill_analyze_progress con input: {input_data}"
        )
        user_id = input_data.user_id
        time_period = input_data.time_period
        metrics = input_data.metrics
        user_profile = (
            input_data.user_profile if hasattr(input_data, "user_profile") else {}
        )
        ...
        try:
            ...
            try:
                ...
                logger.info(
                    f"Tipo de programa determinado para análisis de progreso: {program_type}"
                )
                ...
                if program_def:
                    ...
                    if program_type == "PRIME":
                        ...
                        program_context += (
                            "- Enfoque en métricas de rendimiento y progresión\n"
                        )
                        program_context += (
                            "- Análisis de tendencias de capacidad de trabajo\n"
                        )
                        program_context += (
                            "- Identificación de patrones de recuperación\n"
                        )
                        ...
                        program_context += (
                            "- Enfoque en métricas de salud a largo plazo\n"
                        )
                        program_context += "- Análisis de tendencias de biomarcadores\n"
                        program_context += (
                            "- Identificación de patrones de bienestar general\n"
                        )
            except Exception as e:
                logger.warning(
            ...
            metrics_str = (
                ", ".join(metrics) if metrics else "todas las métricas disponibles"
            )
            ...
            analysis_result = await self.gemini_client.generate_structured_output(
                prompt
            )
            ...
            logger.info(
                f"Análisis completado para user {user_id} con programa {program_type}"
            )
            return AnalyzeProgressOutput(
                analysis_id=analysis_id, result=analysis_result, status="success"
            )

        except Exception as e:
            logger.error(
                f"Error en skill '_skill_analyze_progress': {e}", exc_info=True
            )
            raise

    async def _skill_visualize_progress(
        self, input_data: VisualizeProgressInput
    ) -> VisualizeProgressOutput:
        """Skill para visualizar el progreso del usuario."""
        logger.info(
            f"Ejecutando habilidad: _skill_visualize_progress con input: {input_data}"
        )
        user_id = input_data.user_id
        ...
        try:
            ...
            try:
                ...
                logger.info(
                    f"Tipo de programa determinado para visualización de progreso: {program_type}"
                )
                ...
                if program_def and program_def.get("key_metrics"):
                    ...
                    if is_key_metric:
                        logger.info(
                            f"La métrica {metric} es una métrica clave para el programa {program_type}"
                        )
            except Exception as e:
                logger.warning(
                    f"No se pudo determinar el tipo de programa: {e}. Usando visualización general."
                )
                program_type = "GENERAL"
                program_def = None
                is_key_metric = False
            ...
            if not user_data or metric not in user_data or not user_data[metric]:
                raise ValueError(
                    f"No se encontraron datos para la métrica '{metric}'."
            ...
            try:
                ...
                logger.warning(
                    f"No se pudieron parsear algunas fechas para visualización: {dates}"
                )
            ...
            if (
                program_def
                and program_def.get("metric_targets")
                and metric in program_def.get("metric_targets", {})
            ):
                try:
                    target_range = program_def["metric_targets"][metric]
                    if (
                        isinstance(target_range, dict)
                        and "min" in target_range
                        and "max" in target_range
                    ):
            ...
            plt.title(
                f"{title_prefix}Progreso de {metric.capitalize()} para Usuario {user_id} ({time_period})"
            )
            ...
            return VisualizeProgressOutput(
                visualization_url=viz_url, filepath=viz_filepath, status="success"
            )

        except Exception as e:
            logger.error(
                f"Error en skill '_skill_visualize_progress': {e}", exc_info=True
            )
    ...
    async def _skill_compare_progress(
        self, input_data: CompareProgressInput
    ) -> CompareProgressOutput:
        """Skill para comparar el progreso entre dos periodos."""
        logger.info(
            f"Ejecutando habilidad: _skill_compare_progress con input: {input_data}"
        )
        user_id = input_data.user_id
        ...
        try:
            ...
            try:
                ...
                logger.info(
                    f"Tipo de programa determinado para comparación de progreso: {program_type}"
                )
                ...
                if program_def:
                    ...
                    if program_type == "PRIME":
                        ...
                        program_context += (
                            "- Análisis de consistencia en patrones de entrenamiento\n"
                        )
                        program_context += (
                            "- Evaluación de adaptación a cargas de trabajo\n"
                        )
                    elif program_type == "LONGEVITY":
                        ...
                        program_context += (
                            "- Análisis de estabilidad en métricas de salud\n"
                        )
                        program_context += (
                            "- Evaluación de mejoras sostenibles en bienestar general\n"
                        )
            except Exception as e:
                logger.warning(
                    f"No se pudo determinar el tipo de programa: {e}. Usando comparación general."
                )
                program_type = "GENERAL"
                program_context = ""
                program_def = None
            ...
            key_metrics_str = (
                ", ".join(key_metrics_in_request) if key_metrics_in_request else ""
            )
            key_metrics_context = (
                f"\nPresta especial atención a las siguientes métricas clave para el programa {program_type}: {key_metrics_str}"
            ...
            comparison_result = await self.gemini_client.generate_structured_output(
                prompt
            )
            ...
            if program_def:
                comparison_result["program_objective"] = program_def.get(
                    "objective", ""
                )
                if key_metrics_in_request:
            ...
            logger.info(
                f"Comparación completada para user {user_id} con programa {program_type}"
            )
            return CompareProgressOutput(result=comparison_result, status="success")

        except Exception as e:
            logger.error(
                f"Error en skill '_skill_compare_progress': {e}", exc_info=True
            )
            raise

    # Método auxiliar para obtener datos (simulación)
    async def _get_user_data(
        self, user_id: str, time_period: str, metrics: Optional[List[str]]
    ) -> Optional[Dict[str, Any]]:
        """Obtiene datos del usuario (simulado). Debería interactuar con Supabase/StateManager."""
        logger.debug(
            f"Simulando obtención de datos para user {user_id}, periodo {time_period}, métricas: {metrics}"
        )
        ...
        for metric in requested_metrics:
            ...
            while current_date <= datetime.date.today():
                value = (
                    base_value
                    + (current_date - start_date).days * trend
                    + np.random.randn() * (1 if metric == "weight" else 2)
                )
                mock_data[metric].append(
                    {"date": current_date.isoformat(), "value": round(value, 2)}
                )
    ...
    async def _skill_analyze_body_progress(
        self, input_data: AnalyzeBodyProgressInput
    ) -> AnalyzeBodyProgressOutput:
        """
        ...
        """
        logger.info(
            f"Ejecutando habilidad: _skill_analyze_body_progress para usuario: {input_data.user_id}"
        )
        ...
        try:
            ...
            try:
                ...
                logger.info(
                    f"Tipo de programa determinado para análisis de progreso corporal: {program_type}"
                )
                ...
                if program_def:
                    ...
                    if program_type == "PRIME":
                        program_context += (
                            "- Enfoque en desarrollo muscular y rendimiento físico\n"
                        )
                    elif program_type == "LONGEVITY":
                        program_context += (
                            "- Enfoque en composición corporal saludable y postura\n"
                        )
            except Exception as e:
                logger.warning(
                    f"No se pudo determinar el tipo de programa: {e}. Usando análisis general."
            ...
            with self.tracer.start_as_current_span("body_progress_analysis"):
                ...
                before_analysis = await self.vision_processor.analyze_image(
                    before_image
                )
                ...
                metrics_response = await self.gemini_client.generate_structured_output(
                    metrics_prompt
                )
                ...
                if isinstance(body_areas_response, list):
                        ...
                        if isinstance(area, dict) and "body_area" in area:
                            body_areas.append(
                                BodyAreaChange(
                                    ...
                                    changes_observed=area.get(
                                        "changes_observed", ["No especificado"]
                                    ),
                                    improvement_level=area.get(
                    ...
                    isinstance(body_areas_response, dict)
                    and "body_areas" in body_areas_response
                ):
                ...
                if isinstance(recommendations_response, list):
                    recommendations = [
                        rec for rec in recommendations_response if isinstance(rec, str)
                    ]
                elif (
                ...
                progress_summary = (
                    ...
                    else "No se pudo generar un resumen del progreso."
                )
            ...
            return AnalyzeBodyProgressOutput(
                ...
                body_areas=[
                    BodyAreaChange(
                        body_area="General",
                        changes_observed=[
                            "No se pudieron determinar cambios debido a un error"
                        ],
                ...
                recommendations=[
                    "Intente nuevamente con imágenes de mejor calidad o en mejor iluminación."
                ],
                comparison_image_url=None,
            )

    async def _skill_generate_progress_visualization(
        self, input_data: GenerateProgressVisualizationInput
    ) -> GenerateProgressVisualizationOutput:
        """
        ...
        """
        logger.info(
            f"Ejecutando habilidad: _skill_generate_progress_visualization para usuario: {input_data.user_id}"
        )
        ...
        if (
            ...
            logger.warning(
                "Las capacidades de visión no están disponibles. Usando visualización simulada."
            )
        ...
        try:
            ...
            if not images or len(images) < 2:
                raise ValueError(
                    "Se requieren al menos dos imágenes para generar una visualización de progreso."
                )
            ...
            with self.tracer.start_as_current_span("progress_visualization_generation"):
                ...
                visualization_filename = (
                    f"progress_viz_{user_id}_{visualization_id}.png"
                )
                visualization_filepath = os.path.join(
                    self.tmp_dir, visualization_filename
                )
                ...
                if visualization_type == "side_by_side":
                    ...
                    fig, axes = plt.subplots(
                        1, len(processed_images), figsize=(5 * len(processed_images), 5)
                    )
                    ...
                    ax.plot(
                        [i for i in range(len(processed_images))],
            ...
            logger.error(
                f"Error al generar visualización de progreso: {e}", exc_info=True
            )
    ...
    async def _generate_mock_body_progress_analysis(
        self, input_data: AnalyzeBodyProgressInput
    ) -> AnalyzeBodyProgressOutput:
        """
    ...
    async def _generate_mock_progress_visualization(
        self, input_data: GenerateProgressVisualizationInput
    ) -> GenerateProgressVisualizationOutput:
        """
        ...
        ax.text(
            0.5, 0.2, note, horizontalalignment="center", fontsize=10, style="italic"
        )
        ...
        if len(images) >= 2:
            try:
                ...
                logger.warning(
                    f"No se pudo calcular el período de tiempo simulado: {e}"
                )
