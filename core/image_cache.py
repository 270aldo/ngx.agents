...
class ImageCache:
    ...
    def __init__(self, max_size: int = 100, ttl_seconds: int = 3600, telemetry: Optional[Telemetry] = None):
        """
        ...
        logger.info(f"ImageCache inicializado con max_size={max_size}, ttl_seconds={ttl_seconds}")
    
    async def get(self, key: str, operation_type: Optional[str] = None) -> Optional[Dict[str, Any]]:
        """
        ...
        if self.telemetry:
            ...
            if operation_type:
                self.telemetry.add_span_attribute(span, "operation_type", operation_type)
            ...
            async with self.lock:
                if key in self.cache:
                    ...
                    if time.time() - entry["timestamp"] > self.ttl_seconds:
                        ...
                        if self.telemetry:
                            self.telemetry.add_span_attribute(span, "cache_result", "expired")
                            self.telemetry.record_metric("image_cache.expired", 1, {"operation_type": operation_type or "unknown"})
                    ...
                    if self.telemetry:
                        ...
                        self.telemetry.record_metric("image_cache.hits", 1, {"operation_type": operation_type or "unknown"})
                ...
                if self.telemetry:
                    ...
                    self.telemetry.record_metric("image_cache.misses", 1, {"operation_type": operation_type or "unknown"})
    ...
    async def set(self, key: str, result: Dict[str, Any], image_size: int = 0, operation_type: Optional[str] = None) -> None:
        """
            ...
            async with self.lock:
                ...
                if len(self.cache) >= self.max_size and key not in self.cache:
                    ...
                    if oldest_key:
                        ...
                        if self.telemetry:
                            self.telemetry.record_metric("image_cache.evictions", 1, {"operation_type": operation_type or "unknown"})
                ...
                if self.telemetry:
                    self.telemetry.record_metric("image_cache.sets", 1, {"operation_type": operation_type or "unknown"})
    ...
    async def generate_key(self, image_data: Union[str, bytes], params: Optional[Dict[str, Any]] = None) -> str:
        """
    ...
    async def clear_expired(self) -> int:
        ...
        try:
            async with self.lock:
                ...
                if self.telemetry:
                    self.telemetry.add_span_attribute(span, "expired_entries_removed", len(keys_to_remove))
                    self.telemetry.record_metric("image_cache.expired_entries_removed", len(keys_to_remove))
                
                logger.debug(f"Eliminadas {len(keys_to_remove)} entradas expiradas de la caché")
    ...
    async def clear(self) -> None:
        ...
        try:
            async with self.lock:
                ...
                if self.telemetry:
                    self.telemetry.add_span_attribute(span, "entries_removed", previous_size)
                    self.telemetry.record_metric("image_cache.clear", 1)
                    self.telemetry.record_metric("image_cache.size", 0)
                
                logger.debug(f"Caché limpiada completamente, eliminadas {previous_size} entradas")
