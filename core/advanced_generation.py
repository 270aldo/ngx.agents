...
class AdvancedGeneration:
    ...
    async def generate_response(self, 
        ...
        try:
            ...
            telemetry_manager.set_span_attribute(span_id, "response_length", len(response.get("text", "")))
            return response
    ...
    async def generate_with_few_shot(self, 
                                  prompt: str,
        ...
        try:
            ...
            logger.error(f"Error al generar respuesta con few-shot: {str(e)}", exc_info=True)
            self.stats["errors"] += 1
            telemetry_manager.set_span_attribute(span_id, "error", str(e))
    ...
    async def generate_with_chain_of_thought(self, 
                                         prompt: str,
                                         system_instruction: Optional[str] = None,
                                         temperature: Optional[float] = None,
                                         max_tokens: Optional[int] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            telemetry_manager.set_span_attribute(span_id, "response_length", len(full_text))
            return structured_response
            
        except Exception as e:
            logger.error(f"Error al generar respuesta con chain-of-thought: {str(e)}", exc_info=True)
            self.stats["errors"] += 1
            telemetry_manager.set_span_attribute(span_id, "error", str(e))
    ...
    async def generate_structured_output(self, 
        ...
        try:
            ...
            telemetry_manager.set_span_attribute(span_id, "response_length", len(text_response))
            return result
            
        except Exception as e:
            logger.error(f"Error al generar respuesta estructurada: {str(e)}", exc_info=True)
            self.stats["errors"] += 1
            telemetry_manager.set_span_attribute(span_id, "error", str(e))
    ...
    async def generate_with_rag(self, 
        ...
        try:
            ...
            if context_keys:
                ...
                similar_items = await embeddings_manager.find_similar(search_text, top_k=top_k)
            ...
            telemetry_manager.set_span_attribute(span_id, "response_length", len(result["text"]))
            telemetry_manager.set_span_attribute(span_id, "context_items_count", len(context_items))
            return result
