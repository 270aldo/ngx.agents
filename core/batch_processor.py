...
import time
import uuid
from typing import Dict, List, Any, Optional, Callable, Awaitable, Union, Tuple, TypeVar, Generic
...
class BatchResult(Generic[T, R]):
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "execution_time": (self.end_time - self.start_time).total_seconds() if self.end_time and self.start_time else None,
...
class BatchProcessor:
    ...
    def __init__(
        ...
        logger.info(f"BatchProcessor inicializado (max_workers={max_workers}, default_chunk_size={default_chunk_size})")
        ...
        async with self.semaphore:
            for i, item in enumerate(chunk):
                ...
                try:
                    ...
                    logger.error(f"Error procesando elemento {item_index} en lote {batch_result.batch_id}: {error_msg}")
    ...
    async def process_batch(
        ...
        try:
            ...
            if batch_result.end_time and batch_result.start_time:
                execution_time = (batch_result.end_time - batch_result.start_time).total_seconds()
                completed_count = self.stats["completed_batches"] + self.stats["partial_batches"]
    ...
    async def clear_completed_batches(self, older_than: Optional[int] = None) -> int:
        ...
        for batch_id, batch_result in self.results.items():
            if batch_result.status in [BatchStatus.COMPLETED, BatchStatus.FAILED, BatchStatus.CANCELLED, BatchStatus.PARTIAL]:
                if older_than is None or (batch_result.end_time and (now - batch_result.end_time).total_seconds() > older_than):
