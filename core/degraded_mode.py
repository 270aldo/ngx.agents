...
class ServiceStatus:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "degradation_level": self.degradation_level.value if self.degradation_level else 0,
...
class DegradedModeManager:
    ...
    def __init__(self):
        ...
        self.callbacks: List[Callable[[DegradationLevel, Optional[DegradationReason], Optional[str]], None]] = []
        ...
        self.recovery_threshold = 3  # Número de comprobaciones exitosas para recuperación
    ...
    async def _monitor_services(self) -> None:
        ...
        while True:
            try:
                ...
                logger.error(f"Error en monitorización de servicios: {e}", exc_info=True)
        ...
        async with self.lock:
            ...
            if max_level != self.system_degradation_level:
                ...
                if max_level != DegradationLevel.NONE:
                    ...
                    self.system_degradation_message = f"Sistema en modo degradado debido a problemas con el servicio {critical_service.name}" if critical_service else "Sistema en modo degradado"
                else:
                    self.system_degradation_start = None
                    self.system_degradation_message = None
    ...
    async def _notify_status_change(self, old_level: DegradationLevel, new_level: DegradationLevel) -> None:
        """
        ...
        if new_level == DegradationLevel.NONE:
            ...
            logger.warning(f"Sistema en modo degradado: nivel {new_level.name}, razón: {self.system_degradation_reason.value if self.system_degradation_reason else 'desconocida'}")
        
        # Ejecutar callbacks
        for callback in self.callbacks:
            try:
                callback(new_level, self.system_degradation_reason, self.system_degradation_message)
            except Exception as e:
    ...
    async def get_system_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "degradation_reason": self.system_degradation_reason.value if self.system_degradation_reason else None,
            "degradation_start": self.system_degradation_start.isoformat() if self.system_degradation_start else None,
            "degradation_message": self.system_degradation_message,
            "degradation_duration": (datetime.now() - self.system_degradation_start).total_seconds() if self.system_degradation_start else 0,
            "services_total": len(self.services),
            "services_available": sum(1 for service in self.services.values() if service.is_available),
            "services_unavailable": sum(1 for service in self.services.values() if not service.is_available),
            "critical_services_total": sum(1 for service in self.services.values() if service.is_critical),
            "critical_services_available": sum(1 for service in self.services.values() if service.is_critical and service.is_available),
            "critical_services_unavailable": sum(1 for service in self.services.values() if service.is_critical and not service.is_available)
        }
    ...
    async def set_feature_enabled(self, feature_id: str, enabled: bool) -> None:
        ...
        async with self.lock:
            ...
            logger.info(f"Funcionalidad {feature_id} {'habilitada' if enabled else 'deshabilitada'}")
    ...
    async def is_feature_enabled(self, feature_id: str, default: bool = True) -> bool:
        ...
        if self.system_degradation_level == DegradationLevel.CRITICAL and not feature_id.startswith("essential:"):
            return False
            
        # Si el sistema está en modo degradado alto, deshabilitar funcionalidades avanzadas
        if self.system_degradation_level == DegradationLevel.HIGH and feature_id.startswith("advanced:"):
            return False
    ...
    async def register_callback(
        self,
        callback: Callable[[DegradationLevel, Optional[DegradationReason], Optional[str]], None]
    ) -> None:
        """
    ...
    def decorator(func):
        ...
        async def wrapper_async(*args, **kwargs):
            ...
            if not await manager.is_feature_enabled(feature_id, default):
                raise FeatureDisabledException(f"Funcionalidad {feature_id} deshabilitada")
