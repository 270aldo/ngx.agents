...
class VisionProcessor:
    ...
    def __init__(self, model: str = "gemini-1.5-pro-vision"):
        ...
        try:
            ...
            logger.info(f"Inicializando Vertex AI para VisionProcessor con Proyecto: {gcp_project_id}, Región: {gcp_region}")
            aiplatform.init(project=gcp_project_id, location=gcp_region)
            self.vertex_ai_initialized = True
            logger.info("Vertex AI inicializado correctamente para VisionProcessor")
        except Exception as e:
            logger.error(f"Error al inicializar Vertex AI para VisionProcessor: {e}", exc_info=True)
    
    async def analyze_image(self, image_data: Union[str, Dict[str, Any]]) -> Dict[str, Any]:
        """
