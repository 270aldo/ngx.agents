...
class TelemetryManager:
    ...
    def end_span(self, span_id: str) -> None:
        ...
        if span_id in self.spans:
            ...
            duration = self.spans[span_id]["end_time"] - self.spans[span_id]["start_time"]
            self.spans[span_id]["duration"] = duration
            STATS['spans_completed'] += 1
            logger.debug(f"Ended span {self.spans[span_id]['name']} with ID {span_id} (duration: {duration:.3f}s)")
    ...
    def set_span_attribute(self, span_id: str, key: str, value: Any) -> None:
        ...
        if span_id in self.spans:
            ...
            logger.debug(f"Set attribute {key}={value} on span {self.spans[span_id]['name']}")
        else:
    ...
    def record_exception(self, span_id: str, exception: Exception, attributes: Optional[Dict[str, Any]] = None) -> None:
        ...
        if span_id in self.spans:
            ...
            logger.debug(f"Recorded exception {type(exception).__name__} in span {self.spans[span_id]['name']}")
        else:
            logger.warning(f"Attempted to record exception in non-existent span with ID {span_id}")
    ...
    def increment_counter(self, counter_id: str, value: int = 1, attributes: Optional[Dict[str, Any]] = None) -> None:
        """
        ...
        if counter_id in self.counters:
            ...
            logger.debug(f"Incremented counter {self.counters[counter_id]['name']} by {value}")
        else:
            logger.warning(f"Attempted to increment non-existent counter with ID {counter_id}")
    ...
    def record_histogram(self, histogram_id: str, value: float, attributes: Optional[Dict[str, Any]] = None) -> None:
        """
        ...
        if histogram_id in self.histograms:
            ...
            STATS['metrics_recorded'] += 1
            logger.debug(f"Recorded value {value} in histogram {self.histograms[histogram_id]['name']}")
        else:
            logger.warning(f"Attempted to record value in non-existent histogram with ID {histogram_id}")
...
def record_event(component: str, event_name: str, attributes: Optional[Dict[str, Any]] = None) -> None:
    """
    Registra un evento.
...
def record_error(component: str, error_name: str, attributes: Optional[Dict[str, Any]] = None) -> None:
    """
    Registra un error.
    ...
    if telemetry_manager.current_span_id:
        ...
        if telemetry_manager.current_span_id in telemetry_manager.spans:
            telemetry_manager.spans[telemetry_manager.current_span_id]["status"] = "ERROR"
    else:
...
def get_current_span():
    ...
    """
    if telemetry_manager.current_span_id and telemetry_manager.current_span_id in telemetry_manager.spans:
