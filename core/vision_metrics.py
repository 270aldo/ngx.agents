...
class VisionMetrics:
    ...
    async def record_api_call(self, operation: str, agent_id: str, success: bool, 
                            latency_ms: float, tokens: Optional[Dict[str, int]] = None,
        ...
        try:
            async with self.lock:
                ...
                self.metrics["latency"]["min_ms"] = min(self.metrics["latency"]["min_ms"], latency_ms)
                self.metrics["latency"]["max_ms"] = max(self.metrics["latency"]["max_ms"], latency_ms)
                ...
                self.metrics["latency"]["by_operation"][operation]["total_ms"] += latency_ms
                self.metrics["latency"]["by_operation"][operation]["count"] += 1
                self.metrics["latency"]["by_operation"][operation]["min_ms"] = min(
                    self.metrics["latency"]["by_operation"][operation]["min_ms"], latency_ms)
                self.metrics["latency"]["by_operation"][operation]["max_ms"] = max(
                    self.metrics["latency"]["by_operation"][operation]["max_ms"], latency_ms)
                ...
                self.metrics["quality"]["error_rate"] = (
                    self.metrics["api_calls"]["error"] / self.metrics["api_calls"]["total"]
                    if self.metrics["api_calls"]["total"] > 0 else 0.0
                )
                ...
                if self.telemetry:
                    self.telemetry.record_metric("vision.api_calls", 1, {
                        "operation": operation,
                        "agent_id": agent_id,
                        "success": success,
                    ...
                    self.telemetry.record_metric("vision.latency_ms", latency_ms, {
                        "operation": operation,
                        "agent_id": agent_id
                    })
                    ...
                    if tokens:
                        if "prompt" in tokens:
                            self.telemetry.record_metric("vision.tokens.prompt", tokens["prompt"], {
                                "operation": operation,
                                "agent_id": agent_id
                            })
                        if "completion" in tokens:
                            self.telemetry.record_metric("vision.tokens.completion", tokens["completion"], {
                                "operation": operation,
                                "agent_id": agent_id
                            })
                        if "total" in tokens:
                            self.telemetry.record_metric("vision.tokens.total", tokens["total"], {
                                "operation": operation,
                                "agent_id": agent_id
                            })
                
        except Exception as e:
            logger.error(f"Error al registrar llamada a API de visión: {e}", exc_info=True)
    ...
    async def record_image_processed(self, image_size_bytes: int, image_type: str) -> None:
        """
        ...
        async with self.lock:
            ...
            # Exportar métricas a telemetría si está disponible
            if self.telemetry:
                self.telemetry.record_metric("vision.images.processed", 1, {
                    "size_category": "small" if image_size_bytes < 102400 else 
                                    "medium" if image_size_bytes < 1048576 else "large",
                    "image_type": image_type
                })
                
                self.telemetry.record_metric("vision.images.size_bytes", image_size_bytes, {
                    "image_type": image_type
                })
    ...
    async def record_cache_operation(self, hit: bool) -> None:
        ...
        async with self.lock:
            ...
            self.metrics["cache"]["hit_rate"] = (
                self.metrics["cache"]["hits"] / total_cache_ops if total_cache_ops > 0 else 0.0
            )
            
            # Exportar métricas a telemetría si está disponible
            if self.telemetry:
                self.telemetry.record_metric("vision.cache.operation", 1, {
                    "result": "hit" if hit else "miss"
                })
                
                self.telemetry.record_metric("vision.cache.hit_rate", self.metrics["cache"]["hit_rate"])
    ...
    async def record_timeout(self, operation: str, agent_id: str) -> None:
        ...
        async with self.lock:
            ...
            self.metrics["quality"]["timeout_rate"] = (
                (self.metrics["quality"]["timeout_rate"] * self.metrics["api_calls"]["total"] + 1) /
                (self.metrics["api_calls"]["total"] + 1)
            )
            
            # Exportar métricas a telemetría si está disponible
            if self.telemetry:
                self.telemetry.record_metric("vision.timeout", 1, {
                    "operation": operation,
                    "agent_id": agent_id
                })
                
                self.telemetry.record_metric("vision.quality.timeout_rate", self.metrics["quality"]["timeout_rate"])
    ...
    async def record_retry(self, operation: str, agent_id: str, attempt: int) -> None:
        ...
        async with self.lock:
            ...
            self.metrics["quality"]["retry_rate"] = (
                (self.metrics["quality"]["retry_rate"] * self.metrics["api_calls"]["total"] + 1) /
                (self.metrics["api_calls"]["total"] + 1)
            )
            
            # Exportar métricas a telemetría si está disponible
            if self.telemetry:
                self.telemetry.record_metric("vision.retry", 1, {
                    "operation": operation,
                    "agent_id": agent_id,
                    "attempt": attempt
                })
                
                self.telemetry.record_metric("vision.quality.retry_rate", self.metrics["quality"]["retry_rate"])
    ...
    async def _check_thresholds(self) -> None:
        ...
        if (self.metrics["latency"]["count"] > 20 and 
            self.metrics["latency"]["max_ms"] > self.thresholds["latency_p95_ms"]):
            await self._generate_alert(
                "medium",
    ...
    async def _generate_alert(self, severity: str, title: str, message: str, data: Dict[str, Any]) -> None:
        """
        Genera una alerta.
        ...
        for callback in self.alert_callbacks:
            try:
                ...
                logger.error(f"Error al notificar alerta a callback: {e}", exc_info=True)
        
        # Exportar alerta a telemetría si está disponible
        if self.telemetry:
            self.telemetry.record_metric("vision.alert", 1, {
                "severity": severity,
                "title": title
            })
    
    async def register_alert_callback(self, callback: Callable[[Dict[str, Any]], None]) -> None:
        """
    ...
    async def snapshot_metrics(self) -> None:
        ...
        async with self.lock:
            ...
            snapshot = {
                ...
                "latency": {
                    "avg_ms": (self.metrics["latency"]["total_ms"] / self.metrics["latency"]["count"] 
                ...
                "tokens": {
                    "total": self.metrics["tokens"]["total"]
                },
                "images": {
                    "processed": self.metrics["images"]["processed"]
                },
                "cache": {
