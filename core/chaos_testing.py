...
class ChaosTestingManager:
    ...
    async def register_event(self, event: ChaosEvent) -> str:
        ...
        async with self.lock:
            ...
            logger.info(f"Evento de caos registrado: {event.event_id} ({event.event_type})")
    ...
    async def start_event(self, event_id: str) -> bool:
        ...
        if not self.is_enabled:
            logger.warning("No se puede iniciar el evento, las pruebas de caos están deshabilitadas")
            return False
        ...
        async with self.lock:
            ...
            if event.status != "pending":
                logger.warning(f"No se puede iniciar el evento {event_id}, estado actual: {event.status}")
                return False
            ...
            if self.safe_mode and event.event_type in [ChaosEventType.PROCESS_KILL]:
                logger.warning(f"No se puede iniciar el evento {event_id}, es destructivo y el modo seguro está activado")
                return False
    ...
    async def cancel_event(self, event_id: str) -> bool:
        ...
        async with self.lock:
            ...
            if event.status != "running":
                logger.warning(f"No se puede cancelar el evento {event_id}, estado actual: {event.status}")
                return False
    ...
    async def _execute_event(self, event: ChaosEvent) -> None:
        ...
        try:
            ...
            if not handler:
                raise ValueError(f"No hay handler para el tipo de evento {event.event_type}")
            ...
            event.end_time = datetime.now()
            event.error = str(e)
            logger.error(f"Error en evento de caos {event.event_id}: {e}", exc_info=True)
    ...
    async def _handle_service_failure(self, event: ChaosEvent) -> Dict[str, Any]:
        ...
        from core.degraded_mode import degraded_mode_manager, DegradationReason, DegradationLevel
        ...
        logger.info(f"Servicio {target_service} marcado como no disponible (evento {event.event_id})")
    ...
    async def _handle_network_latency(self, event: ChaosEvent) -> Dict[str, Any]:
        ...
        logger.info(f"Latencia de red aplicada a {target_host}: {latency_ms}ms ±{jitter_ms}ms (evento {event.event_id})")
        ...
        try:
            ...
            logger.info(f"Latencia de red eliminada para {target_host} (evento {event.event_id})")
    ...
    async def _handle_network_partition(self, event: ChaosEvent) -> Dict[str, Any]:
        ...
        logger.info(f"Partición de red aplicada a {target_hosts} con tasa de pérdida {drop_rate} (evento {event.event_id})")
    ...
    async def _handle_memory_pressure(self, event: ChaosEvent) -> Dict[str, Any]:
        ...
        logger.info(f"Aplicando presión de memoria: {target_mb}MB (evento {event.event_id})")
        ...
        logger.info(f"Presión de memoria aplicada: {len(memory_hogs)}MB (evento {event.event_id})")
    ...
    async def _handle_cpu_pressure(self, event: ChaosEvent) -> Dict[str, Any]:
        ...
        logger.info(f"Aplicando presión de CPU: {cpu_usage*100}% en {num_cores} cores (evento {event.event_id})")
    ...
    async def _handle_disk_pressure(self, event: ChaosEvent) -> Dict[str, Any]:
        ...
        logger.info(f"Aplicando presión de disco: {target_mb}MB en {target_path} (evento {event.event_id})")
        ...
        try:
            ...
            logger.info(f"Presión de disco aplicada: {target_mb}MB en {temp_file.name} (evento {event.event_id})")
    ...
    async def _handle_clock_skew(self, event: ChaosEvent) -> Dict[str, Any]:
        ...
        logger.info(f"Desviación del reloj aplicada: {skew_seconds} segundos (evento {event.event_id})")
    ...
    async def _handle_process_kill(self, event: ChaosEvent) -> Dict[str, Any]:
        ...
        if self.safe_mode:
            raise ValueError("No se puede ejecutar el evento de matar proceso en modo seguro")
        ...
        logger.warning(f"¡ATENCIÓN! Enviando señal {signal_type} al proceso actual (evento {event.event_id})")
    ...
    async def _handle_random_exception(self, event: ChaosEvent) -> Dict[str, Any]:
        ...
        message = event.parameters.get("message", f"Excepción simulada por prueba de caos (evento {event.event_id})")
        ...
        # Lanzar excepción
        logger.warning(f"Lanzando excepción {exception_type}: {message} (evento {event.event_id})")
...
chaos_testing_manager = ChaosTestingManager()
    ...
    async def _notify_event_update(self, event: ChaosEvent) -> None:
        """
        Notifica una actualización de un evento.
