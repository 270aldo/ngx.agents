...
class ObjectRecognition:
    ...
    def __init__(self, telemetry: Optional[Telemetry] = None):
        ...
        self.domains = {
            "general": {
                ...
                "objects": [
                    "persona", "coche", "bicicleta", "moto", "avión", "autobús", "tren", "camión", 
                    "barco", "semáforo", "hidrante", "señal de stop", "parquímetro", "banco", 
                    "pájaro", "gato", "perro", "caballo", "oveja", "vaca", "elefante", "oso", 
                    "cebra", "jirafa", "mochila", "paraguas", "bolso", "corbata", "maleta", "frisbee", 
                    "esquís", "tabla de snowboard", "pelota", "cometa", "bate", "guante", "patineta", 
                    "tabla de surf", "raqueta", "botella", "copa", "taza", "tenedor", "cuchillo", 
                    "cuchara", "cuenco", "plátano", "manzana", "sándwich", "naranja", "brócoli", 
                    "zanahoria", "perrito caliente", "pizza", "dona", "pastel", "silla", "sofá", 
                    "planta", "cama", "mesa", "inodoro", "televisión", "portátil", "ratón", "control remoto", 
                    "teclado", "teléfono", "microondas", "horno", "tostadora", "fregadero", "refrigerador", 
            ...
            "medical": {
                ...
                "objects": [
                    "fractura", "tumor", "quiste", "nódulo", "hemorragia", "edema", "inflamación",
                    "calcificación", "estenosis", "aneurisma", "atrofia", "hipertrofia", "metástasis",
                    "neumotórax", "derrame pleural", "consolidación", "enfisema", "fibrosis",
                    "cardiomegalia", "osteoporosis", "artritis", "escoliosis", "hernia discal"
                ]
            },
            "industrial": {
                ...
                "objects": [
                    "grieta", "corrosión", "desgaste", "deformación", "rotura", "soldadura defectuosa",
                    "tornillo suelto", "fuga", "sobrecalentamiento", "desalineación", "vibración excesiva",
                    "contaminación", "obstrucción", "falta de componente", "componente incorrecto",
                    "etiqueta faltante", "código de barras ilegible", "embalaje dañado"
                ]
            },
            "retail": {
                ...
                "objects": [
                    "producto", "estantería", "etiqueta de precio", "código de barras", "caja registradora",
                    "carrito de compra", "cesta", "cliente", "vendedor", "probador", "escaparate",
                    "promoción", "descuento", "oferta", "pago", "recibo", "bolsa", "seguridad", "cámara"
                ]
            },
            "custom": {
    ...
    async def recognize_objects(self, image_data: Union[str, bytes, Dict[str, Any]], 
                              domain: str = "general",
        ...
        if self.telemetry:
            ...
            self.telemetry.add_span_attribute(span, "confidence_threshold", confidence_threshold)
            self.telemetry.add_span_attribute(span, "agent_id", agent_id)
        
        try:
            # Verificar que el dominio existe
            if domain not in self.domains:
                raise ValueError(f"Dominio no válido: {domain}. Dominios disponibles: {', '.join(self.domains.keys())}")
            
            # Procesar la entrada de imagen
            processed_image, image_format, image_size = await self._process_image_input(image_data)
            
            # Generar clave para caché
            cache_key = await image_cache.generate_key(processed_image, {
            ...
            detections = await self._detect_objects(processed_image, domain, confidence_threshold)
            ...
            if self.telemetry:
                ...
                self.telemetry.add_span_attribute(span, "processing_time_ms", latency_ms)
                self.telemetry.record_metric("object_recognition.objects_detected", len(detections), {"domain": domain})
    ...
    async def register_custom_domain(self, domain_name: str, objects: List[str], 
                                   description: str = "", 
                                   agent_id: str = "unknown") -> Dict[str, Any]:
        """
        ...
        if self.telemetry:
            span = self.telemetry.start_span("object_recognition.register_custom_domain")
        ...
        try:
            ...
            if not domain_name or not objects:
                raise ValueError("Se requiere un nombre de dominio y al menos un objeto")
            ...
            async with self.lock:
                self.domains[domain_key] = {
                    "name": domain_name,
                    "description": description or f"Dominio personalizado: {domain_name}",
                    "objects": objects,
                    "custom": True
                }
            ...
            logger.info(f"Dominio personalizado registrado: {domain_name} con {len(objects)} objetos")
            
            if self.telemetry:
                self.telemetry.record_metric("object_recognition.custom_domains_registered", 1)
            ...
            # Registrar error
            logger.error(f"Error al registrar dominio personalizado: {e}", exc_info=True)
    ...
    async def _process_image_input(self, image_data: Union[str, bytes, Dict[str, Any]]) -> Tuple[bytes, str, int]:
        """
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.get(image_data["url"]) as response:
                        if response.status == 200:
                            ...
                            format_detected = os.path.splitext(image_data["url"])[1].lower().replace(".", "")
    ...
    async def _detect_objects(self, image_bytes: bytes, domain: str, confidence_threshold: float) -> List[Dict[str, Any]]:
        """
        ...
        for i in range(num_objects):
            ...
            available_objects = [obj for obj in possible_objects if obj not in used_objects]
