...
class VertexGeminiGenerateInput(BaseModel):
    ...
    model: str = Field(DEFAULT_MODEL, description=f"Modelo a utilizar (disponibles: {', '.join(AVAILABLE_MODELS)})")
...
class VertexGeminiChatInput(BaseModel):
    ...
    model: str = Field(DEFAULT_MODEL, description=f"Modelo a utilizar (disponibles: {', '.join(AVAILABLE_MODELS)})")
    temperature: float = Field(0.7, description="Control de aleatoriedad (0.0-1.0)")
    session_id: Optional[str] = Field(None, description="ID de sesión para continuar una conversación")
