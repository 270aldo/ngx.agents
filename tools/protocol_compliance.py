...
class ProtocolComplianceChecker:
    ...
    async def load_agents(self) -> None:
        ...
        agents_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), self.agents_dir)
                    ...
                    for name, obj in inspect.getmembers(module):
                        if (inspect.isclass(obj) and 
                            ...
                            try:
                                ...
                                logger.info(f"Agente cargado: {agent.agent_id} ({agent.name})")
                            except Exception as e:
                                logger.error(f"Error al instanciar el agente {name}: {e}")
        ...
        try:
            ...
            for field in ADK_REQUIRED_FIELDS:
                if field not in agent_card:
                    results["compliant"] = False
                    results["agent_card_issues"].append(f"Campo requerido '{field}' no está presente en Agent Card")
            
            # Verificar que las capabilities sean una lista
            if "capabilities" in agent_card and not isinstance(agent_card["capabilities"], list):
                results["compliant"] = False
                results["agent_card_issues"].append("El campo 'capabilities' debe ser una lista")
            
            # Verificar que el nombre y la descripción no estén vacíos
            if "name" in agent_card and not agent_card["name"]:
                results["compliant"] = False
                results["agent_card_issues"].append("El campo 'name' no puede estar vacío")
                
            if "description" in agent_card and not agent_card["description"]:
                results["compliant"] = False
                results["agent_card_issues"].append("El campo 'description' no puede estar vacío")
        
        except Exception as e:
            results["compliant"] = False
            results["agent_card_issues"].append(f"Error al obtener Agent Card: {str(e)}")
    ...
    async def check_adk_compliance(self, agent: A2AAgent) -> Dict[str, Any]:
        ...
        if hasattr(agent, "run") and callable(getattr(agent, "run")):
            try:
                ...
                response = await agent.run("Este es un mensaje de prueba para verificar la conformidad con ADK.")
                
                # Verificar que la respuesta sea un diccionario
                if not isinstance(response, dict):
                    results["compliant"] = False
                    results["run_method_issues"].append("La respuesta de run() no es un diccionario")
                
                # Verificar campos requeridos en la respuesta
                for field in RUN_RESPONSE_REQUIRED_FIELDS:
                    if field not in response:
                        results["compliant"] = False
                        results["run_method_issues"].append(f"Campo requerido '{field}' no está presente en la respuesta de run()")
                
                # Verificar que el campo status sea "success" o "error"
                if "status" in response and response["status"] not in ["success", "error"]:
                    results["compliant"] = False
                    results["run_method_issues"].append("El campo 'status' debe ser 'success' o 'error'")
                ...
                if "agent_id" in response and response["agent_id"] != agent.agent_id:
                    results["compliant"] = False
                    results["run_method_issues"].append("El campo 'agent_id' en la respuesta no coincide con el ID del agente")
            
            except Exception as e:
                results["compliant"] = False
                results["run_method_issues"].append(f"Error al ejecutar run(): {str(e)}")
        else:
            results["compliant"] = False
    ...
    async def check_all_agents(self) -> Dict[str, Dict[str, Any]]:
        ...
        for agent_id, agent in self.agents.items():
            ...
            self.compliance_results[agent_id] = {
                ...
                "overall_compliant": a2a_results["compliant"] and adk_results["compliant"],
            ...
            if self.verbose:
                ...
                logger.info(f"  Overall compliant: {a2a_results['compliant'] and adk_results['compliant']}")
                
                if not a2a_results["compliant"]:
                    logger.info(f"  A2A issues:")
                    if a2a_results["missing_methods"]:
                        logger.info(f"    Missing methods: {', '.join(a2a_results['missing_methods'])}")
                    if a2a_results["agent_card_issues"]:
                        logger.info(f"    Agent Card issues: {', '.join(a2a_results['agent_card_issues'])}")
                    if a2a_results["other_issues"]:
                        logger.info(f"    Other issues: {', '.join(a2a_results['other_issues'])}")
                
                if not adk_results["compliant"]:
                    logger.info(f"  ADK issues:")
                    if adk_results["missing_fields"]:
                        logger.info(f"    Missing fields: {', '.join(adk_results['missing_fields'])}")
                    if adk_results["run_method_issues"]:
                        logger.info(f"    Run method issues: {', '.join(adk_results['run_method_issues'])}")
                    if adk_results["other_issues"]:
                        logger.info(f"    Other issues: {', '.join(adk_results['other_issues'])}")
    ...
    def generate_report(self, output_file: Optional[str] = None) -> str:
        ...
        if not self.compliance_results:
            logger.warning("No hay resultados de conformidad disponibles. Ejecute check_all_agents() primero.")
            return "{}"
        
        # Calcular estadísticas
        total_agents = len(self.compliance_results)
        a2a_compliant = sum(1 for result in self.compliance_results.values() if result["a2a_compliant"])
        adk_compliant = sum(1 for result in self.compliance_results.values() if result["adk_compliant"])
        overall_compliant = sum(1 for result in self.compliance_results.values() if result["overall_compliant"])
        ...
        report = {
            "summary": {
                ...
                "a2a_compliance_rate": a2a_compliant / total_agents if total_agents > 0 else 0,
                "adk_compliance_rate": adk_compliant / total_agents if total_agents > 0 else 0,
                "overall_compliance_rate": overall_compliant / total_agents if total_agents > 0 else 0
            },
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="Verificador de conformidad con protocolos A2A y ADK")
    parser.add_argument("--agents-dir", default="agentes", help="Directorio donde se encuentran los agentes")
    parser.add_argument("--output", help="Archivo donde guardar el informe")
    parser.add_argument("--verbose", action="store_true", help="Mostrar información detallada")
    
    args = parser.parse_args()
    
    checker = ProtocolComplianceChecker(agents_dir=args.agents_dir, verbose=args.verbose)
