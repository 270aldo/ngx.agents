...
def clean_obsolete_files():
    ...
    """
    print("Iniciando limpieza de archivos y directorios obsoletos...")
        ...
        if os.path.exists(absolute_path):
            try:
                if os.path.isfile(absolute_path):
                    ...
                    print(f"Archivo eliminado: {absolute_path}")
                elif os.path.isdir(absolute_path):
                    shutil.rmtree(absolute_path)
                    print(f"Directorio eliminado: {absolute_path}")
            except OSError as e:
                print(f"Error eliminando {absolute_path}: {e.strerror}")
        else:
            print(f"Ruta no encontrada, omitiendo: {absolute_path}")
