...
from prototipo.clients.supabase_client import supabase_client
...
class SupabaseQueryInput(BaseModel):
    ...
    filters: Optional[Dict[str, Any]] = Field(None, description="Filtros para la consulta")
...
class SupabaseInsertInput(BaseModel):
    ...
    data: Union[Dict[str, Any], List[Dict[str, Any]]] = Field(..., description="Datos a insertar")
    upsert: bool = Field(False, description="Si es True, actualiza registros existentes")
...
class SupabaseSQLInput(BaseModel):
    ...
    params: Optional[Dict[str, Any]] = Field(None, description="Parámetros para la consulta")
    ...
    async def execute(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for keyword in dangerous_keywords:
            if keyword in sql.upper() and not sql.upper().startswith("SELECT"):
                raise ValueError(f"Consulta SQL potencialmente peligrosa. No se permiten operaciones {keyword}")
        
        # Ejecutar consulta
        results = await supabase_client.execute_sql(
            sql=sql,
            params=params
        )
...
class SupabaseLogActivityOutput(BaseModel):
    ...
    activity_id: Optional[str] = Field(None, description="ID de la actividad registrada")
...
class SupabaseUpdateInput(BaseModel):
    ...
    filters: Dict[str, Any] = Field(..., description="Filtros para identificar registros")
...
class SupabaseDeleteInput(BaseModel):
    ...
    filters: Dict[str, Any] = Field(..., description="Filtros para identificar registros a eliminar")
...
class SupabaseDeleteSkill(Skill):
    ...
    async def execute(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not confirm:
            raise ValueError("Se requiere confirmación para eliminar datos. Establezca 'confirm' en True.")
        
        # Ejecutar eliminación
        results = await supabase_client.delete(
            table_name=table,
            filters=filters
        )
...
class SupabaseHealthCheckInput(BaseModel):
    ...
    detailed: bool = Field(False, description="Si es True, devuelve información detallada")


class SupabaseHealthCheckOutput(BaseModel):
    """Esquema de salida para la skill de verificación de estado de Supabase."""
    status: str = Field(..., description="Estado de la conexión (online, offline, error)")
    ...
    async def execute(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if detailed:
                ...
                stats = supabase_client.get_stats() if hasattr(supabase_client, "get_stats") else {}
