...
class PerplexityResearchInput(BaseModel):
    ...
    focus: Optional[str] = Field(None, description="Enfoque específico de la investigación")
    model: str = Field("llama-3-sonar-small-online", description="Modelo a utilizar")
...
class PerplexityResearchOutput(BaseModel):
    ...
    sources: List[Dict[str, Any]] = Field(default_factory=list, description="Fuentes utilizadas")
    sections: List[str] = Field(default_factory=list, description="Secciones de la investigación")
...
class PerplexityFactCheckOutput(BaseModel):
    ...
    verdict: str = Field(..., description="Veredicto (verdadero, falso, parcialmente verdadero)")
...
class PerplexityFactCheckSkill(Skill):
    ...
    async def execute(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        result = await perplexity_client.fact_check(
            statement=statement,
            model=model
        )
        ...
        explanation = fact_check.get("explanation", "No se pudo verificar la afirmación.")
...
class PerplexityCompareInput(BaseModel):
    ...
    criteria: Optional[List[str]] = Field(None, description="Criterios específicos para la comparación")
    detailed: bool = Field(True, description="Si es True, devuelve una comparación detallada")
    model: str = Field("llama-3-sonar-small-online", description="Modelo a utilizar")
...
class PerplexityCompareOutput(BaseModel):
    ...
    recommendations: List[str] = Field(default_factory=list, description="Recomendaciones basadas en la comparación")
    sources: List[Dict[str, Any]] = Field(default_factory=list, description="Fuentes utilizadas")
...
class PerplexitySummarizeInput(BaseModel):
    ...
    length: str = Field("medium", description="Longitud del resumen (short, medium, long)")
    focus: Optional[str] = Field(None, description="Aspecto específico en el que centrarse")
    model: str = Field("llama-3-sonar-small-online", description="Modelo a utilizar")
...
class PerplexitySummarizeOutput(BaseModel):
    ...
    key_points: List[str] = Field(default_factory=list, description="Puntos clave extraídos")
    sources: List[Dict[str, Any]] = Field(default_factory=list, description="Fuentes utilizadas")
