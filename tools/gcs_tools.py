...
class GCSUploadInput(BaseModel):
    ...
    file_content: Optional[str] = Field(None, description="Contenido del archivo a subir (alternativa a file_path)")
    destination_path: str = Field(..., description="Ruta de destino en GCS (incluyendo nombre de archivo)")
    content_type: Optional[str] = Field(None, description="Tipo MIME del contenido")
    metadata: Optional[Dict[str, str]] = Field(None, description="Metadatos adicionales para el archivo")
...
class GCSDownloadInput(BaseModel):
    ...
    local_path: Optional[str] = Field(None, description="Ruta local donde guardar el archivo (opcional)")
    as_text: bool = Field(False, description="Si es True, devuelve el contenido como texto")


class GCSDownloadOutput(BaseModel):
    """Esquema de salida para la skill de descarga de archivos de GCS."""
    local_path: Optional[str] = Field(None, description="Ruta local donde se guardó el archivo")
    content: Optional[str] = Field(None, description="Contenido del archivo (si as_text=True)")
    size: int = Field(..., description="Tamaño del archivo en bytes")
...
class GCSListFilesInput(BaseModel):
    ...
    delimiter: Optional[str] = Field("/", description="Delimitador para simular carpetas")
...
class GCSListFilesSkill(Skill):
    ...
    async def execute(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Listar archivos
        files = await gcs_client.list_files(
            prefix=prefix,
            delimiter=delimiter
        )
