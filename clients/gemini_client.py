...
class GeminiClient(BaseClient):
    ...
    def __init__(
        ...
        use_cache: bool = True,
        cache_ttl: Optional[int] = 3600 * 24,
        ...
        try:
            ...
            if self.optimize_prompts:
                ...
                if analysis["token_reduction"] > 0:
                    logger.info(
                        f"Prompt optimizado: {analysis['token_reduction']} tokens reducidos ({analysis['percentage_reduction']:.1f}%)"
                    )
            ...
            if not allowed:
                logger.warning(
                    f"Llamada bloqueada por límite de presupuesto para agente {agent_id}"
                )
            ...
            if fallback_model:
                logger.info(
                    f"Cambiando de {self.model_name} a {fallback_model} por restricciones de presupuesto"
                )
    ...
    async def chat(
        ...
        if self.use_cache:
            ...
            if cached_result is not None:
                logger.info(
                    f"Resultado de chat obtenido de caché para agente {agent_id}"
                )
                return cached_result
        ...
        if self.optimize_prompts:
            ...
            if token_reduction > 0:
                ...
                logger.info(
                    f"Mensajes de chat optimizados: {token_reduction} tokens reducidos ({percentage_reduction:.1f}%)"
                )
        ...
        if not allowed:
            logger.warning(
                f"Llamada de chat bloqueada por límite de presupuesto para agente {agent_id}"
            )
            return "Lo siento, no puedo procesar esta solicitud debido a restricciones de presupuesto."
    ...
    async def analyze_intent(self, user_input: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(
                f"Error al parsear la respuesta de análisis de intención: {str(e)}"
            )
    ...
    async def analyze_image(
        self, image_data: Union[str, bytes, BinaryIO], prompt: str
    ) -> str:
        """
    ...
    async def analyze_pdf(self, pdf_path: str, prompt: str, max_pages: int = 5) -> str:
        ...
        try:
            ...
            logger.error(
                "PyPDF2 no está instalado. Por favor, instálalo con: pip install PyPDF2"
            )
            raise
        except Exception as e:
    ...
    async def analyze_csv(
        self, csv_path: str, prompt: str, sample_rows: int = 5
    ) -> str:
        """
        ...
        try:
            ...
            logger.error(
                "Pandas no está instalado. Por favor, instálalo con: pip install pandas"
            )
            raise
        except Exception as e:
    ...
    async def analyze_sentiment(
        ...
        try:
            ...
            logger.error(
                f"Error al parsear resultados de análisis de sentimiento: {str(e)}"
            )
            return {
                "sentiment": "neutral",
                "score": 0.0,
