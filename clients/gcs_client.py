...
class GCSClient(BaseClient):
    ...
    async def initialize(self) -> None:
        ...
        if not self.bucket_name:
            raise ValueError("Nombre del bucket no especificado. Configure GCS_BUCKET en .env")
        ...
        self.bucket = await loop.run_in_executor(None, self.client.bucket, self.bucket_name)
    ...
    async def upload_file(
        ...
        if isinstance(file_path_or_content, str) and os.path.isfile(file_path_or_content):
            ...
            await loop.run_in_executor(None, blob.upload_from_filename, file_path_or_content)
        elif isinstance(file_path_or_content, bytes):
            # Es contenido en bytes
            await loop.run_in_executor(None, blob.upload_from_string, file_path_or_content)
        elif hasattr(file_path_or_content, 'read'):
            # Es un stream de archivo
            await loop.run_in_executor(None, blob.upload_from_file, file_path_or_content)
        else:
            # Es un string (contenido)
            await loop.run_in_executor(None, blob.upload_from_string, str(file_path_or_content))
    ...
    async def download_file(
        ...
        if not exists:
            raise FileNotFoundError(f"El blob {blob_name} no existe en el bucket {self.bucket_name}")
        
        if destination_file_path:
            # Descargar a un archivo
            await loop.run_in_executor(None, blob.download_to_filename, destination_file_path)
    ...
    async def delete_file(self, blob_name: str) -> bool:
        ...
        exists = await loop.run_in_executor(None, blob.exists)
        if not exists:
            logger.warning(f"El blob {blob_name} no existe en el bucket {self.bucket_name}")
            return False
