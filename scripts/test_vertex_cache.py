...
async def run_cache_test(config: Dict[str, Any], iterations: int = 50) -> Dict[str, Any]:
    ...
    # Ejecutar pruebas
    logger.info(f"Iniciando prueba con política {config['policy']}, {iterations} iteraciones")
    ...
    for i in range(len(TEST_PROMPTS)):
        ...
        stats["total_latency_ms"] += client.stats["latency_ms"]["content_generation"][-1]
...
def compare_results(results: List[Dict[str, Any]]) -> None:
    ...
    # Tabla de resultados
    headers = ["Política", "Hit Ratio", "Latencia Prom.", "L1 Hits", "L2 Hits", "Prefetches", "Ahorro Compresión"]
    print(f"{headers[0]:<10} {headers[1]:<10} {headers[2]:<15} {headers[3]:<10} {headers[4]:<10} {headers[5]:<10} {headers[6]:<20}")
    print("-" * 80)
    
    for result in results:
        config = result["config"]
        print(f"{config['policy']:<10} {result['hit_ratio']:.2%} {result['avg_latency_ms']:.2f}ms {result['l1_hits']:<10} {result['l2_hits']:<10} {result['prefetches']:<10} {result['compression_savings_bytes']/1024:.2f} KB")
    ...
    if best_hit_ratio["config"]["policy"] == best_latency["config"]["policy"]:
        print(f"Usar política {best_hit_ratio['config']['policy']} que ofrece el mejor rendimiento general")
    else:
...
async def main():
    ...
    for config in TEST_CONFIGS:
        ...
        logger.info(f"  Ahorro por Compresión: {result['compression_savings_bytes']/1024:.2f} KB")
