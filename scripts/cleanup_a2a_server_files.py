...
REDUNDANT_FILES = ["infrastructure/a2a_server.py", "tests/test_a2a_server.py"]

# Directorios a escanear para referencias
DIRS_TO_SCAN = ["agents", "app", "core", "infrastructure", "tests"]
...
class A2ACleanupTool:
    ...
    def __init__(
        ...
        self.archive_dir = os.path.join(
            self.base_dir, f"{ARCHIVE_DIR}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        )
        self.cleanup_report = {
                        ...
                        with open(file_path, "r", encoding="utf-8") as f:
                            ...
                            if found_references:
                                ...
                                self.cleanup_report["references"]["files"].append(
                                    {"file": rel_path, "references": found_references}
                                )

                                logger.info(
                                    f"Referencias encontradas en {rel_path}: {len(found_references)}"
                                )
    ...
    def _cleanup_files(
        self, redundant_files: List[str], references: Dict[str, List[str]]
    ) -> None:
        """
        ...
        for file_path in redundant_files:
            ...
            for ref_file, ref_list in references.items():
                if any(
                    file_path.replace("/", ".").replace(".py", "") in ref
                    for ref in ref_list
                ):
                    has_references = True
                    self.cleanup_report["files"]["with_references"].append(
                        {"file": file_path, "referenced_by": ref_file}
                    )
                    logger.warning(
                        f"Archivo {file_path} tiene referencias en {ref_file}"
                    )
                    break

            if has_references and not self.force:
                logger.warning(
                    f"No se puede eliminar {file_path} porque tiene referencias"
                )
                continue
    ...
    def _generate_report(self) -> None:
        ...
        self.cleanup_report["summary"] = {
            ...
            "files_with_references": len(
                self.cleanup_report["files"]["with_references"]
            ),
        ...
        print(
            f"  - Archivos redundantes: {self.cleanup_report['summary']['redundant_files']}"
        )
        print(
            f"  - Archivos con referencias: {self.cleanup_report['summary']['files_with_references']}"
        )
        print(
            f"  - Archivos archivados: {self.cleanup_report['summary']['archived_files']}"
        )
        print(
            f"  - Archivos eliminados: {self.cleanup_report['summary']['deleted_files']}"
        )
        print(
            f"  - Archivos con errores: {self.cleanup_report['summary']['files_with_errors']}"
        )
        print(
            f"  - Total de referencias: {self.cleanup_report['summary']['total_references']}"
        )
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Herramienta para limpiar archivos redundantes del servidor A2A"
    )
