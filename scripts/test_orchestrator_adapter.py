...
from infrastructure.adapters.orchestrator_adapter import orchestrator_adapter, initialize_orchestrator_adapter
...
async def test_direct_call():
    ...
    # Datos de prueba
    query = "Necesito un plan de entrenamiento y recuperación para una lesión en la rodilla"
...
async def test_multi_agent_coordination():
    ...
    # Mostrar respuesta
    logger.info(f"Respuesta de coordinación recibida: {json.dumps(response, indent=2, ensure_ascii=False)}")
...
async def test_intent_analysis():
    ...
    # Mostrar resultado
    logger.info(f"Intención analizada: {json.dumps(intent, indent=2, ensure_ascii=False)}")
    
    # Determinar agentes objetivo
    target_agents, priority = await orchestrator_adapter._determine_target_agents(intent, query, context)
...
async def test_emergency_routing():
    ...
    # Mostrar respuesta
    logger.info(f"Respuesta de emergencia recibida: {json.dumps(response, indent=2, ensure_ascii=False)}")
...
async def main():
    ...
    try:
        ...
        if intent_success and direct_success and a2a_success and multi_agent_success and emergency_success:
