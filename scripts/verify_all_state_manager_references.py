...
def generate_report(results: Dict[str, Dict[str, List[str]]]) -> str:
    ...
    files_with_original = sum(1 for file_data in results.values() if file_data["original_references"])
    files_with_optimized = sum(1 for file_data in results.values() if file_data["optimized_references"])
    
    report.append(f"## Estadísticas")
    report.append(f"- Total de archivos con referencias: {total_files}")
    report.append(f"- Archivos con referencias al State Manager original: {files_with_original}")
    report.append(f"- Archivos con referencias al State Manager optimizado: {files_with_optimized}")
    report.append("")
...
def main():
    parser = argparse.ArgumentParser(description='Verificar referencias al State Manager')
