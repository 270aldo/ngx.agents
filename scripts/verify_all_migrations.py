...
VERTEX_ADAPTER_IMPORT_PATTERN = re.compile(r"from\s+infrastructure\.adapters\.vertex_ai_client_adapter\s+import")

# Patrones para State Manager
STATE_ORIGINAL_IMPORT_PATTERN = re.compile(r"from\s+core\.state_manager\s+import")
STATE_OPTIMIZED_IMPORT_PATTERN = re.compile(r"from\s+core\.state_manager_optimized\s+import")
STATE_ADAPTER_IMPORT_PATTERN = re.compile(r"from\s+infrastructure\.adapters\.state_manager_adapter\s+import")
...
def analyze_directory(directory: Path) -> Tuple[List[Dict], List[Dict], int, int, int, int]:
    """
    ...
    for root, _, files in os.walk(directory):
        for file in files:
            ...
            vertex_is_correct = (
                vertex_check["uses_new"] or 
                not (vertex_check["uses_old"] or vertex_check["uses_adapter"])
            )
            ...
            state_is_correct = (
                state_check["uses_adapter"] or 
                state_check["uses_optimized"] or 
                not state_check["uses_original"]
            )
    ...
    return vertex_results, state_results, total_files, vertex_correct, vertex_incorrect, state_correct, state_incorrect

def print_vertex_results(results: List[Dict], total: int, correct: int, incorrect: int, directory_name: str) -> None:
    """
    ...
    print(f"Archivos correctos: {correct} ({(correct/total*100) if total > 0 else 0:.2f}%)")
    print(f"Archivos incorrectos: {incorrect} ({(incorrect/total*100) if total > 0 else 0:.2f}%)")
    ...
    if incorrect > 0:
        ...
        for result in results:
            if not result["is_correct"]:
                ...
                if result["uses_old"]:
                    print("    Recomendación: Reemplazar 'from clients.vertex_ai_client import' por 'from clients.vertex_ai import'")
                elif result["uses_adapter"]:
                    print("    Recomendación: Reemplazar 'from infrastructure.adapters.vertex_ai_client_adapter import' por 'from clients.vertex_ai import'")

def print_state_results(results: List[Dict], total: int, correct: int, incorrect: int, directory_name: str) -> None:
    """
...
def main() -> int:
    ...
    adapter_vertex_results, adapter_state_results, adapter_total, adapter_vertex_correct, adapter_vertex_incorrect, adapter_state_correct, adapter_state_incorrect = analyze_directory(ADAPTERS_DIR)
    
    # Analizar agentes
    agent_vertex_results, agent_state_results, agent_total, agent_vertex_correct, agent_vertex_incorrect, agent_state_correct, agent_state_incorrect = analyze_directory(AGENTS_DIR)
    
    # Imprimir resultados de Vertex AI
    print_vertex_results(adapter_vertex_results, adapter_total, adapter_vertex_correct, adapter_vertex_incorrect, "Adaptadores")
    print_vertex_results(agent_vertex_results, agent_total, agent_vertex_correct, agent_vertex_incorrect, "Agentes")
    ...
    print("\n=== Resultados Totales de Vertex AI ===")
    print(f"Total de archivos analizados: {total}")
    print(f"Archivos correctos: {vertex_correct} ({(vertex_correct/total*100) if total > 0 else 0:.2f}%)")
    print(f"Archivos incorrectos: {vertex_incorrect} ({(vertex_incorrect/total*100) if total > 0 else 0:.2f}%)")
    
    # Imprimir resultados de State Manager
    print_state_results(adapter_state_results, adapter_total, adapter_state_correct, adapter_state_incorrect, "Adaptadores")
    print_state_results(agent_state_results, agent_total, agent_state_correct, agent_state_incorrect, "Agentes")
    ...
    print("\n=== Resultados Totales de State Manager ===")
    print(f"Total de archivos analizados: {total}")
    print(f"Archivos correctos: {state_correct} ({(state_correct/total*100) if total > 0 else 0:.2f}%)")
    print(f"Archivos incorrectos: {state_incorrect} ({(state_incorrect/total*100) if total > 0 else 0:.2f}%)")
