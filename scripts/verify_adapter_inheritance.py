...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='Verifica qué adaptadores necesitan ser actualizados.')
    parser.add_argument('--fix', action='store_true', help='Intenta corregir automáticamente los adaptadores')
...
def find_adapter_files(directory: str = 'infrastructure/adapters') -> List[str]:
    ...
    for root, _, files in os.walk(directory):
        for file in files:
            if (file.endswith('_adapter.py') and
                file != 'base_agent_adapter.py' and
                file not in INFRASTRUCTURE_ADAPTERS):
...
def check_adapter_inheritance(file_path: str) -> Tuple[bool, str, str]:
    ...
    has_import = bool(re.search(r'from\s+infrastructure\.adapters\.base_agent_adapter\s+import\s+BaseAgentAdapter', content))
    ...
    inherits_from_base = second_parent == 'BaseAgentAdapter' or agent_class == 'BaseAgentAdapter'
...
def analyze_adapters() -> Dict[str, Dict]:
    ...
    for file_path in adapter_files:
        inherits_from_base, adapter_class, agent_class = check_adapter_inheritance(file_path)
        results[file_path] = {
        ...
        if updated > 0 and verbose:
            ...
            for file_path, info in results.items():
                if info['inherits_from_base']:
                    print(f"  ✓ {info['adapter_class']} ({os.path.basename(file_path)})")
...
def main():
    ...
    if args.fix:
        ...
        print("Por favor, actualiza manualmente los adaptadores de agentes siguiendo el ejemplo de ClientSuccessLiaisonAdapter.")
    ...
    for adapter in INFRASTRUCTURE_ADAPTERS:
        print(f"  - {adapter}")
    print("Estos adaptadores no necesitan heredar de BaseAgentAdapter porque son componentes de infraestructura, no agentes.")
