...
class LoadTestResult:
    ...
    def add_result(self, latency: float, success: bool, error: Optional[str] = None, tokens: int = 0, cache_hit: bool = False):
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "error_rate": (self.requests_sent - self.requests_successful) / self.requests_sent if self.requests_sent > 0 else 0,
            ...
            "latency_ms": {
                ...
                "p95": statistics.quantiles(self.latencies, n=20)[19] if len(self.latencies) >= 20 else max(self.latencies) if self.latencies else 0,
                "p99": statistics.quantiles(self.latencies, n=100)[99] if len(self.latencies) >= 100 else max(self.latencies) if self.latencies else 0
            },
            "cache": {
            ...
            "errors": self.errors[:10]  # Limitar a 10 errores para no sobrecargar el resumen
        }
    ...
    def save_to_csv(self, filename: str):
        ...
        try:
            with open(filename, 'w', newline='') as csvfile:
                fieldnames = ['timestamp', 'latency_ms', 'success', 'tokens', 'cache_hit']
...
async def generate_load(
    ...
    logger.info(f"Iniciando prueba de carga: {scenario.get('description', 'Personalizada')}")
    logger.info(f"Duración: {duration} segundos, Total de solicitudes estimadas: {total_requests}")
    ...
    for i in range(total_requests):
        ...
        if distribution == "constant":
            ...
            cycle_position = (i / scenario.get("requests_per_second", 10)) % cycle_seconds
            ...
            if cycle_progress < 0.5:
                ...
                # Bajando
                current_rps = max_rps - (max_rps - min_rps) * ((cycle_progress - 0.5) * 2)
            delay = i / current_rps
        else:
...
async def send_request_with_delay(
    ...
    try:
        ...
        if "error" in response:
            ...
            cache_hit = latency < 100  # Asumimos que respuestas muy rápidas son de caché
...
async def main():
    parser = argparse.ArgumentParser(description="Realiza pruebas de carga en el cliente Vertex AI")
    parser.add_argument("--scenario", type=str, choices=list(SCENARIOS.keys()), default="normal",
                        help="Escenario de prueba predefinido")
    parser.add_argument("--custom", type=str, help="Ruta a un archivo JSON con escenario personalizado")
    parser.add_argument("--prompts", type=str, help="Ruta a un archivo de texto con prompts personalizados (uno por línea)")
    ...
    if args.prompts and os.path.exists(args.prompts):
        try:
            ...
            logger.info(f"Cargados {len(custom_prompts)} prompts personalizados desde {args.prompts}")
        except Exception as e:
    ...
    results_file = os.path.join(output_dir, f"vertex_ai_load_test_{scenario_name}_{timestamp}.csv")
    summary_file = os.path.join(output_dir, f"vertex_ai_load_test_{scenario_name}_{timestamp}_summary.json")
