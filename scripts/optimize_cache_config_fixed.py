...
class CacheOptimizer:
    ...
    def print_results(self):
        ...
        if sorted_results and "error" not in sorted_results[0]:
            ...
            print("\nNo se encontró una configuración óptima. Revise los errores.")
            for result in self.results:
                if "error" in result:
                    print(f"Error en {result['config']['policy']}: {result['error']}")
    ...
    def plot_results(self):
        ...
        valid_results = [r for r in self.results if "error" not in r or r.get("hit_ratio", 0) > 0]
        
        if not valid_results:
            logger.warning("No hay resultados válidos para generar gráficos")
            return
            
        # Preparar datos
        policies = [r["config"]["policy"] for r in valid_results]
        hit_ratios = [r.get("hit_ratio", 0) * 100 for r in valid_results]
        latencies = [r.get("avg_latency_ms", 0) for r in valid_results]
        throughputs = [r.get("requests_per_second", 0) for r in valid_results]
    ...
    def export_results(self, filename="cache_optimization_results.json"):
        ...
        # Preparar resultados para exportación (eliminar latencies que pueden ser muy grandes)
        export_results = []
        for result in self.results:
            export_result = result.copy()
            if "latencies" in export_result:
                # Solo guardar estadísticas de latencias, no todos los valores
                latencies = export_result.pop("latencies")
                if latencies:
                    export_result["latency_stats"] = {
                        "min": min(latencies),
                        "max": max(latencies),
