...
ADAPTER_IMPORT_PATTERN = re.compile(r"from\s+infrastructure\.adapters\.state_manager_adapter\s+import")
...
def analyze_directory(directory: Path) -> Tuple[List[Dict], int, int, int]:
    ...
    for root, _, files in os.walk(directory):
        for file in files:
            ...
            is_correct = (
                check_result["uses_adapter"] or 
                check_result["uses_optimized"] or 
                not check_result["uses_original"]
            )
...
def print_results(results: List[Dict], total: int, correct: int, incorrect: int, directory_name: str) -> None:
    """
...
def main() -> int:
    ...
    adapter_results, adapter_total, adapter_correct, adapter_incorrect = analyze_directory(ADAPTERS_DIR)
    print_results(adapter_results, adapter_total, adapter_correct, adapter_incorrect, "Adaptadores")
    
    # Analizar agentes
    agent_results, agent_total, agent_correct, agent_incorrect = analyze_directory(AGENTS_DIR)
