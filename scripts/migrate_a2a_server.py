...
class A2AMigrationTool:
    ...
    async def _test_message_sending(self) -> None:
        ...
        try:
            ...
            registered_agents = a2a_adapter.get_registered_agents()
            assert sender_id in registered_agents, f"El agente emisor {sender_id} no está registrado"
            assert receiver_id in registered_agents, f"El agente receptor {receiver_id} no está registrado"
    ...
    async def _test_agent_calling(self) -> None:
        ...
        try:
            ...
            registered_agents = a2a_adapter.get_registered_agents()
            assert agent_id in registered_agents, f"El agente {agent_id} no está registrado"
            ...
            assert hasattr(a2a_adapter, 'call_agent'), "El adaptador no tiene el método call_agent"
            assert hasattr(a2a_adapter, 'call_multiple_agents'), "El adaptador no tiene el método call_multiple_agents"
...
async def main():
    """Función principal."""
    parser = argparse.ArgumentParser(description="Herramienta para migrar el servidor A2A")
