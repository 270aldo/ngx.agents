...
def check_file_for_method(file_path: str) -> Tuple[bool, bool]:
    ...
    imports_base_adapter = re.search(r"from\s+infrastructure\.adapters\.base_agent_adapter\s+import\s+BaseAgentAdapter", content) is not None
    
    # Verificar si implementa _get_program_type_from_profile
    implements_method = re.search(r"async\s+def\s+_get_program_type_from_profile\s*\(", content) is not None
...
def update_file(file_path: str, dry_run: bool = False) -> bool:
    ...
    method_pattern = re.compile(r"(\s+async\s+def\s+_get_program_type_from_profile\s*\([^)]*\)\s*->\s*str\s*:.*?)(\s+async\s+def|\s*$)", re.DOTALL)
    match = method_pattern.search(content)
    
    if not match:
        print(f"No se encontró implementación de _get_program_type_from_profile en {file_path}")
        return False
    ...
    if returns_general:
        ...
        if dry_run:
            print(f"[DRY RUN] Se eliminaría _get_program_type_from_profile de {file_path}")
            return True
        ...
        print(f"La implementación de _get_program_type_from_profile en {file_path} no devuelve 'general'. Se mantiene.")
        return False


def main():
    """Función principal del script."""
    parser = argparse.ArgumentParser(description="Actualiza las llamadas a _get_program_type_from_profile en los adaptadores")
    parser.add_argument("--directory", "-d", default="infrastructure/adapters", help="Directorio donde buscar los adaptadores")
    parser.add_argument("--dry-run", action="store_true", help="No realizar cambios, solo mostrar qué se haría")
    ...
    for file_path in adapter_files:
        ...
        if imports_base and implements_method:
            ...
            print(f"ADVERTENCIA: {file_path} implementa _get_program_type_from_profile pero no importa BaseAgentAdapter")
