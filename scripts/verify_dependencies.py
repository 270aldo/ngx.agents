...
def parse_pyproject_dependencies() -> Dict[str, str]:
    ...
    main_deps_match = re.search(r"\[tool\.poetry\.dependencies\](.*?)(?:\[tool\.poetry\.group|$)", content, re.DOTALL)
    if main_deps_match:
        ...
        for line in main_deps.strip().split("\n"):
            ...
            if extras_match:
                ...
                # Extraer versión
                version_match = re.search(r'version\s*=\s*["\'](.*?)["\']\'', extras_content)
                if version_match:
    ...
    group_matches = re.finditer(r"\[tool\.poetry\.group\.(.*?)\.dependencies\](.*?)(?:\[|\Z)", content, re.DOTALL)
...
def check_package_installed(package: str, required_version: str, installed_packages: Dict[str, str]) -> Tuple[bool, Optional[str]]:
    """
...
def main() -> int:
    ...
    for package, version in dependencies.items():
        ...
        # Verificar si está instalado
        is_installed, installed_version = check_package_installed(package_name, version, installed_packages)
    ...
    for package in installed_packages_info:
        optional_str = " (opcional)" if package["is_optional"] else ""
        print(f"  - {package['package']}: {package['installed']} (requerido: {package['required']}) {optional_str}")
