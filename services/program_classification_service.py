...
class ProgramClassificationService:
    ...
    def __init__(self, gemini_client: Optional[GeminiClient] = None, use_cache: bool = True):
        """
        ...
        for program_type in get_all_program_types():
            self._program_definitions_cache[program_type] = get_program_definition(program_type)
    ...
    async def classify_program_type(self, context: Dict[str, Any], use_llm: bool = True) -> str:
        """
        ...
        if program_type and isinstance(program_type, str):
            ...
            if standardized_type in valid_types:
                self.logger.info(f"Tipo de programa encontrado explícitamente en contexto: {standardized_type}")
        ...
        if use_llm and self.gemini_client:
            try:
                ...
                if llm_program_type:
                    self.logger.info(f"Tipo de programa determinado por LLM: {llm_program_type}")
                ...
                self.logger.error(f"Error al clasificar tipo de programa con LLM: {e}", exc_info=True)
                # Continuar con enfoque basado en reglas si falla LLM
        elif use_llm and not self.gemini_client:
            self.logger.warning("GeminiClient no disponible para clasificación avanzada de tipo de programa")
    ...
    def enrich_query_with_program_context(self, query: str, program_type: str) -> str:
        ...
        if not program_type or program_type not in self._program_definitions_cache:
            self.logger.warning(f"Tipo de programa no reconocido para enriquecer consulta: {program_type}")
            return query
        ...
        if program_def.get("key_protocols"):
            ...
            elif isinstance(program_def["key_protocols"], list) and len(program_def["key_protocols"]) > 0:
        ...
        has_instructions = re.search(r'(considera|ten en cuenta|basado en|enfocado en|para el programa)', query.lower())
        ...
        self.logger.debug(f"Consulta enriquecida con información del programa {program_type}")
    ...
    async def _classify_with_llm(self, context: Dict[str, Any]) -> Optional[str]:
        ...
        """
            ...
            if age_range:
                ...
                if min_age is not None and max_age is not None:
                    prompt += f"           - Rango de edad típico: {min_age}-{max_age} años\n"
        ...
        try:
            ...
            if hasattr(self.gemini_client, 'generate_structured_output'):
                self.logger.debug("Usando generate_structured_output para clasificar tipo de programa")
                classification = await self.gemini_client.generate_structured_output(prompt)
            else:
                # Fallback a generate_text con temperatura baja para mayor consistencia
                self.logger.debug("Usando generate_text para clasificar tipo de programa")
                response = await self.gemini_client.generate_text(prompt, temperature=0.1)
            ...
            if program_type in valid_types:
                ...
                self.logger.warning(f"LLM devolvió un tipo de programa no reconocido: {program_type}")
                return None
    ...
    def _classify_with_rules(self, context: Dict[str, Any]) -> str:
        ...
        combined_text = f"{profile_description} {' '.join(profile_tags)} {profile_role} {goals_str}"
        ...
        if is_prime_profile:
            ...
            if age is None or (isinstance(age, (int, float)) and 30 <= age <= 55):
                logger.info("Tipo de programa determinado como PRIME basado en perfil/objetivos y/o edad.")
                return "PRIME"
            else:
                logger.info(f"Palabras clave PRIME encontradas, pero edad ({age}) fuera de rango (30-55). Verificando otros tipos.")
        
        # Verificar LONGEVITY (priorizar edad si > 55)
        if age and isinstance(age, (int, float)) and age > 55:
            logger.info(f"Tipo de programa determinado como LONGEVITY basado en edad ({age}).")
            return "LONGEVITY"
        ...
        if goals:
            for program_type in ["STRENGTH", "HYPERTROPHY", "ENDURANCE", "ATHLETIC"]:
                ...
                if any(kw.lower() in goals_str for kw in keywords):
                    logger.info(f"Tipo de programa determinado como {program_type} basado en objetivos.")
                    return program_type
        
        # Valor por defecto
        logger.info("No se pudo determinar un tipo de programa específico, usando PERFORMANCE por defecto.")
        return "PERFORMANCE"
    
    async def get_program_specific_recommendations(self, 
                                           program_type: str, 
        ...
        try:
            ...
            if self.use_cache:
                ...
                if cached_result:
                    self.cache_stats['hits'] += 1
                    self.logger.debug(f"Recomendaciones obtenidas de caché para {program_type}/{category}")
                    return cached_result
                else:
                    self.cache_stats['misses'] += 1
            ...
            logger.warning(f"No se encontraron recomendaciones para {program_type}/{category}")
            return []
    ...
    async def flush_cache(self) -> bool:
        ...
        try:
            ...
            self.logger.info("Caché del servicio de clasificación de programas limpiada correctamente")
            return True
        except Exception as e:
