...
from core.degraded_mode import degraded_mode_manager, DegradationLevel, DegradationReason
...
class ServiceStatusResponse(BaseModel):
    ...
    dependencies: List[str] = Field(..., description="IDs de servicios de los que depende")
    ...
    degradation_reason: Optional[str] = Field(default=None, description="Razón de la degradación")
    last_status_change: str = Field(..., description="Fecha del último cambio de estado")
...
class SystemStatusResponse(BaseModel):
    ...
    degradation_level: int = Field(..., description="Nivel de degradación")
    degradation_level_name: str = Field(..., description="Nombre del nivel de degradación")
    degradation_reason: Optional[str] = Field(default=None, description="Razón de la degradación")
    degradation_start: Optional[str] = Field(default=None, description="Fecha de inicio de la degradación")
    degradation_message: Optional[str] = Field(default=None, description="Mensaje de degradación")
    degradation_duration: float = Field(..., description="Duración de la degradación en segundos")
    ...
    services_unavailable: int = Field(..., description="Número de servicios no disponibles")
    critical_services_total: int = Field(..., description="Número total de servicios críticos")
    critical_services_available: int = Field(..., description="Número de servicios críticos disponibles")
    critical_services_unavailable: int = Field(..., description="Número de servicios críticos no disponibles")
...
class ServiceRegisterRequest(BaseModel):
    ...
    dependencies: List[str] = Field(default=[], description="IDs de servicios de los que depende")
...
async def unregister_service(
    ...
    return {
        "success": True,
        "message": f"Servicio {service_id} eliminado"
    }
...
async def update_service_status(
    ...
    if request.is_available:
        ...
        reason = DegradationReason(request.reason) if request.reason else DegradationReason.MANUAL
        level = DegradationLevel(request.level) if request.level is not None else DegradationLevel.MEDIUM
...
async def get_feature_status(
    feature_id: str = Path(..., description="ID de la funcionalidad"),
    default: bool = Query(True, description="Valor por defecto si la funcionalidad no está registrada"),
    user_id: str = Depends(get_current_user)
):
    """
...
async def start_monitoring(
    ...
    return {
        "success": True,
        "message": "Monitorización de servicios iniciada"
    }
...
async def stop_monitoring(
    ...
    return {
        "success": True,
        "message": "Monitorización de servicios detenida"
    }