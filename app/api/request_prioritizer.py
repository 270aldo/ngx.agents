...
from core.request_prioritizer import request_prioritizer, SLATier, RequestStatus
...
class RequestSubmitRequest(BaseModel):
    ...
    sla_tier: Optional[str] = Field(default=None, description="Nivel de SLA (platinum, gold, silver, bronze, free)")
...
class RequestResultResponse(BaseModel):
    ...
    wait_time: float = Field(..., description="Tiempo de espera en segundos")
    processing_time: float = Field(..., description="Tiempo de procesamiento en segundos")
    total_time: Optional[float] = Field(default=None, description="Tiempo total en segundos")
...
class PrioritizerStatsResponse(BaseModel):
    ...
    completed_requests: int = Field(..., description="Número de solicitudes completadas")
    ...
    max_processing_time: float = Field(..., description="Tiempo máximo de procesamiento")
    queue_size: int = Field(..., description="Tamaño de la cola")
    status_counts: Dict[str, int] = Field(..., description="Número de solicitudes por estado")
    sla_counts: Dict[str, int] = Field(..., description="Número de solicitudes por nivel de SLA")
    agent_counts: Dict[str, int] = Field(..., description="Número de solicitudes por agente")
...
class SLAConfigResponse(BaseModel):
    ...
    priority_boost: int = Field(..., description="Incremento de prioridad por segundo de espera")
...
async def submit_request(
    ...
    if request.handler_name not in AVAILABLE_HANDLERS:
        raise HTTPException(status_code=400, detail=f"Handler '{request.handler_name}' no disponible")
    ...
    try:
        ...
        if request.sla_tier:
            try:
                ...
                raise HTTPException(status_code=400, detail=f"Nivel de SLA '{request.sla_tier}' no válido")
        ...
        raise HTTPException(status_code=500, detail=f"Error al enviar solicitud: {str(e)}")
...
async def get_request_result(
    ...
    try:
        result = await request_prioritizer.get_request_result(request_id, wait=wait, timeout=timeout)
...
async def get_prioritizer_stats(
    user_id: str = Depends(get_current_user)
):
    """
...
async def clear_completed_requests(
    older_than: Optional[int] = Query(None, description="Eliminar solicitudes completadas hace más de X segundos"),
    user_id: str = Depends(get_current_user)
):
    """
