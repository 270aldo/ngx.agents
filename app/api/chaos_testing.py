...
class ChaosEventRequest(BaseModel):
    ...
    delay: int = Field(default=0, description="Retraso antes de iniciar el evento en segundos")
    description: Optional[str] = Field(default=None, description="Descripción del evento")
    parameters: Optional[Dict[str, Any]] = Field(default=None, description="Parámetros adicionales para el evento")
...
class ChaosEventResponse(BaseModel):
    ...
    delay: int = Field(..., description="Retraso antes de iniciar el evento en segundos")
    description: str = Field(..., description="Descripción del evento")
    parameters: Dict[str, Any] = Field(..., description="Parámetros adicionales para el evento")
...
class ChaosEnableRequest(BaseModel):
    ...
    safe_mode: bool = Field(default=True, description="Si se debe ejecutar en modo seguro (sin eventos destructivos)")

@router.post("/enable", response_model=Dict[str, Any])
async def enable_chaos_testing(
    request: ChaosEnableRequest,
...
async def disable_chaos_testing(
    user_id: str = Depends(get_current_user)
):
    """
    ...
    return {
        "success": True,
        "message": "Pruebas de caos deshabilitadas"
    }
...
async def register_chaos_event(
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail=f"Tipo de evento no válido: {request.event_type}")
...
async def start_chaos_event(
    ...
    if not success:
        raise HTTPException(status_code=400, detail=f"No se pudo iniciar el evento {event_id}")
...
async def cancel_chaos_event(
    ...
    if not success:
        raise HTTPException(status_code=400, detail=f"No se pudo cancelar el evento {event_id}")
...
async def get_all_chaos_events(
    user_id: str = Depends(get_current_user)
):
    """
...
async def get_chaos_event_types(
    user_id: str = Depends(get_current_user)
):
    """
