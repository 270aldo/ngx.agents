...
class CacheStats(BaseModel):
    ...
    strategies: Dict[str, int] = Field(..., description="Número de entradas por estrategia")
...
class DomainRuleRequest(BaseModel):
    ...
    similarity_threshold: float = Field(default=0.9, description="Umbral de similitud para búsquedas semánticas")
    default_ttl: Optional[int] = Field(default=None, description="TTL por defecto para este dominio")
...
class CacheEntryRequest(BaseModel):
    ...
    strategy: CacheStrategy = Field(default=CacheStrategy.EXACT_MATCH, description="Estrategia de caché")
    params: Optional[Dict[str, Any]] = Field(default=None, description="Parámetros adicionales")
...
async def clear_cache(
    domain: Optional[str] = Query(default=None, description="Dominio a limpiar (None = todos)"),
    user_id: str = Depends(get_current_user)
):
    """
    Limpia el caché.
...
async def register_domain_rule(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error al registrar regla: {str(e)}")

@router.post("/set", response_model=Dict[str, Any])
async def set_cache_entry(
    entry: CacheEntryRequest,
    user_id: str = Depends(get_current_user)
):
    """
...
async def get_cache_entry(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error al consultar caché: {str(e)}")