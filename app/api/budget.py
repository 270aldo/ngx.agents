...
from core.budget import budget_manager, AgentBudget, TokenUsage, BudgetPeriod, BudgetAction
...
async def get_all_budgets():
    ...
    if not settings.enable_budgets:
        raise HTTPException(status_code=400, detail="Sistema de presupuestos deshabilitado")
...
async def get_agent_budget(agent_id: str = Path(..., description="ID del agente")):
    ...
    if status.get("status") == "no_budget":
        raise HTTPException(status_code=404, detail=f"No se encontró presupuesto para el agente {agent_id}")
...
async def update_agent_budget(
    ...
    if update_data.period is not None:
        try:
            ...
            raise HTTPException(status_code=400, detail=f"Período inválido: {update_data.period}")
    
    if update_data.action_on_limit is not None:
        try:
            budget.action_on_limit = update_data.action_on_limit
        except ValueError:
            raise HTTPException(status_code=400, detail=f"Acción inválida: {update_data.action_on_limit}")
    ...
    if update_data.reset_day is not None:
        if update_data.reset_day < 1 or update_data.reset_day > 31:
            raise HTTPException(status_code=400, detail="El día de reset debe estar entre 1 y 31")
...
async def create_agent_budget(
    ...
    if existing_budget:
        raise HTTPException(status_code=400, detail=f"Ya existe un presupuesto para el agente {agent_id}")
