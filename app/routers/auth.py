...
async def login_for_access_token(
    ...
    try:
        ...
        if not session or not session.session or not session.session.access_token:
            ...
            logger.error(f"Respuesta inesperada de Supabase al intentar login para: {form_data.username}")
            raise HTTPException(
...
async def register_user(
    ...
    try:
        ...
        if not response or not response.user:
            ...
            logger.error(f"Respuesta inesperada de Supabase durante el registro para: {user_data.email}")
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
        ...
        logger.info(f"Usuario registrado exitosamente: {user_data.email}, ID: {response.user.id}")
        ...
        return User.model_validate(response.user) # Usar model_validate para Pydantic v2
        ...
        raise HTTPException(
            ...
            detail=e.message or "Error durante el registro"
        )
    except Exception as e:
        logger.error(f"Error inesperado durante el registro para {user_data.email}: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Error en el servidor"
        )
