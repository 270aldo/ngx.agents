...
async def process_agent_request(
    ...
    try:
        ...
        logger.warning(f"A2A Router: Agente {agent_path_id} no encontrado. Detalle: {e.detail}")
        raise e
    except Exception as e:
        logger.error(f"A2A Router: Error inesperado al obtener el agente {agent_path_id}: {e}", exc_info=True)
        raise HTTPException(
    ...
    try:
        ...
        logger.debug(f"A2A Router: Resultado del agente {agent_path_id}: {agent_result}")
        ...
        if agent_result.get("status") == "success":
            ...
            response_content = agent_result.get("response", f"Error al procesar la solicitud por el agente {agent_path_id}.")
            logger.error(f"A2A Router: Agente {agent_path_id} devolvió un error: {error_detail}. Respuesta: {response_content}")
        ...
        logger.error(f"A2A Router: Error inesperado al ejecutar el agente {agent_path_id}: {e}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
