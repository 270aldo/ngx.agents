...
from infrastructure.adapters.a2a_adapter import A2AAdapter, a2a_adapter, get_a2a_server, get_a2a_server_status
...
async def test_register_agent(adapter):
    ...
    with patch('infrastructure.a2a_optimized.a2a_server.register_agent', new_callable=AsyncMock) as mock_register:
        # Datos de prueba
        agent_id = "test_agent"
        agent_info = {
            "name": "Test Agent",
...
async def test_unregister_agent(adapter):
    ...
    with patch('infrastructure.a2a_optimized.a2a_server.unregister_agent', new_callable=AsyncMock) as mock_unregister:
...
async def test_send_message(adapter):
    ...
    with patch('infrastructure.a2a_optimized.a2a_server.send_message', new_callable=AsyncMock) as mock_send:
        # Configurar mock
...
async def test_call_agent_success(adapter):
    ...
    # Mocks
    with patch.object(adapter, 'register_agent') as mock_register, \
...
async def test_call_agent_send_failure(adapter):
    ...
    # Mocks
    with patch.object(adapter, 'register_agent'), \
         patch.object(adapter, 'unregister_agent') as mock_unregister, \
         patch.object(adapter, 'send_message', new_callable=AsyncMock) as mock_send:
...
async def test_get_a2a_server_status():
    ...
    with patch('infrastructure.a2a_optimized.a2a_server.get_stats', new_callable=AsyncMock) as mock_get_stats:
        # Configurar mock
