...
@pytest.mark.asyncio
async def test_get_state_field(state_manager: StateManager, test_settings: Dict[str, Any]):
    ...
    theme = await state_manager.get_state_field(user_id, session_id, "user.preferences.theme")
    count = await state_manager.get_state_field(user_id, session_id, "conversation.count")
    messages = await state_manager.get_state_field(user_id, session_id, "conversation.messages")
    ...
    # Campo que no existe
    non_existent = await state_manager.get_state_field(user_id, session_id, "user.age")
    assert non_existent is None


@pytest.mark.asyncio
async def test_update_state_field(state_manager: StateManager, test_settings: Dict[str, Any]):
    ...
    await state_manager.update_state_field(user_id, session_id, "user.preferences.theme", "dark")
    await state_manager.update_state_field(user_id, session_id, "conversation.messages", ["New message"])
    await state_manager.update_state_field(user_id, session_id, "conversation.count", 1)
    ...
    await state_manager.update_state_field(user_id, session_id, "user.age", 30)
    loaded_state = await state_manager.load_state(user_id, session_id)
    assert loaded_state["user"]["age"] == 30


@pytest.mark.asyncio
async def test_list_user_sessions(state_manager: StateManager, test_settings: Dict[str, Any]):
    ...
    # Guardar estados
    await state_manager.save_state({"data": "session1"}, user_id, session_id1)
    await state_manager.save_state({"data": "session2"}, user_id, session_id2)
    
    # Listar sesiones
    sessions = await state_manager.list_user_sessions(user_id)
