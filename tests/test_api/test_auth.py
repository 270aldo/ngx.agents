...
def test_client(supabase_client_mock: MagicMock) -> TestClient:
    ...
    async def mock_initialize():
        logger.debug(
            "DEBUG [test_client fixture]: Mock initialize called, doing nothing"
        )
    ...
    logger.debug(
        f"DEBUG [test_client fixture]: Patched supabase_client.client with mock, overrode initialize method, and overrode SupabaseClient dependency"
    )
...
async def async_client(supabase_client_mock: MagicMock) -> AsyncClient:
    ...
    async def mock_initialize():
        logger.debug(
            "DEBUG [async_client fixture]: Mock initialize called, doing nothing"
        )
        # No hacemos nada, ya que el cliente ya está mockeado
        return
    ...
    logger.debug(
        f"DEBUG [async_client fixture]: Patched supabase_client.client with mock, overrode initialize method, and overrode SupabaseClient dependency"
    )
...
def test_settings():
    ...
    if not settings.test_user_email or not settings.valid_test_token:
        logger.debug(
            f"DEBUG [test_settings fixture]: Valores de prueba no encontrados en settings: test_user_email={settings.test_user_email}, valid_test_token={settings.valid_test_token}"
        )
    ...
    logger.debug(
        f"DEBUG [test_settings fixture]: Usando valores de settings: test_user_email={settings.test_user_email}"
    )
    return {
...
def supabase_client_mock(mocker, test_settings: dict) -> MagicMock:
    ...
    async def mock_sign_in_with_password(credentials):
        logger.debug(
            f"DEBUG: supabase_client_mock: mock_sign_in_with_password llamado con credentials: {credentials}"
        )
    ...
    async def mock_sign_up(credentials: dict):
        logger.debug(
            f"DEBUG: supabase_client_mock: mock_sign_up llamado con credentials: {credentials}"
        )
        email = credentials.get("email")
    ...
    async def mock_get_user(jwt: str):
        logger.debug(
            f"DEBUG: supabase_client_mock: mock_get_user llamado con jwt: {jwt}"
        )
    ...
    logger.debug(
        f"DEBUG [supabase_client_mock fixture]: Created mock with auth attribute"
    )
...
def test_login_success(test_client: TestClient, test_settings: dict):
    ...
    login_data = {
        "username": test_settings[
            "test_user_email"
        ],  # FastAPI OAuth2PasswordRequestForm usa 'username'
...
def test_login_failure_invalid_credentials(
    test_client: TestClient, test_settings: dict
):
...
def test_register_success(test_client: TestClient):
    ...
    assert (
        json_response["id"] == "11111111-1111-1111-1111-111111111111"
    )  # Del mock_sign_up
    assert (
        "hashed_password" not in json_response
...
def test_register_failure_user_exists(test_client: TestClient, test_settings: dict):
    ...
    register_data = {
        "email": test_settings[
            "test_user_email"
        ],  # Usuario que ya existe según el mock
    ...
    )  # Esperamos 409 por AuthException("User already registered") que se convierte a HTTP_409_CONFLICT
    json_response = response.json()
    assert "detail" in json_response
