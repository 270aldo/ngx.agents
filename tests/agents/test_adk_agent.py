...
class TestADKAgentIntegration:
    ...
    @pytest.mark.asyncio
    async def test_adk_agent_initialization(self, mock_gemini_client, mock_supabase_client, mock_state_manager, test_skill):
        ...
        try:
            ...
            processed_google_adk_tools, processed_a2a_skills_for_card, skill_names = agent._initialize_and_prepare_skills()
            ...
            logger.info("Agente ADK inicializado correctamente con integración de Google ADK")
    ...
    @pytest.mark.asyncio
    async def test_adk_agent_run(self, mock_gemini_client, mock_supabase_client, mock_state_manager, test_skill):
        ...
        try:
            ...
            with patch.object(GoogleADKAgent, 'run', return_value={"response": "Respuesta de prueba"}):
                # Ejecutar el agente
            ...
            logger.info("Agente ADK ejecutado correctamente con integración de Google ADK")
        except ImportError as e:
            logger.warning(f"No se pudo importar la biblioteca oficial de Google ADK: {e}")
            pytest.skip("Biblioteca oficial de Google ADK no disponible")
        except Exception as e:
    ...
    @pytest.mark.asyncio
    async def test_adk_agent_skill_execution(self, mock_gemini_client, mock_supabase_client, mock_state_manager, test_skill):
        ...
        try:
            ...
            if hasattr(toolkit, 'execute_skill'):
                result = await toolkit.execute_skill("test_skill", input_text="Hola mundo")
            ...
            logger.info("Skill ejecutada correctamente a través del agente ADK con integración de Google ADK")
        except ImportError as e:
            logger.warning(f"No se pudo importar la biblioteca oficial de Google ADK: {e}")
            pytest.skip("Biblioteca oficial de Google ADK no disponible")
        except Exception as e:
