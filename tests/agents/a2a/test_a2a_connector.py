...
async def test_discover_agents_http_error(a2a_connector):
    ...
    mock_error = HTTPStatusError("Error HTTP", request=MagicMock(), response=mock_response)
...
async def test_discover_agents_connection_error(a2a_connector):
    ...
    a2a_connector.client.get.side_effect = RequestError("Error de conexión", request=MagicMock())
...
async def test_request_task_success(a2a_connector):
    ...
    mock_response = create_mock_response(
        status_code=200,
        json_data={"task_id": "task123", "status": "pending"}
    )
...
async def test_get_task_status_success(a2a_connector):
    ...
    mock_response = create_mock_response(
        status_code=200,
        json_data={"task_id": "task123", "status": "completed", "result": "test result"}
    )
    a2a_connector.client.get.return_value = mock_response
    ...
    # Verificar el resultado
    assert result == {"task_id": "task123", "status": "completed", "result": "test result"}
...
async def test_wait_for_task_completion_success(a2a_connector):
    ...
    pending_response = create_mock_response(
        status_code=200,
        json_data={"task_id": "task123", "status": "submitted"}
    )
    ...
    result = await a2a_connector.wait_for_task_completion("task123", timeout=1.0, poll_interval=0.1)
...
async def test_wait_for_task_completion_timeout(a2a_connector):
    ...
    with pytest.raises(TimeoutError) as excinfo:
        await a2a_connector.wait_for_task_completion("task123", timeout=0.2, poll_interval=0.1)
...
async def test_execute_task_success(a2a_connector):
    ...
    with patch.object(a2a_connector, 'request_task', return_value=request_response) as mock_request:
        with patch.object(a2a_connector, 'wait_for_task_completion', return_value=completion_response) as mock_wait:
            # Ejecutar la función
            result = await a2a_connector.execute_task("agent1", {"input": "test"}, wait_for_completion=True)
...
async def test_execute_task_no_wait(a2a_connector):
    ...
    with patch.object(a2a_connector, 'request_task', return_value=request_response) as mock_request:
        with patch.object(a2a_connector, 'wait_for_task_completion') as mock_wait:
            ...
            result = await a2a_connector.execute_task("agent1", {"input": "test"}, wait_for_completion=False)
...
async def test_register_agent_success(a2a_connector):
    ...
    mock_response = create_mock_response(
        status_code=200,
        json_data={"success": True, "agent_id": "agent1"}
    )
    a2a_connector.client.post.return_value = mock_response
    ...
    agent_info.dict = MagicMock(return_value=agent_data)  # Para compatibilidad con versiones anteriores
...
async def test_update_agent_status_success(a2a_connector):
    ...
    # Verificar el resultado
    assert result == {"success": True, "agent_id": "agent1", "status": AgentStatus.ONLINE}
    a2a_connector.client.put.assert_called_once_with(
        "/agents/status", 
        json={"agent_id": "agent1", "status": AgentStatus.ONLINE}
    )
