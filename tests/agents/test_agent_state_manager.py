...
@pytest.mark.asyncio
async def test_progress_tracker_state_manager(progress_tracker, test_settings: Dict[str, Any]):
...
@pytest.mark.asyncio
async def test_elite_training_strategist_state_manager(elite_training_strategist, test_settings: Dict[str, Any]):
...
@pytest.mark.asyncio
async def test_precision_nutrition_architect_state_manager(precision_nutrition_architect, test_settings: Dict[str, Any]):
...
@pytest.mark.asyncio
async def test_biohacking_innovator_state_manager(biohacking_innovator, test_settings: Dict[str, Any]):
...
@pytest.mark.asyncio
async def test_biometrics_insight_engine_state_manager(biometrics_insight_engine, test_settings: Dict[str, Any]):
...
@pytest.mark.asyncio
async def test_client_success_liaison_state_manager(client_success_liaison, test_settings: Dict[str, Any]):
    ...
    # Ejecutar el agente
    await client_success_liaison._run_async_impl(input_text, user_id, session_id=session_id)
...
@pytest.mark.asyncio
async def test_recovery_corrective_state_manager(recovery_corrective, test_settings: Dict[str, Any]):
    ...
    # Ejecutar el agente
    await recovery_corrective._run_async_impl(input_text, user_id, session_id=session_id)
...
@pytest.mark.asyncio
async def test_security_compliance_guardian_state_manager(security_compliance_guardian, test_settings: Dict[str, Any]):
    """Prueba la integración del StateManager en el agente SecurityComplianceGuardian."""
    # Datos de prueba
    user_id = test_settings["test_user_id"]
    session_id = str(uuid.uuid4())
    input_text = "¿Qué medidas de seguridad debo implementar para proteger datos de salud?"
    
    # Ejecutar el agente
    await security_compliance_guardian._run_async_impl(input_text, user_id, session_id=session_id)
...
@pytest.mark.asyncio
async def test_systems_integration_ops_state_manager(systems_integration_ops, test_settings: Dict[str, Any]):
...
@pytest.mark.asyncio
async def test_gemini_training_assistant_state_manager(gemini_training_assistant, test_settings: Dict[str, Any]):
    ...
    # Ejecutar el agente
    await gemini_training_assistant._run_async_impl(input_text, user_id, session_id=session_id)
...
@pytest.mark.asyncio
async def test_motivation_behavior_coach_state_manager(motivation_behavior_coach, test_settings: Dict[str, Any]):
    ...
    # Ejecutar el agente
    await motivation_behavior_coach._run_async_impl(input_text, user_id, session_id=session_id)
...
@pytest.mark.asyncio
async def test_context_persistence_across_sessions(orchestrator, test_settings: Dict[str, Any]):
...
@pytest.mark.asyncio
async def test_context_sharing_between_agents(orchestrator, elite_training_strategist, test_settings: Dict[str, Any]):
