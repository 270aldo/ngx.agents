...
@pytest.mark.asyncio
async def test_orchestrator_process_request_successful_single_agent(
    orchestrator, mock_user_id, mock_session_id
):
    ...
    mock_artifacts = []
    orchestrator._synthesize = AsyncMock(
        return_value=(mock_synthesized_response, mock_artifacts)
    )

    # Mockear _determine_target_agents (asumiendo que existe y devuelve [1] para este test)
    orchestrator._determine_target_agents = AsyncMock(
        return_value=([1], "direct_request")
    )  # Simula la selección de un agente
    ...
    result = await orchestrator._process_request(
        input_text, user_id=mock_user_id, session_id=mock_session_id
    )
