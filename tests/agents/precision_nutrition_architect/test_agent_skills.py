...
async def test_skill_create_meal_plan(mocked_agent):
    ...
    with patch.object(mocked_agent, '_generate_meal_plan', new_callable=AsyncMock) as mock_generate:
...
async def test_skill_recommend_supplements(mocked_agent):
    ...
    with patch.object(mocked_agent, '_generate_supplement_recommendation', new_callable=AsyncMock) as mock_generate:
        ...
        input_params = RecommendSupplementsInput(
            ...
            user_profile={"age": 40, "sex": "male", "health_conditions": ["hipertensión"]},
...
async def test_skill_analyze_biomarkers(mocked_agent):
    ...
    with patch.object(mocked_agent, '_generate_biomarker_analysis', new_callable=AsyncMock) as mock_generate:
...
async def test_skill_plan_chrononutrition(mocked_agent):
        ...
        "time_windows": [
            {
                ...
                "nutritional_focus": ["Proteínas de alta calidad", "Carbohidratos complejos"]
            }
        ],
    ...
    with patch.object(mocked_agent, '_generate_chrononutrition_plan', new_callable=AsyncMock) as mock_generate:
