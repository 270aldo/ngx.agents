...
def mock_vertex_ai_client():
    ...
    with patch('clients.vertex_ai_client.vertex_ai_client') as mock:
        ...
        mock.generate_multimodal_content = AsyncMock(return_value={
...
def mock_gemini_client():
    ...
    mock.generate_structured_output = AsyncMock(return_value={
        "result": "Resultado estructurado simulado",
        "confidence": 0.9
    })
...
async def setup_agents(mock_gemini_client, mock_supabase_client, setup_a2a_server):
    ...
    await setup_a2a_server.register_agent("client_success", client_success._run_async_impl)
    await setup_a2a_server.register_agent("security_guardian", security_guardian._run_async_impl)
    await setup_a2a_server.register_agent("systems_integration", systems_integration._run_async_impl)
...
class TestSystemIntegration:
    ...
    @pytest.mark.asyncio
    async def test_agent_communication(self, setup_agents, setup_a2a_server, mock_state_manager_adapter):
    ...
    @pytest.mark.asyncio
    async def test_cross_agent_consultation(self, setup_agents, setup_a2a_server, mock_state_manager_adapter):
    ...
    @pytest.mark.asyncio
    async def test_multi_agent_workflow(self, setup_agents, setup_a2a_server, mock_state_manager_adapter):
