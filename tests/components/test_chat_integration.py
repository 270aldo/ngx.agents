...
from unittest.mock import patch, MagicMock, AsyncMock # Mantener AsyncMock si MockOrchestrator lo usa

# Importar la aplicación principal y los esquemas correctos
from app.main import app
from app.schemas.chat import ChatRequest, ChatResponse, AgentResponse # Esquemas correctos

# Constantes para pruebas
TEST_API_KEY = "test_api_key" # Podría no ser necesario si get_current_user se mockea directamente
...
class MockNGXNexusOrchestrator:
    ...
    async def connect(self):
        ...
        print("MockNGXNexusOrchestrator connected")

    async def run_async(self, input_text: str, user_id: str, session_id: str, context: dict):
        ...
        if "vacío" in input_text.lower():
            return {
                "response": "",
                "agents_used": ["agent1"],
                "agent_responses": [AgentResponse(agent_id="agent1", agent_name="Agent 1", response="", confidence=0.7, artifacts=[])],
                "metadata": {},
                "session_id": session_id
            }
        ...
        if "múltiple" in input_text.lower():
            return {
                ...
                "agent_responses": [
                    AgentResponse(agent_id="agent1", agent_name="Agent 1", response="R1", confidence=0.9, artifacts=[]),
                    AgentResponse(agent_id="agent2", agent_name="Agent 2", response="R2", confidence=0.9, artifacts=[]),
                    AgentResponse(agent_id="agent3", agent_name="Agent 3", response="R3", confidence=0.9, artifacts=[])
                ],
                "metadata": {},
                "session_id": session_id
            }
        
        if any(keyword in input_text.lower() for keyword in ["entrenamiento", "fuerza", "potencia", "rutina"]):
            return {
                "response": "Plan de entrenamiento personalizado para ganar fuerza y potencia",
                "agents_used": ["elite_training_strategist"],
                "agent_responses": [AgentResponse(agent_id="elite_training_strategist", agent_name="ETS", response="Plan...", confidence=0.85, artifacts=[])],
                "metadata": {},
                "session_id": session_id
            }
        ...
        return {
            ...
            "agents_used": ["agent1"],
            "agent_responses": [AgentResponse(agent_id="agent1", agent_name="Agent 1", response=f"Respuesta para {input_text}", confidence=0.8, artifacts=[])],
            "metadata": {},
            "session_id": session_id
        }
    ...
    async def close(self):
        ...
        print("MockNGXNexusOrchestrator closed")
...
def mock_get_orchestrator(mocker): # mocker es un fixture de pytest-mock
    ...
    mock_orch = MockNGXNexusOrchestrator(a2a_server_url="ws://mockhost:8000", state_manager=MagicMock())
...
def test_chat_endpoint_error_response():
    ...
    assert response.status_code == 200 # El endpoint maneja el error y devuelve 200 con error en metadata
...
def test_chat_endpoint_elite_training():
    ...
    request_data = ChatRequest(text="Necesito un plan de entrenamiento para ganar fuerza y potencia")
