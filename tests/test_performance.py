...
from infrastructure.adapters.motivation_behavior_coach_adapter import MotivationBehaviorCoachAdapter
from infrastructure.adapters.recovery_corrective_adapter import RecoveryCorrectiveAdapter
from infrastructure.adapters.client_success_liaison_adapter import ClientSuccessLiaisonAdapter
...
def mock_state_manager_adapter():
    ...
    with patch('core.state_manager_adapter.state_manager_adapter') as mock:
        # Configurar respuestas simuladas
        mock.load_state = AsyncMock(return_value={
...
def mock_gemini_client():
    ...
    mock = MagicMock()
    mock.generate_response = AsyncMock(return_value="Respuesta simulada del modelo Gemini")
...
@pytest.fixture
async def setup_optimized_agents(mock_gemini_client, mock_supabase_client, setup_a2a_server):
    ...
    await setup_a2a_server.register_agent("motivation_coach", motivation_coach._run_async_impl)
    await setup_a2a_server.register_agent("recovery_corrective", recovery_corrective._run_async_impl)
...
class TestPerformance:
    ...
    @pytest.mark.asyncio
    async def test_response_time_comparison(self, setup_optimized_agents, setup_original_agents, mock_state_manager_adapter):
        ...
        # Imprimir resultados
        logger.info(f"Tiempo promedio de respuesta (optimizado): {avg_optimized:.4f} segundos")
        logger.info(f"Tiempo promedio de respuesta (original): {avg_original:.4f} segundos")
        logger.info(f"Mejora de rendimiento: {(1 - avg_optimized/avg_original) * 100:.2f}%")
        
        # Verificar que el rendimiento ha mejorado
        assert avg_optimized <= avg_original, "El rendimiento de los agentes optimizados debería ser igual o mejor que el de los originales"
    
    @pytest.mark.asyncio
    async def test_memory_usage_comparison(self, setup_optimized_agents, setup_original_agents):
    ...
    @pytest.mark.asyncio
    async def test_a2a_communication_performance(self, setup_optimized_agents, setup_a2a_server):
        ...
        setup_a2a_server.send_message = AsyncMock(return_value={
            "status": "success",
        ...
        assert avg_time < 0.5, "El tiempo promedio de comunicación A2A debería ser menor a 0.5 segundos"
    ...
    async def test_state_manager_performance(self, mock_state_manager_adapter):
        ...
        # Imprimir resultados
        logger.info(f"Tiempo promedio de guardado de estado: {avg_save_time:.4f} segundos")
        logger.info(f"Tiempo promedio de carga de estado: {avg_load_time:.4f} segundos")
        
        # Verificar que los tiempos son aceptables
        assert avg_save_time < 0.1, "El tiempo promedio de guardado de estado debería ser menor a 0.1 segundos"
        assert avg_load_time < 0.1, "El tiempo promedio de carga de estado debería ser menor a 0.1 segundos"
    
    @pytest.mark.asyncio
    async def test_vertex_ai_client_performance(self, mock_vertex_ai_client, mock_vertex_client):
        ...
        # Imprimir resultados
        logger.info(f"Tiempo promedio de respuesta (cliente centralizado): {avg_centralized:.4f} segundos")
        logger.info(f"Tiempo promedio de respuesta (cliente antiguo): {avg_old:.4f} segundos")
        logger.info(f"Mejora de rendimiento: {(1 - avg_centralized/avg_old) * 100:.2f}%")
        
        # Verificar que el rendimiento ha mejorado
        assert avg_centralized <= avg_old, "El rendimiento del cliente centralizado debería ser igual o mejor que el del cliente antiguo"