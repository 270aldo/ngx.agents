...
from infrastructure.adapters import get_telemetry_adapter, measure_execution_time, TelemetryAdapter
...
class TestTelemetryAdapter:
    ...
    def test_mock_mode_when_telemetry_not_available(self):
        ...
        with patch('infrastructure.adapters.telemetry_adapter.TELEMETRY_AVAILABLE', False):
    ...
    def test_integration_with_real_client(self, mock_telemetry_client_class):
        ...
        with patch('infrastructure.adapters.telemetry_adapter.TELEMETRY_AVAILABLE', True):
            # Reiniciar la instancia global para forzar la creación de una nueva
            import infrastructure.adapters.telemetry_adapter
            infrastructure.adapters.telemetry_adapter._telemetry_adapter_instance = None
            
            adapter = get_telemetry_adapter()
            assert adapter.client is mock_client, "El cliente debería ser el mock en modo real"
            
            # Verificar que las operaciones llaman a los métodos correctos del cliente
            adapter.start_span("test_span", {"attr": "value"})
            mock_client.start_span.assert_called_once_with("test_span", {"attr": "value"})
            
            adapter.record_metric("test_metric", 42, {"dim": "value"})
            mock_client.record_metric.assert_called_once_with("test_metric", 42, {"dim": "value"})
    ...
    async def test_measure_execution_time_decorator(self):
        ...
        with patch('infrastructure.adapters.telemetry_adapter.get_telemetry_adapter') as mock_get_adapter:
            ...
            assert result == "result", "La función decorada debería devolver el resultado original"
            ...
            assert args[0] == "test_function", "El nombre de la métrica debería ser 'test_function'"
            ...
            assert args[2] == {"test_attr": "test_value"}, "Los atributos deberían ser correctos"
    ...
    def test_span_management_in_mock_mode(self):
        ...
        assert span["attributes"] == {"attr1": "value1"}, "Los atributos deberían ser correctos"
        assert span["events"] == [], "La lista de eventos debería estar vacía inicialmente"
        
        # Añadir atributos
        adapter.set_span_attribute(span, "attr2", "value2")
        assert span["attributes"]["attr2"] == "value2", "El atributo debería haberse añadido"
        
        # Añadir eventos
        adapter.add_span_event(span, "test_event", {"event_attr": "event_value"})
        assert len(span["events"]) == 1, "Debería haber un evento"
        assert span["events"][0]["name"] == "test_event", "El nombre del evento debería ser correcto"
        assert span["events"][0]["attributes"] == {"event_attr": "event_value"}, "Los atributos del evento deberían ser correctos"
        
        # Registrar excepción
        test_exception = ValueError("Test exception")
        adapter.record_exception(span, test_exception)
        assert len(span["events"]) == 2, "Debería haber dos eventos"
        assert span["events"][1]["name"] == "exception", "El nombre del evento debería ser 'exception'"
        assert span["events"][1]["attributes"]["exception.type"] == "ValueError", "El tipo de excepción debería ser correcto"
        assert span["events"][1]["attributes"]["exception.message"] == "Test exception", "El mensaje de excepción debería ser correcto"
    ...
    async def test_exception_handling_in_decorated_function(self):
        ...
        with patch('infrastructure.adapters.telemetry_adapter.get_telemetry_adapter') as mock_get_adapter:
            ...
            # Verificar el nombre de la métrica
            assert args[0] == "test_function_with_exception", "El nombre de la métrica debería ser correcto"
    ...
    def test_logging_in_mock_mode(self, caplog):
        ...
        assert "METRIC: test_metric = 42 {dim=value}" in caplog.text, "La métrica debería registrarse en el log"
        ...
        # Verificar que se registró en el log
        assert "COUNTER: test_counter += 1 {dim=value}" in caplog.text, "El contador debería registrarse en el log"
        ...
        # Verificar que se registró en el log
        assert "HISTOGRAM: test_histogram = 100 {dim=value}" in caplog.text, "El histograma debería registrarse en el log"
