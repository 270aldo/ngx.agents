...
from infrastructure.adapters.precision_nutrition_architect_adapter import PrecisionNutritionArchitectAdapter
...
def precision_nutrition_architect_adapter():
    ...
    gemini_client_mock.generate_text = AsyncMock(return_value=json.dumps({
        ...
        "meals": [
            {
                "name": "Desayuno",
                "examples": ["Avena con frutas y nueces"]
            },
            {
                "name": "Almuerzo",
                "examples": ["Ensalada con proteína magra"]
            }
        ],
    ...
    with patch.object(state_manager_adapter, 'load_state', new_callable=AsyncMock) as mock_load_state:
        ...
        mock_load_state.return_value = {
            "conversation_history": [
                {"role": "user", "content": "Necesito un plan de alimentación para bajar de peso"}
            ],
            "user_profile": {"age": 30, "gender": "masculino", "dietary_restrictions": "sin gluten"},
            "meal_plans": [],
        ...
        context = await precision_nutrition_architect_adapter._get_context("test_user", "test_session")
...
async def test_update_context(precision_nutrition_architect_adapter):
    ...
    with patch.object(state_manager_adapter, 'save_state', new_callable=AsyncMock) as mock_save_state:
        ...
        test_context = {
            "conversation_history": [
                {"role": "user", "content": "Necesito un plan de alimentación para bajar de peso"},
                {"role": "assistant", "content": "Aquí tienes un plan de alimentación..."}
            ],
            "user_profile": {"age": 30, "gender": "masculino", "dietary_restrictions": "sin gluten"},
            "meal_plans": [
                {
                    ...
                    "macronutrient_distribution": {"protein": "25-30%", "carbs": "40-50%", "fats": "20-30%"}
                }
            ],
            "supplement_recommendations": [],
            "biomarker_analyses": [],
        ...
        # Llamar al método a probar
        await precision_nutrition_architect_adapter._update_context(test_context, "test_user", "test_session")
...
async def test_classify_query(precision_nutrition_architect_adapter):
    ...
    with patch.object(intent_analyzer_adapter, 'analyze_intent', new_callable=AsyncMock) as mock_analyze_intent:
        ...
        # Llamar al método a probar
        query_type = await precision_nutrition_architect_adapter._classify_query("Necesito un plan de alimentación para bajar de peso que sea sin gluten")
...
async def test_skill_create_meal_plan(precision_nutrition_architect_adapter):
    ...
    # Llamar al método a probar
    result = await precision_nutrition_architect_adapter._skill_create_meal_plan(input_data)
