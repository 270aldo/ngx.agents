...
def motivation_behavior_coach_adapter():
    ...
    gemini_client_mock.generate_structured_output = AsyncMock(return_value={
        ...
        "steps": [
            {"description": "Comenzar con 5 minutos", "timeframe": "Semana 1", "difficulty": "Baja"},
            {"description": "Aumentar a 10 minutos", "timeframe": "Semana 2", "difficulty": "Media"},
            {"description": "Llegar a 15 minutos", "timeframe": "Semana 3", "difficulty": "Media"}
        ],
...
async def test_get_context(motivation_behavior_coach_adapter):
    ...
    with patch.object(state_manager_adapter, 'load_state', new_callable=AsyncMock) as mock_load_state:
        ...
        context = await motivation_behavior_coach_adapter._get_context("test_user", "test_session")
...
async def test_update_context(motivation_behavior_coach_adapter):
    ...
    with patch.object(state_manager_adapter, 'save_state', new_callable=AsyncMock) as mock_save_state:
        ...
        test_context = {
            "conversation_history": [
                {"role": "user", "content": "Quiero establecer un hábito de ejercicio"},
                {"role": "assistant", "content": "Aquí tienes un plan para establecer ese hábito..."}
            ],
            "user_profile": {"goals": ["Mejorar salud", "Aumentar energía"]},
            "habit_plans": [
                {"habit": "Ejercicio diario", "cue": "Después de despertarse", "routine": "15 minutos de ejercicio"}
            ],
            "goal_plans": [],
            "last_updated": datetime.now().isoformat()
        }
        ...
        # Llamar al método a probar
        await motivation_behavior_coach_adapter._update_context(test_context, "test_user", "test_session")
...
async def test_classify_query(motivation_behavior_coach_adapter):
    ...
    with patch.object(intent_analyzer_adapter, 'analyze_intent', new_callable=AsyncMock) as mock_analyze_intent:
        ...
        # Llamar al método a probar
        query_type = await motivation_behavior_coach_adapter._classify_query("Quiero establecer un hábito de ejercicio diario")
...
async def test_classify_query_by_keywords(motivation_behavior_coach_adapter):
    ...
    assert motivation_behavior_coach_adapter._classify_query_by_keywords("Quiero establecer un hábito de meditación") == "habit_formation"
    assert motivation_behavior_coach_adapter._classify_query_by_keywords("Necesito motivación para seguir con mi dieta") == "motivation_strategies"
    assert motivation_behavior_coach_adapter._classify_query_by_keywords("Cómo puedo cambiar mi comportamiento de procrastinación") == "behavior_change"
    assert motivation_behavior_coach_adapter._classify_query_by_keywords("Quiero establecer una meta de correr un maratón") == "goal_setting"
    assert motivation_behavior_coach_adapter._classify_query_by_keywords("Cómo superar el obstáculo de la falta de tiempo") == "obstacle_management"
    
    # Probar consulta sin palabras clave específicas
    assert motivation_behavior_coach_adapter._classify_query_by_keywords("Ayúdame con mi entrenamiento") == "motivation_strategies"
...
async def test_skill_habit_formation(motivation_behavior_coach_adapter):
    ...
    with patch.object(motivation_behavior_coach_adapter, '_generate_habit_plan', new_callable=AsyncMock) as mock_generate_habit_plan:
        ...
        mock_generate_habit_plan.return_value = {
            ...
            "tips": ["Comenzar pequeño", "Ser consistente", "Celebrar logros"],
            "obstacles": [{"obstacle": "Falta de tiempo", "strategy": "Levantarse 15 minutos antes"}],
            "consistency_strategies": ["Usar recordatorios", "Preparar ropa de ejercicio la noche anterior"]
        }
        ...
        # Llamar al método a probar
        result = await motivation_behavior_coach_adapter._skill_habit_formation(input_data)
