...
from infrastructure.adapters.elite_training_strategist_adapter import EliteTrainingStrategistAdapter
    ...
    with patch.object(state_manager_adapter, 'load_state', new_callable=AsyncMock) as mock_load_state:
        ...
        mock_load_state.return_value = {
            "conversation_history": [
                {"role": "user", "content": "Necesito un plan de entrenamiento para aumentar mi fuerza"}
            ],
        ...
        context = await elite_training_strategist_adapter._get_context("test_user", "test_session")
...
async def test_update_context(elite_training_strategist_adapter):
    ...
    with patch.object(state_manager_adapter, 'save_state', new_callable=AsyncMock) as mock_save_state:
        ...
        test_context = {
            "conversation_history": [
                {"role": "user", "content": "Necesito un plan de entrenamiento para aumentar mi fuerza"},
                {"role": "assistant", "content": "Aquí tienes un plan de entrenamiento..."}
            ],
            "user_profile": {"age": 30, "gender": "masculino"},
            "training_plans": [
                {
        ...
        # Llamar al método a probar
        await elite_training_strategist_adapter._update_context(test_context, "test_user", "test_session")
...
async def test_classify_query(elite_training_strategist_adapter):
    ...
    with patch.object(intent_analyzer_adapter, 'analyze_intent', new_callable=AsyncMock) as mock_analyze_intent:
        ...
        # Llamar al método a probar
        query_type = await elite_training_strategist_adapter._classify_query("Necesito un plan de entrenamiento para aumentar mi fuerza en 8 semanas")
...
async def test_skill_generate_training_plan(elite_training_strategist_adapter):
    ...
    # Llamar al método a probar
    result = await elite_training_strategist_adapter._skill_generate_training_plan(input_data)
