...
def client_success_liaison_adapter():
    ...
    mcp_toolkit_mock.invoke = AsyncMock(return_value={
        "results": [
            {"title": "Resultado 1", "snippet": "Descripción 1", "url": "https://example.com/1"},
            {"title": "Resultado 2", "snippet": "Descripción 2", "url": "https://example.com/2"}
        ]
    })
    ...
    with patch.object(state_manager_adapter, 'load_state', new_callable=AsyncMock) as mock_load_state:
        ...
        mock_load_state.return_value = {
            "conversation_history": [
                {"role": "user", "content": "¿Cómo puedo mejorar la retención de usuarios?"}
            ],
            "user_profile": {"company": "Tech Startup", "industry": "SaaS"},
            "calendars": [{"timestamp": "2025-10-01T12:00:00", "community_plan": {"objective": "Construir comunidad"}}],
            "journey_maps": [{"timestamp": "2025-10-01T12:00:00", "journey_map": {"stages": ["Descubrimiento", "Onboarding"]}}],
            "support_requests": [{"timestamp": "2025-10-01T12:00:00", "ticket": {"ticket_id": "123", "status": "open"}}],
            "last_updated": datetime.now().isoformat()
        }
        
        # Llamar al método a probar
        context = await client_success_liaison_adapter._get_context("test_user", "test_session")
        ...
        assert context["calendars"][0]["community_plan"]["objective"] == "Construir comunidad"
...
async def test_update_context(client_success_liaison_adapter):
    ...
    with patch.object(state_manager_adapter, 'save_state', new_callable=AsyncMock) as mock_save_state:
        ...
        test_context = {
            "conversation_history": [
                {"role": "user", "content": "¿Cómo puedo mejorar la retención de usuarios?"},
                {"role": "assistant", "content": "Aquí tienes algunas estrategias para mejorar la retención..."}
            ],
            "user_profile": {"company": "Tech Startup", "industry": "SaaS"},
            "calendars": [{"timestamp": "2025-10-01T12:00:00", "community_plan": {"objective": "Construir comunidad"}}],
            "journey_maps": [],
        ...
        # Llamar al método a probar
        await client_success_liaison_adapter._update_context(test_context, "test_user", "test_session")
...
async def test_classify_query_with_intent_analyzer(client_success_liaison_adapter):
    ...
    with patch.object(intent_analyzer_adapter, 'analyze_intent', new_callable=AsyncMock) as mock_analyze_intent:
        ...
        # Llamar al método a probar
        query_type = await client_success_liaison_adapter._classify_query_with_intent_analyzer("¿Cómo puedo construir una comunidad online para mi producto?")
...
@pytest.mark.asyncio
async def test_classify_query_with_intent_analyzer_fallback(client_success_liaison_adapter):
    ...
    with patch.object(intent_analyzer_adapter, 'analyze_intent', new_callable=AsyncMock) as mock_analyze_intent:
        ...
        with patch.object(client_success_liaison_adapter, '_classify_query', return_value="retention_strategies") as mock_classify_query:
            # Llamar al método a probar
            query_type = await client_success_liaison_adapter._classify_query_with_intent_analyzer("¿Cómo puedo mejorar la retención de usuarios?")
...
async def test_run_async_impl(client_success_liaison_adapter):
    ...
    with patch.object(client_success_liaison_adapter, '_classify_query_with_intent_analyzer', new_callable=AsyncMock) as mock_classify_query:
        ...
        with patch.object(client_success_liaison_adapter, '_get_context', new_callable=AsyncMock) as mock_get_context:
            ...
            community_skill_mock.handler = AsyncMock(return_value=MagicMock(
            ...
            try:
                # Llamar al método a probar
                with patch.object(client_success_liaison_adapter, '_update_context', new_callable=AsyncMock) as mock_update_context:
                    # Configurar el mock para no hacer nada
                    mock_update_context.return_value = None
                    
                    # Llamar al método a probar
                    with patch.object(client_success_liaison_adapter, '_classify_query', return_value="community_building"):
