...
async def test_state_manager_adapter_get_conversation():
    ...
    retrieved_context = await state_manager_adapter.get_conversation(context.conversation_id)
...
async def test_state_manager_adapter_delete_conversation():
    ...
    deleted_context = await state_manager_adapter.get_conversation(context.conversation_id)
...
async def test_state_manager_adapter_switch_mode():
    ...
    try:
        ...
        # En modo original
        retrieved_optimized = await state_manager_adapter.get_conversation(context_optimized.conversation_id)
        assert retrieved_optimized is not None
        assert retrieved_optimized.metadata.get("mode") == "optimized"
        
        # En modo optimizado
        state_manager_adapter.use_optimized = True
        retrieved_original = await state_manager_adapter.get_conversation(context_original.conversation_id)
...
async def test_state_manager_adapter_conversion():
    ...
    converted_context = state_manager_adapter._convert_to_conversation_context(optimized_state)
