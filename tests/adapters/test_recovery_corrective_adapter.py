...
async def test_analyze_recovery_needs(recovery_adapter, mock_vertex_client):
    ...
    with patch.object(recovery_adapter, '_parse_recovery_analysis') as mock_parse:
        ...
        # Llamar al método
        result = await recovery_adapter.analyze_recovery_needs(user_data, training_history)
...
@pytest.mark.asyncio
async def test_generate_recovery_plan(recovery_adapter, mock_a2a_client, mock_vertex_client):
    """Prueba el método generate_recovery_plan()."""
    # Configurar mocks
    mock_a2a_client.call_agent.side_effect = [
        {"recommendations": ["nutrición1", "nutrición2"]},  # Respuesta del agente de nutrición
        {"recommendations": ["entrenamiento1", "entrenamiento2"]}  # Respuesta del agente de entrenamiento
    ]
    ...
    with patch.object(recovery_adapter, '_parse_recovery_plan') as mock_parse:
        ...
        # Llamar al método
        result = await recovery_adapter.generate_recovery_plan(recovery_needs, user_preferences)
...
@pytest.mark.asyncio
async def test_adjust_training_program(recovery_adapter, mock_a2a_client, mock_vertex_client):
    ...
    # Datos de prueba
    current_program = {"user_id": "test_user", "sessions": [{"day": "lunes", "type": "strength"}]}
    ...
    with patch.object(recovery_adapter, '_parse_adjusted_program') as mock_parse:
        ...
        # Llamar al método
        result = await recovery_adapter.adjust_training_program(current_program, recovery_plan)
...
async def test_provide_recovery_guidance(recovery_adapter):
    ...
    with patch.object(recovery_adapter, '_get_user_data') as mock_get_user_data:
        with patch.object(recovery_adapter, '_get_training_history') as mock_get_training_history:
            with patch.object(recovery_adapter, 'analyze_recovery_needs') as mock_analyze:
                with patch.object(recovery_adapter, '_build_guidance_prompt') as mock_build_prompt:
                    with patch.object(recovery_adapter, '_parse_guidance') as mock_parse:
                        with patch.object(recovery_adapter.vertex_client, 'generate_text') as mock_generate:
                            # Configurar valores de retorno
                            mock_get_user_data.return_value = {"user_id": "test_user", "age": 30}
                            mock_get_training_history.return_value = [{"date": "2025-10-01", "type": "strength"}]
                            mock_analyze.return_value = {"factors": ["factor1"], "fatigue_level": "medio"}
                            ...
                            # Llamar al método
                            result = await recovery_adapter.provide_recovery_guidance("test_user")
                            
                            # Verificar que se llamaron los métodos correctos
                            mock_get_user_data.assert_called_once_with("test_user")
                            mock_get_training_history.assert_called_once_with("test_user")
                            mock_analyze.assert_called_once()
                            mock_build_prompt.assert_called_once()
                            mock_generate.assert_called_once_with("Prompt de orientación")
...
async def test_consult_other_agent(recovery_adapter, mock_a2a_client):
    ...
    # Llamar al método
    result = await recovery_adapter._consult_other_agent("test_agent", "consulta de prueba")
    
    # Verificar que se llamó al método correcto
    mock_a2a_client.call_agent.assert_called_once_with("test_agent", "consulta de prueba", None)
...
def test_build_recovery_analysis_prompt(recovery_adapter):
    ...
    # Llamar al método
    result = recovery_adapter._build_recovery_analysis_prompt(user_data, training_history)
