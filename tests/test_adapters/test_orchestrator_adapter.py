...
async def test_initialize():
    ...
    with patch('infrastructure.adapters.orchestrator_adapter.a2a_adapter') as mock_a2a_adapter:
        # Configurar mocks
...
async def test_analyze_intent():
    ...
    # Mockear las dependencias
    with patch('infrastructure.adapters.orchestrator_adapter.intent_analyzer_adapter') as mock_intent_analyzer, \
         patch('infrastructure.adapters.orchestrator_adapter.telemetry') as mock_telemetry:
...
async def test_determine_target_agents():
    ...
    # Mockear las dependencias
    with patch('infrastructure.adapters.orchestrator_adapter.telemetry') as mock_telemetry:
        ...
        target_agents, priority = await orchestrator_adapter._determine_target_agents(intent, user_input, context)
        
        # Verificar resultado
        assert target_agents == ["elite_training_strategist", "precision_nutrition_architect"]
...
async def test_route_message_single_agent():
    ...
    # Mockear las dependencias
    with patch('infrastructure.adapters.orchestrator_adapter.telemetry') as mock_telemetry, \
         patch.object(orchestrator_adapter, '_consult_other_agent') as mock_consult_agent:
...
async def test_route_message_multiple_agents():
    ...
    # Mockear las dependencias
    with patch('infrastructure.adapters.orchestrator_adapter.telemetry') as mock_telemetry, \
         patch.object(orchestrator_adapter, '_call_multiple_agents_parallel') as mock_call_multiple, \
...
async def test_process_query():
    ...
    with patch('infrastructure.adapters.orchestrator_adapter.telemetry') as mock_telemetry, \
         ...
         patch.object(orchestrator_adapter, '_determine_target_agents') as mock_determine_agents, \
