...
from infrastructure.adapters.biometrics_insight_engine_adapter import biometrics_insight_engine_adapter
...
async def test_initialize():
    ...
    with patch('infrastructure.adapters.biometrics_insight_engine_adapter.intent_analyzer_adapter') as mock_intent_analyzer, \
         patch('infrastructure.adapters.biometrics_insight_engine_adapter.state_manager_adapter') as mock_state_manager, \
         patch('infrastructure.adapters.biometrics_insight_engine_adapter.a2a_adapter') as mock_a2a_adapter:
...
async def test_process_query():
    ...
    # Mockear las dependencias
    with patch('infrastructure.adapters.biometrics_insight_engine_adapter.intent_analyzer_adapter') as mock_intent_analyzer, \
         patch('infrastructure.adapters.biometrics_insight_engine_adapter.telemetry_adapter') as mock_telemetry, \
         patch('infrastructure.adapters.biometrics_insight_engine_adapter.vertex_ai_client') as mock_vertex_ai:
        ...
        mock_vertex_ai.generate_text.return_value = "Análisis biométrico generado por el modelo"
...
async def test_determine_query_type():
    ...
    for intent_result, query, expected_type in test_cases:
        result = biometrics_insight_engine_adapter._determine_query_type(intent_result, query)
        assert result == expected_type, f"Para intent={intent_result}, query='{query}', se esperaba '{expected_type}' pero se obtuvo '{result}'"
...
async def test_build_prompt_with_context():
    ...
    # Construir prompt
    result = biometrics_insight_engine_adapter._build_prompt_with_context(prompt, context, profile, program_type)
...
async def test_a2a_integration():
    ...
    # Mockear las dependencias
    with patch('infrastructure.adapters.a2a_adapter.a2a_server') as mock_a2a_server, \
         patch.object(biometrics_insight_engine_adapter, 'run_async_impl') as mock_run_async:
        ...
        assert biometrics_insight_engine_adapter.agent_id in a2a_adapter.registered_agents
        ...
        callback = a2a_adapter.registered_agents[biometrics_insight_engine_adapter.agent_id]["message_callback"]
...
async def test_handle_biometric_analysis():
    ...
    # Mockear las dependencias
    with patch.object(biometrics_insight_engine_adapter, '_generate_response') as mock_generate_response:
        ...
        assert result["context"]["analyses"][0]["analysis"] == "Análisis biométrico generado"
...
async def test_handle_pattern_recognition():
    ...
    with patch.object(biometrics_insight_engine_adapter, '_generate_response') as mock_generate_response:
        ...
        assert result["context"]["pattern_analyses"][0]["analysis"] == "Análisis de patrones generado"
...
async def test_handle_trend_identification():
    ...
    with patch.object(biometrics_insight_engine_adapter, '_generate_response') as mock_generate_response:
        ...
        assert result["context"]["trend_analyses"][0]["analysis"] == "Análisis de tendencias generado"
...
async def test_handle_data_visualization():
    ...
    with patch.object(biometrics_insight_engine_adapter, '_generate_response') as mock_generate_response:
        ...
        assert result["context"]["visualizations"][0]["description"] == "Descripción de visualización generada"
...
async def test_handle_generic_query():
    ...
    with patch.object(biometrics_insight_engine_adapter, '_generate_response') as mock_generate_response:
        ...
        assert result["context"]["conversation_history"][0]["response"] == "Respuesta genérica generada"
