...
class TestMotivationBehaviorCoachAdapter:
    ...
    def adapter(self):
        ...
        with patch('infrastructure.adapters.motivation_behavior_coach_adapter.MotivationBehaviorCoach.__init__', return_value=None):
    ...
    def test_determine_query_type(self, adapter):
        ...
        query_type = adapter._determine_query_type("Quiero crear un hábito de meditación diaria")
        assert query_type == "habit_formation"
        
        # Prueba para estrategias de motivación
        query_type = adapter._determine_query_type("Necesito motivación para seguir con mi dieta")
        assert query_type == "motivation_strategies"
        
        # Prueba para cambio de comportamiento
        query_type = adapter._determine_query_type("Quiero cambiar mi comportamiento sedentario")
        assert query_type == "behavior_change"
        
        # Prueba para establecimiento de metas
        query_type = adapter._determine_query_type("Ayúdame a establecer una meta para correr un maratón")
        assert query_type == "goal_setting"
        
        # Prueba para manejo de obstáculos
        query_type = adapter._determine_query_type("Tengo un obstáculo para mantener mi rutina de ejercicio")
    ...
    async def test_handle_habit_formation(self, adapter):
        ...
        result = await adapter._handle_habit_formation(query, context, profile, program_type)
    ...
    async def test_handle_motivation_strategies(self, adapter):
        ...
        result = await adapter._handle_motivation_strategies(query, context, profile, program_type)
        ...
        assert result["context"]["motivation_strategies"][0]["program_type"] == program_type
    ...
    async def test_handle_behavior_change(self, adapter):
        ...
        result = await adapter._handle_behavior_change(query, context, profile, program_type)
        ...
        assert result["context"]["behavior_change_plans"][0]["program_type"] == program_type
    ...
    async def test_handle_goal_setting(self, adapter):
        ...
        result = await adapter._handle_goal_setting(query, context, profile, program_type)
    ...
    async def test_handle_obstacle_management(self, adapter):
        ...
        result = await adapter._handle_obstacle_management(query, context, profile, program_type)
        ...
        assert result["context"]["obstacle_management_plans"][0]["program_type"] == program_type
    ...
    async def test_handle_generic_query(self, adapter):
        ...
        result = await adapter._handle_generic_query(query, context, profile, program_type)
