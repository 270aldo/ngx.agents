...
def mock_opentelemetry():
    ...
    with (
        patch("core.telemetry.Resource") as mock_resource,
...
def settings_with_telemetry():
    ...
    return Settings(
        telemetry_enabled=True, gcp_project_id="test-project-id", environment="test"
    )
...
def settings_without_telemetry():
    ...
    return Settings(
        telemetry_enabled=False, gcp_project_id="test-project-id", environment="test"
    )
...
def test_initialize_telemetry_when_disabled(
    mock_opentelemetry, settings_without_telemetry
):
...
def test_shutdown_telemetry_when_disabled(
    mock_opentelemetry, settings_without_telemetry
):
...
@pytest.mark.parametrize(
    "telemetry_enabled,expected_calls",
    [
...
def test_exception_handler_with_telemetry_enabled():
    ...
    with (
        ...
        mock_record_exception.assert_called_once_with(
            exception,
            {
