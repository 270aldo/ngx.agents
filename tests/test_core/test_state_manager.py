...
async def test_update_existing_state(
    state_manager: StateManager, test_settings: Dict[str, Any]
):
...
@pytest.mark.asyncio
async def test_get_state_field(
    state_manager: StateManager, test_settings: Dict[str, Any]
):
    ...
    theme = await state_manager.get_state_field(
        user_id, session_id, "user.preferences.theme"
    )
    count = await state_manager.get_state_field(
        user_id, session_id, "conversation.count"
    )
    messages = await state_manager.get_state_field(
...
async def test_update_state_field(
    ...
    await state_manager.update_state_field(
