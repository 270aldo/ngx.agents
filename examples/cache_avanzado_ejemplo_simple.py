...
sys.modules['core.telemetry'] = type('MockTelemetryModule', (), {
...
async def ejemplo_basico():
    ...
    async def mock_generate_content(prompt, temperature=0.7, max_output_tokens=100, cache_namespace=None):
...
async def ejemplo_invalidacion():
    ...
    for usuario in usuarios:
        ...
        logger.info(f"Usuario: {usuario}, Duración: {duration:.2f}ms (esperado: {expected})")
...
async def ejemplo_estadisticas():
    ...
    if not isinstance(stats, dict):
        logger.warning(f"Las estadísticas no son un diccionario: {type(stats).__name__}")
        stats = {}
    
    # Obtener el diccionario de caché o un diccionario vacío si no existe
    cache_stats = stats.get('cache', {})
    if not isinstance(cache_stats, dict):
        logger.warning(f"Las estadísticas de caché no son un diccionario: {type(cache_stats).__name__}")
        cache_stats = {}
...
async def main():
    """Función principal."""
    logger.info("Iniciando ejemplos de uso del sistema de caché avanzado (versión simplificada)")
    ...
    try:
        ...
        if not isinstance(stats, dict):
            logger.warning(f"Las estadísticas finales no son un diccionario: {type(stats).__name__}")
            stats = {}
        
        # Obtener el diccionario de caché o un diccionario vacío si no existe
        cache_stats = stats.get('cache', {})
        if not isinstance(cache_stats, dict):
            logger.warning(f"Las estadísticas finales de caché no son un diccionario: {type(cache_stats).__name__}")
            cache_stats = {}
