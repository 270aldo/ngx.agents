...
class VertexAIClientWithTelemetry:
    ...
    async def initialize(self) -> bool:
        ...
        add_span_event(None, "vertex_ai.initialized", {
            "success": True,
            "timestamp": time.time()
        })
    ...
    async def generate_content(
        ...
        span = start_span("vertex_ai.content_generation", {
        ...
        try:
            ...
            record_metric("vertex_ai.tokens.completion", response["usage"]["completion_tokens"])
            ...
            set_span_attribute(span, "prompt_tokens", response["usage"]["prompt_tokens"])
            set_span_attribute(span, "completion_tokens", response["usage"]["completion_tokens"])
            set_span_attribute(span, "total_tokens", response["usage"]["total_tokens"])
            set_span_attribute(span, "latency_ms", latency_ms)
            
            # Registrar finalización
            add_span_event(span, "generation.complete", {
    ...
    async def generate_embedding(self, text: str) -> List[float]:
        ...
        # Crear span para la operación
        span = start_span("vertex_ai.embedding_generation", {
        ...
        try:
            ...
            # Registrar finalización
            add_span_event(span, "embedding.complete", {
    ...
    async def get_stats(self) -> Dict[str, Any]:
        ...
        for operation, latencies in self.stats["latency_ms"].items():
            if latencies:
                self.stats[f"latency_avg_ms_{operation}"] = sum(latencies) / len(latencies)
