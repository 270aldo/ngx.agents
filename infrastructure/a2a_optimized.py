    ...
    async def execute(self, func: Callable, *args, **kwargs) -> Any:
        ...
        async with self._lock:
            ...
            if self.state == CircuitBreakerState.OPEN:
                ...
                if time.time() - self.last_failure_time > self.recovery_timeout:
                    logger.info(f"Circuit Breaker '{self.name}' cambiando a estado HALF_OPEN")
            ...
            if self.state == CircuitBreakerState.HALF_OPEN and self.half_open_calls >= self.half_open_max_calls:
                self.prevented_calls += 1
                raise Exception(f"Circuit Breaker '{self.name}' ha alcanzado el máximo de llamadas en HALF_OPEN")
        ...
        try:
            ...
            async with self._lock:
                ...
                if self.state == CircuitBreakerState.HALF_OPEN:
                    logger.info(f"Circuit Breaker '{self.name}' recuperado, cambiando a CLOSED")
                    self.state = CircuitBreakerState.CLOSED
                    self.failure_count = 0
            ...
            async with self._lock:
                ...
                if self.state == CircuitBreakerState.CLOSED and self.failure_count >= self.failure_threshold:
                    logger.warning(f"Circuit Breaker '{self.name}' cambiando a estado OPEN")
                    self.state = CircuitBreakerState.OPEN
                    
                # Si estamos en half-open y falla, volver a abrir
                elif self.state == CircuitBreakerState.HALF_OPEN:
                    logger.warning(f"Circuit Breaker '{self.name}' fallo en recuperación, volviendo a OPEN")
                    self.state = CircuitBreakerState.OPEN
    ...
    async def send_message(self, 
        ...
        try:
            ...
            if to_agent_id not in self.agent_queues:
                ...
                telemetry_manager.set_span_attribute(span_id, "error", "agent_not_registered")
    ...
    async def _process_messages(self, agent_id: str) -> None:
        ...
        while self.running:
            try:
                ...
                if message:
                    ...
                    try:
                        ...
                        if handler:
                            ...
                            logger.warning(f"No hay manejador registrado para agente '{agent_id}'")
                            telemetry_manager.set_span_attribute(span_id, "error", "no_handler")
                            
                    except Exception as e:
                        logger.error(f"Error al procesar mensaje para agente '{agent_id}': {str(e)}")
                        telemetry_manager.set_span_attribute(span_id, "error", str(e))
                ...
                logger.info(f"Procesamiento de mensajes cancelado para agente '{agent_id}'")
                break
                
            except Exception as e:
                logger.error(f"Error en bucle de procesamiento para agente '{agent_id}': {str(e)}")
