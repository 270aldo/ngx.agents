...
class HealthCheck:
    ...
    def register_dependency(self, name: str, check_func: callable, critical: bool = True) -> None:
        """
        ...
        logger.info(f"Dependencia registrada para health check: {name} (crítica: {critical})")
    ...
    async def check_a2a_optimized(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            if stats.get("running", False):
                ...
                return True, f"Servidor A2A optimizado activo con {num_agents} agentes registrados"
            else:
...
health_check.register_dependency("a2a_optimized", health_check.check_a2a_optimized, critical=True)
