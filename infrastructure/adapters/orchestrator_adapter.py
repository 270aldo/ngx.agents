...
class OrchestratorAdapter(NGXNexusOrchestrator):
    ...
    async def _run_async_impl(self, user_input: str, user_id: Optional[str] = None, 
        ...
        try:
            with telemetry.start_span("orchestrator.run"):
                ...
                target_agents, priority = await self._determine_target_agents(intent, user_input, context)
                ...
                response = await self._route_message(user_input, target_agents, priority, user_id, session_id, context)
                end_time = time.time()
            ...
            telemetry.record_error("orchestrator", "run_failed", {
                "error": str(e),
                "user_id": user_id,
                "session_id": session_id
            })
    ...
    async def _analyze_intent(self, user_input: str, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            with telemetry.start_span("orchestrator.analyze_intent"):
                ...
                # Registrar telemetría
                telemetry.record_event("orchestrator", "intent_analyzed", {
            ...
            telemetry.record_error("orchestrator", "intent_analysis_failed", {
                "error": str(e)
            })
    ...
    async def _determine_target_agents(self, intent: Dict[str, Any], user_input: str, context: Dict[str, Any]) -> Tuple[List[str], MessagePriority]:
        """
        ...
        try:
            with telemetry.start_span("orchestrator.determine_target_agents"):
                ...
                priority = self.PRIORITY_LEVELS.get(priority_str, MessagePriority.NORMAL)
                
                # Optimización: Ajustar la prioridad basada en palabras clave de emergencia
                emergency_keywords = ["emergencia", "urgente", "crítico", "inmediato", "peligro"]
                ...
                # Registrar telemetría
                telemetry.record_event("orchestrator", "target_agents_determined", {
                    "target_agents": target_agents,
            ...
            logger.error(f"Error al determinar los agentes objetivo: {e}", exc_info=True)
            telemetry.record_error("orchestrator", "target_determination_failed", {
                "error": str(e)
            })
            # Devolver un agente por defecto y prioridad normal en caso de error
            return ["fallback_agent"], MessagePriority.NORMAL
    
    async def _route_message(self, user_input: str, target_agents: List[str], priority: MessagePriority, 
                           user_id: Optional[str], session_id: Optional[str], context: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        try:
            with telemetry.start_span("orchestrator.route_message"):
                ...
                if len(target_agents) > 1:
                    ...
                    combined_response = self._combine_responses(responses, target_agents)
                    ...
                    try:
                        ...
                        telemetry.record_error("orchestrator", "agent_call_timeout", {
                            "agent_id": agent_id,
                            "timeout": timeout
                        })
            ...
            telemetry.record_error("orchestrator", "message_routing_failed", {
                "error": str(e),
                "target_agents": target_agents
            })
    ...
    async def _call_multiple_agents_parallel(self, user_input: str, agent_ids: List[str], 
                                           priority: MessagePriority, context: Dict[str, Any]) -> Dict[str, Dict[str, Any]]:
        """
        ...
        try:
            with telemetry.start_span("orchestrator.call_multiple_agents"):
                ...
                # Registrar telemetría
                telemetry.record_event("orchestrator", "multiple_agents_called", {
                    "agent_count": len(agent_ids),
                    "success_count": sum(1 for r in responses.values() if r.get("status") != "error"),
                    "error_count": sum(1 for r in responses.values() if r.get("status") == "error"),
                    "response_time_ms": telemetry.get_current_span().duration_ms
                })
            ...
            telemetry.record_error("orchestrator", "multiple_agents_call_failed", {
                "error": str(e),
                "agent_ids": agent_ids
            })
    ...
    async def _safe_call_agent(self, agent_id: str, query: str, context: Dict[str, Any], timeout: int) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            logger.error(f"Error al llamar al agente {agent_id}: {e}", exc_info=True)
            telemetry.record_error("orchestrator", "agent_call_failed", {
                "agent_id": agent_id,
                "error": str(e)
            })
    ...
    def _combine_responses(self, responses: Dict[str, Dict[str, Any]], agent_ids: List[str]) -> Dict[str, Any]:
        """
        ...
        try:
            with telemetry.start_span("orchestrator.combine_responses"):
                ...
                all_errors = all(response.get("status") == "error" for response in responses.values())
                if all_errors:
                    return {
                        "status": "error",
                ...
                # Registrar telemetría
                telemetry.record_event("orchestrator", "responses_combined", {
            ...
            telemetry.record_error("orchestrator", "response_combination_failed", {
                "error": str(e)
            })
    ...
    def _update_metrics(self, target_agents: List[str], response_time: float) -> None:
        ...
        try:
            ...
            telemetry.record_event("orchestrator", "metrics_updated", {
    ...
    async def _consult_other_agent(self, agent_id: str, query: str, user_id: Optional[str] = None, 
        ...
        try:
            with telemetry.start_span(f"orchestrator.consult_{agent_id}"):
                ...
                # Registrar telemetría
                telemetry.record_event("orchestrator", "agent_consulted", {
                    "agent_id": agent_id,
            ...
            telemetry.record_error("orchestrator", "agent_consultation_failed", {
                "agent_id": agent_id,
                "error": str(e)
            })
