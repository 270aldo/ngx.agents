...
class SystemsIntegrationOpsAdapter(SystemsIntegrationOps, BaseAgentAdapter):
    ...
    async def _process_query(
        ...
        **kwargs,
        ...
        try:
            ...
            logger.info(
                f"SystemsIntegrationOpsAdapter procesando consulta de tipo: {query_type}"
            )
            ...
            if query_type == "integration_request":
                result = await self._handle_integration_request(
                    query, context, profile, program_type
                )
            elif query_type == "automation_request":
                result = await self._handle_automation_request(
                    query, context, profile, program_type
                )
                ...
                result = await self._handle_infrastructure_request(
                    query, context, profile, program_type
                )
            elif query_type == "data_pipeline_request":
                result = await self._handle_data_pipeline_request(
                    query, context, profile, program_type
                )
            else:
            ...
            logger.error(
                f"Error al procesar consulta en SystemsIntegrationOpsAdapter: {str(e)}",
                exc_info=True,
            )
            return {
                "success": False,
    ...
    def _determine_query_type(self, query: str) -> str:
        ...
        if any(
            word in query_lower
            for word in ["sistema", "system", "integrar", "conectar"]
        ):
            return "integration_request"
        elif any(
            word in query_lower for word in ["automatizar", "proceso", "workflow"]
        ):
            ...
            word in query_lower
            for word in ["infraestructura", "servidor", "arquitectura"]
        ):
    ...
    async def _handle_infrastructure_request(
        self,
        query: str,
        context: Dict[str, Any],
        profile: Dict[str, Any],
    ...
    async def _handle_data_pipeline_request(
        self,
        query: str,
        context: Dict[str, Any],
        profile: Dict[str, Any],
    ...
    async def _handle_general_request(
        ...
        return {"response": general_response, "context": context}
