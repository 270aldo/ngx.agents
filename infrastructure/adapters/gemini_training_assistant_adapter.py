...
class GeminiTrainingAssistantAdapter(GeminiTrainingAssistant, BaseAgentAdapter):
    ...
    async def _process_query(
        ...
        **kwargs,
        ...
        try:
            ...
            logger.info(
                f"GeminiTrainingAssistantAdapter procesando consulta de tipo: {query_type}"
            )
            ...
            if query_type == "generate_training_plan":
                result = await self._handle_training_plan(
                    query, context, profile, program_type
                )
            elif query_type == "recommend_nutrition":
                result = await self._handle_nutrition_recommendation(
                    query, context, profile, program_type
                )
            elif query_type == "analyze_progress":
                result = await self._handle_progress_analysis(
                    query, context, profile, program_type
                )
            else:
                # Tipo de consulta no reconocido, usar procesamiento genérico
                result = await self._handle_fitness_question(
                    query, context, profile, program_type
                )
            ...
            logger.error(
                f"Error al procesar consulta en GeminiTrainingAssistantAdapter: {str(e)}",
                exc_info=True,
            )
            return {
                "success": False,
    ...
    async def _handle_training_plan(
        self,
        query: str,
        ...
        return {"response": training_plan_response, "context": context}

    async def _handle_nutrition_recommendation(
        self,
        query: str,
        context: Dict[str, Any],
        profile: Dict[str, Any],
        ...
        return {"response": nutrition_response, "context": context}

    async def _handle_progress_analysis(
        self,
        query: str,
        context: Dict[str, Any],
        profile: Dict[str, Any],
        ...
        return {"response": progress_analysis_response, "context": context}

    async def _handle_fitness_question(
        self,
        query: str,
        context: Dict[str, Any],
        profile: Dict[str, Any],
        ...
        return {"response": fitness_response, "context": context}
