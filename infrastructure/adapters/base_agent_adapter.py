...
class BaseAgentAdapter(Agent):
    ...
    async def _classify_query(self, query: str, user_id: str = None, 
        ...
        if tracer:
            context_manager = tracer.start_as_current_span(f"{self.__class__.__name__}._classify_query")
        else:
        ...
        with context_manager:
            try:
                ...
                if self._has_excluded_keywords(query):
                    combined_score *= 0.5  # Reducir puntuación si hay palabras clave excluidas
                
                # Paso 5: Aplicar ajustes específicos del contexto
                context_adjusted_score = self._adjust_score_based_on_context(combined_score, context)
    ...
    def _check_keywords(self, query: str) -> float:
        ...
        matched_keywords = [kw for kw in self.fallback_keywords if kw.lower() in query_lower]
    ...
    async def _get_program_type_from_profile(self, profile: Dict[str, Any]) -> str:
        ...
        try:
            ...
            program_type = await self.program_classification_service.classify_profile(profile)
            return program_type
        except Exception as e:
            logger.error(f"Error al determinar el tipo de programa: {str(e)}", exc_info=True)
    ...
    async def run_async_impl(self, query: str, **kwargs) -> Dict[str, Any]:
        ...
        # Usar span de telemetría si está disponible
        if tracer:
            context_manager = tracer.start_as_current_span(f"{self.__class__.__name__}.run_async_impl")
        else:
            # Usar un context manager nulo si la telemetría no está disponible
            from contextlib import nullcontext
            context_manager = nullcontext()
