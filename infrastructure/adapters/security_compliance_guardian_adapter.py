...
class SecurityComplianceGuardianAdapter(SecurityComplianceGuardian, BaseAgentAdapter):
    ...
    def _adjust_score_based_on_context(
        ...
        if (
            context.get("security_queries")
            and len(context.get("security_queries", [])) > 0
        ):
            score += 0.1  # Aumentar la puntuación si hay consultas de seguridad previas

        # Verificar si hay evaluaciones de seguridad previas
        if (
            context.get("security_assessments")
            and len(context.get("security_assessments", [])) > 0
        ):
            score += (
                0.1  # Aumentar la puntuación si hay evaluaciones de seguridad previas
            )

        # Verificar si hay verificaciones de cumplimiento previas
        if (
            context.get("compliance_checks")
            and len(context.get("compliance_checks", [])) > 0
        ):
        ...
        **kwargs,
        ...
        try:
            ...
            if query_type == "security_assessment":
                response = await self._process_security_assessment(
                    query, state, profile, program_type
                )
            elif query_type == "compliance_check":
                response = await self._process_compliance_check(
                    query, state, profile, program_type
                )
            elif query_type == "vulnerability_scan":
                response = await self._process_vulnerability_scan(
                    query, state, profile, program_type
                )
            elif query_type == "data_protection":
                response = await self._process_data_protection(
                    query, state, profile, program_type
                )
            else:
    ...
    async def _process_security_assessment(
        self,
        query: str,
        state: Dict[str, Any],
        profile: Dict[str, Any],
    ...
    async def _process_compliance_check(
        self,
        query: str,
        state: Dict[str, Any],
        profile: Dict[str, Any],
        ...
        state["compliance_checks"].append(
            {"date": datetime.now().isoformat(), "query": query, "check": response_text}
        )
    ...
    async def _process_vulnerability_scan(
        ...
        state["vulnerability_scans"].append(
            {"date": datetime.now().isoformat(), "query": query, "scan": response_text}
        )
    ...
    async def _process_data_protection(
        self,
        query: str,
        state: Dict[str, Any],
        profile: Dict[str, Any],
    ...
    async def _process_generic_query(
        ...
        state["security_queries"].append(
            {"date": datetime.now().isoformat(), "query": query, "type": "general"}
        )
